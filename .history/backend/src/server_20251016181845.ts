...
await cognitiveGenerationEngine.initialize({
  ...
  aiProvider: process.env['AI_PROVIDER'] || 'perplexity',  // ✅ LÊ DO .env!
  aiModel: process.env['PERPLEXITY_MODEL'] || 'llama-3.1-sonar-large-128k-online',  // ✅ MODELO PERPLEXITY!
...
this.app.post('/api/generate/direct', async (req, res) => {
  try {
    ...
    // ✅ CORRIGIDO: Usar groqService.complete()
    const groqResponse = await groqService.complete({
      systemPrompt: `You are an expert ${language || 'TypeScript'} developer specializing in ${framework || 'React'} applications. Generate clean, production-ready code following best practices.`,
      userPrompt: `Generate ${language || 'TypeScript'} code for: ${prompt}
