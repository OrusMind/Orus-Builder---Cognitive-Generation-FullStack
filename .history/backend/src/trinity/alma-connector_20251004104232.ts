 ...
 * 🏷️ COMPONENT_HASH: orus.builder.trinity.alma.20251004.v1.AC024
 * 
 * ═══════════════════════════════════════════════════════════════
...
export class AlmaConnector {
  ...
  private async queryAlma(almaQuery: AlmaQuery): Promise<AlmaResponse> {
    ...
    if (!response.success) {
      throw new Error(response.metadata?.error || 'Alma query failed');
    }
  ...
  private async indexToAlma(
    ...
    if (!response.success) {
      throw new Error(response.metadata?.error || 'Alma indexing failed');
    }
