...
this.app.post('/api/test/testing/generate', async (req, res) => {
  try {
    ...
    // ✅ FIX COMPLETO: Usar estrutura correta do testing-engine
    const result = await testingEngine.generateTests({
      ...
      sourceFiles: [{
        ...
        content: code,
        language: 'typescript',
        type: 'component' as any // ✅ Adicionar type obrigatório
      }],
      testType: testType || 'unit',
      framework: 'jest' as any, // ✅ Type assertion
      coverage: true,
    ...
    // ✅ Acessar via result diretamente (não result.data)
    res.json({
      success: true,
      engine: 'Testing',
