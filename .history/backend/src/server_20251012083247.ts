...
this.app.post('/api/test/deployment/deploy', async (req, res) => {
  try {
    ...
    // ✅ FIX: Use correct DeploymentRequest structure
    const result = await deploymentEngine.deploy({
      ...
      outputDir: projectPath || './dist', // ✅ Use outputDir instead of projectPath
      environment: 'staging',
      envVars: {},
      enableRollback: true,
    ...
    res.json({
      success: true,
      engine: 'Deployment',
      deploymentUrl: result.data?.deploymentUrl || 'https://staging.example.com', // ✅ Access via data
      buildTime: result.data?.buildDuration || 0, // ✅ Use buildDuration
      status: result.data?.status || 'success', // ✅ Access via data
...
this.app.get('/api/test/monitoring/metrics', async (req, res) => {
  try {
    // ✅ FIX: Use getMetrics or similar method instead of collectMetrics
    const result = await monitoringEngine.getMetrics({
      requestId: 'metrics-' + Date.now(),
      timeRange: {
        start: new Date(Date.now() - 3600000), // Last hour
        end: new Date()
      },
      metricTypes: ['performance', 'system', 'application'],
      aggregation: 'avg',
      interval: 60, // seconds
      version: 1,
      isDeleted: false,
    ...
    res.json({
      ...
      engine: 'Monitoring',
      metrics: {
        cpu: result.data?.systemMetrics?.cpu || 0,
        memory: result.data?.systemMetrics?.memory || 0,
        activeConnections: result.data?.applicationMetrics?.connections || 0,
        requestRate: result.data?.performanceMetrics?.requestsPerSecond || 0,
        errorRate: result.data?.performanceMetrics?.errorRate || 0
      },
      alerts: result.data?.activeAlerts || [],
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
    res.status(500).json({
