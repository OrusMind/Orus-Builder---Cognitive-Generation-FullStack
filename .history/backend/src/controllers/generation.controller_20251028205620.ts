...
/**
 * ✅ FIX #4.2.7: Normaliza paths E concatena filename
 * CORRIGE duplicação de paths
 */
private normalizeFilePaths(files: any[]): any[] {
  console.log('═══════════════════════════════════════════════════════');
  console.log('🔧 Normalizing file paths v4.2.7...');
  console.log('═══════════════════════════════════════════════════════');
  ...
  return files.map((file, index) => {
    ...
    // STEP 1: Se path JÁ tem extensão, não precisa de filename
    if (basePath.includes('.')) {
      // Path já é completo (ex: 'frontend/src/App.tsx')
      console.log(`  [${index + 1}/${files.length}] ✅ ${basePath} (already complete)`);
      return {
        ...file,
        path: basePath,
        name: filename || basePath.split('/').pop() || 'unknown',
        originalPath: originalPath
      };
    }
    
    // STEP 2: Se NÃO tem filename, tenta extrair do path
    if (!filename || filename.trim() === '') {
      // Se path termina com nome (sem extensão), adiciona .ts
      const pathParts = basePath.split('/');
      const lastPart = pathParts[pathParts.length - 1];
      
      if (lastPart && !lastPart.includes('.')) {
        // Exemplo: 'src/ItemController' → filename = 'ItemController.ts'
        filename = `${lastPart}.ts`;
        // Remove o lastPart do path para não duplicar
        pathParts.pop();
        basePath = pathParts.join('/');
        console.log(`  [${index + 1}/${files.length}] 🔧 Extracted filename: ${filename} from path`);
      } else {
        // Fallback
        filename = `unknown-${index}.tsx`;
        console.warn(`  [${index + 1}/${files.length}] ⚠️ No filename found for: ${basePath}`);
      }
    }
    
    // STEP 3: Normaliza basePath
    basePath = basePath.replace(/^\/\*[\s\S]*?\*\//, '');
    
    if (basePath.startsWith('src/frontend')) {
      basePath = basePath.replace('src/frontend', 'frontend/src');
    }
    
    while (basePath.includes('src/src')) basePath = basePath.replace('src/src', 'src');
    while (basePath.includes('frontend/frontend')) basePath = basePath.replace('frontend/frontend', 'frontend');
    while (basePath.includes('backend/backend')) basePath = basePath.replace('backend/backend', 'backend');
    ...
    // STEP 4: Adiciona prefixo frontend se necessário
    const isFrontendFile = filename.endsWith('.tsx') || filename.endsWith('.jsx') || 
                          filename.endsWith('.css') || basePath.includes('component') ||
                          basePath.includes('page') || basePath.includes('types') ||
                          basePath.includes('utils') || basePath.includes('hooks');
    
    if (isFrontendFile && !basePath.startsWith('frontend') && 
        !basePath.startsWith('backend') && basePath !== '') {
      basePath = `frontend/${basePath}`;
      console.log(`  [${index + 1}/${files.length}] 📁 Added frontend prefix: ${basePath}`);
    }
    
    // STEP 5: Adiciona prefixo backend se necessário
    if (!basePath.startsWith('frontend') && !basePath.startsWith('backend') && basePath !== '') {
      basePath = `backend/${basePath}`;
      console.log(`  [${index + 1}/${files.length}] 📁 Added backend prefix: ${basePath}`);
    }
    
    // STEP 6: CONCATENA path + filename (FIX PRINCIPAL!)
    ...
    if (originalPath !== fullPath) {
      ...
      console.log(`  [${index + 1}/${files.length}] ✅ ${fullPath}`);
    }
