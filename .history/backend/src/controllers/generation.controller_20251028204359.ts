...
/**
 * ✅ FIX #4.2.6: Normaliza paths E concatena filename
 * MANTÉM estrutura frontend/backend
 * CONCATENA path + name para paths completos
 */
private normalizeFilePaths(files: any[]): any[] {
  console.log('═══════════════════════════════════════════════════════');
  console.log('🔧 Normalizing file paths v4.2...');
  console.log('═══════════════════════════════════════════════════════');
  
  return files.map((file, index) => {
    let basePath = file.path || '';
    let filename = file.name || file.filename || `unknown-${index}.tsx`;
    ...
    if (basePath.startsWith('src/frontend')) {
      ...
      console.log(`[normalizeFilePaths] Fixed Groq path: from ${originalPath} to ${basePath}`);
    }
    
    // STEP 3: Remove duplicates
    while (basePath.includes('src/src')) {
      basePath = basePath.replace('src/src', 'src');
    }
    while (basePath.includes('frontend/frontend')) {
      basePath = basePath.replace('frontend/frontend', 'frontend');
    }
    while (basePath.includes('backend/backend')) {
      basePath = basePath.replace('backend/backend', 'backend');
    }
    
    // STEP 4: Normalize slashes
    ...
    // STEP 6: Add frontend prefix for frontend files if missing
    ...
    if (isFrontendFile && !basePath.startsWith('frontend') && 
        !basePath.startsWith('backend') && !basePath.startsWith('src/backend')) {
      if (basePath.startsWith('src')) {
        basePath = `frontend/${basePath}`;
        console.log(`[normalizeFilePaths] Added frontend prefix: ${basePath}`);
      }
    }
    
    // STEP 7: DETECTA se path JÁ tem extensão (filename)
const hasExtension = /\.(ts|tsx|js|jsx|css|json|md|sql|prisma)$/i.test(basePath);

let fullPath = basePath;

if (hasExtension) {
  // Path JÁ TEM filename completo, NÃO concatena!
  console.log(`  [${index + 1}/${files.length}] ✅ ${basePath} (already complete)`);
} else {
  // Path NÃO tem filename, CONCATENA!
  ...
  if (originalPath !== fullPath) {
    ...
    return {
      ...file,
      path: fullPath,        // ← PATH COMPLETO COM FILENAME!
      name: filename,
