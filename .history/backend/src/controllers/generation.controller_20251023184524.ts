...
/**
 * ═════════════════════════════════════════════════════════════════════════
 * HELPER: Normalize File Paths (VERSÃO CORRIGIDA v2.0)
 * ═════════════════════════════════════════════════════════════════════════
 * CORRIGE: frontend/src/src/ → frontend/src/
 * CORRIGE: Paths do Groq AI que vêm como src/frontend/...
 */
private normalizeFilePaths(files: any[]): any[] {
  ...
  return files.map((file, index) => {
    ...
    if (path.startsWith('src/frontend/')) {
      // Extrair o path relativo após 'src/frontend/'
      const relativePath = path.replace('src/frontend/', '');
      path = `frontend/src/${relativePath}`;
      
      console.log('[normalizeFilePaths] Fixed Groq path:', {
        from: originalPath,
        to: path
      });
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 3: Fix backend paths (src/backend/ → backend/src/)
    // ═══════════════════════════════════════════════════════════════════════
    if (path.startsWith('src/backend/')) {
      const relativePath = path.replace('src/backend/', '');
      path = `backend/src/${relativePath}`;
      
      console.log('[normalizeFilePaths] Fixed backend path:', {
        from: originalPath,
        to: path
      });
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 4: ✅ FIX CRÍTICO - Remover src/ duplicado
    // ═══════════════════════════════════════════════════════════════════════
    if (path.includes('/src/src/')) {
      path = path.replace('/src/src/', '/src/');
      console.log('[normalizeFilePaths] ✅ Removed duplicate src/:', {
        from: originalPath,
        to: path
      });
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 5: Ensure paths start with frontend/ or backend/
    // ═══════════════════════════════════════════════════════════════════════
    if (!path.startsWith('frontend/') && !path.startsWith('backend/')) {
      // Detect type by file extension or folder
      const isFrontend =
        path.endsWith('.tsx') ||
        path.endsWith('.jsx') ||
        path.endsWith('.css') ||
      ...
      console.log('[normalizeFilePaths] Added layer prefix:', {
        from: originalPath,
        to: path,
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 6: ✅ VALIDAÇÃO FINAL - Garantir estrutura correta
    // ═══════════════════════════════════════════════════════════════════════
    // Padrões válidos:
    ...
    // Se não tem src/ após frontend/ ou backend/, adicionar
    if (path.startsWith('frontend/') && !path.startsWith('frontend/src/') && !path.startsWith('frontend/public/')) {
      const afterFrontend = path.replace('frontend/', '');
      path = `frontend/src/${afterFrontend}`;
      console.log('[normalizeFilePaths] Ensured frontend/src/:', path);
    }

    if (path.startsWith('backend/') && !path.startsWith('backend/src/')) {
      const afterBackend = path.replace('backend/', '');
      path = `backend/src/${afterBackend}`;
      console.log('[normalizeFilePaths] Ensured backend/src/:', path);
    }

    // ═══════════════════════════════════════════════════════════════════════
    // STEP 7: ✅ ÚLTIMA VERIFICAÇÃO - Remover qualquer src/src/ restante
    // ═══════════════════════════════════════════════════════════════════════
    while (path.includes('/src/src/')) {
      path = path.replace('/src/src/', '/src/');
      console.warn('[normalizeFilePaths] ⚠️ Removed ANOTHER duplicate src/!');
    }
...
const allFiles: any[] = this.normalizeFilePaths([...generatedFiles]);
