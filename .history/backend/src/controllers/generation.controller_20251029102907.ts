...
/**
 * FIX #4.2.9: Normaliza um arquivo individual COM CONCATENAÇÃO CORRETA
 */
private normalizeFile(file: any, generatorName: string): any {
  ...
  // ✅ FIX: Se filename não tem extensão, adiciona .ts
  if (!filename.includes('.')) {
    console.log(`[normalizeFile] ⚠️ No extension for: ${filename}`);
    
    // Detecta tipo pelo path ou conteúdo
  ...
  // Se basePath JÁ tem extensão, é o arquivo completo - extrair
  if (basePath.includes('.')) {
    const parts = basePath.split('/');
    const lastPart = parts[parts.length - 1];
    
    // Se último part tem extensão, é o filename
  ...
  // ✅ CONCATENA path + filename (SEM VERIFICAÇÕES COMPLEXAS)
  // Se basePath já tem o filename, não concatena novamente
  ...
  // ✅ RETORNA SEM SPREAD PARA EVITAR SOBRESCRITA
  return {
...
private normalizeFilePaths(files: any[]): any[] {
  ...
  return files.map((file, index) => {
    // Usa normalizeFile() para cada arquivo
    const normalized = this.normalizeFile(file, 'controller');
    
    console.log(`  [${index + 1}/${files.length}] ✅ ${normalized.path}`);
...
allFiles.forEach((file: any, index: number) => {
  // ✅ CONCATENAR path + filename AQUI
  const filePath = file.path && file.name 
                 ? `${file.path}/${file.name}`
                 : file.path || file.name || `file-${index}.tsx`;
