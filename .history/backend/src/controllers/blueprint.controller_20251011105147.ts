 ...
 * COMPONENT_HASH: orus.builder.controller.blueprint.001.20251011
 * VERSION: 1.0
 * ═══════════════════════════════════════════════════════════════════════════
 * 
...
import multer from 'multer'; // ← Adicionar
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * TYPES & INTERFACES
 * ═══════════════════════════════════════════════════════════════════════════
...
public async uploadBlueprint(
  ...
  try {
    // ✅ FIX: Cast req to Multer Request
    ...
    this.processBlueprintPipeline(blueprintId, file.buffer).catch(err => {
      // ✅ FIX: Remove 'component' from error logger
      logger.error('Blueprint processing failed', {
        message: err.message,
        blueprintId
      });
  ...
  private async processBlueprintPipeline(blueprintId: string, fileBuffer: Buffer): Promise<void> {
    ...
    try {
      ...
      blueprint.status = 'failed';
      logger.error('Blueprint processing error', {
        component: 'BlueprintController',
        blueprintId,
        error: error.message
      });
