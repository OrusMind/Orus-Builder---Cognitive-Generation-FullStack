...
public async downloadGeneratedCode(
  ...
  try {
    ...
    allFiles.forEach((file: any) => {
      zip.file(file.path || file.name, file.code || file.content);
    });

    // ✅ ADICIONAR package.json E README.md SE EXISTIREM (SEM ACESSAR PROPRIEDADES QUE NÃO EXISTEM)
    // Remove essas linhas por enquanto ou adicione apenas se você tiver esses dados no cache
