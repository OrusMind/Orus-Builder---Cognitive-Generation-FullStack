...
public async generateFromPrompt(
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    const allFiles: GeneratedFile[] = [...result.files];
    ...
    if (result.files && result.files.length > 0) {
      ...
      if (firstFile && firstFile.content) {
        ...
        if (firstFile.metadata && (firstFile.metadata as any).additionalFiles) {
          const additionalFiles = (firstFile.metadata as any).additionalFiles as GeneratedFile[];
          ...
          logger.info(`📦 Found ${additionalFiles.length} additional files in metadata`, {
            ...
            metadata: {
              ...
              fileNames: additionalFiles.map(f => f.name || f.path).join(', ')
            }
          });
          ...
          additionalFiles.forEach((f: GeneratedFile) => {
    ...
    const totalLinesCount = allFiles.reduce((sum, file) => {
    ...
    const response: GenerationResultResponse = {
      ...
      files: allFiles,  // ✅ Agora inclui arquivos principais + extras
      metrics: {
        totalFiles: totalFilesCount,  // ✅ Recalculado
        totalLines: totalLinesCount,  // ✅ Recalculado
