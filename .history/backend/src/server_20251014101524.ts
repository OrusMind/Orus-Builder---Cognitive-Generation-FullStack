...
import { cognitiveGenerationEngine } from './engines/cognitive-generation-engine';import { promptEngine } from './engines/prompt-engine';
...
class Server {
  ...
  private async initializeEngines(): Promise<void> {
    ...
    try {
      ...
      console.log('  ✅ Engine 4/15: Cognitive Generation');
  ...
  private mountRoutes(): void {
    ...
    this.app.get('/health', async (req, res) => {
      ...
      const enginesStatus = {
        ...
        collaboration: collaborationEngine.getStatus() === 'running',
        security: securityEngine.getStatus() === 'running',
        marketplace: marketplaceEngine.getStatus() === 'running'
      };
...
this.app.post('/api/generate', async (req, res) => {
  try {
    const { prompt, mode, target, language, framework } = req.body;
    ...
    // ✅ ESTRUTURA EXATA do GenerationRequest
    const result = await cognitiveGenerationEngine.generate({
      ...
      // ✅ Input (baseado no enum real)
      mode: (mode || GenerationMode.FROM_PROMPT) as GenerationMode,
      target: (target || GenerationTarget.COMPONENT) as GenerationTarget,
      
      // ✅ Content (prompt obrigatório para FROM_PROMPT)
      prompt: prompt,
      
      // ✅ Configuration
      language: (language || 'typescript') as 'typescript' | 'javascript' | 'python' | 'java',
    ...
    // ✅ Response baseada em GeneratedCode
    return res.json({
      success: result.success,
      engine: 'Cognitive Generation',
      generationId: result.data?.generationId || 'unknown',
      files: result.data?.files || [],
      totalFiles: result.data?.totalFiles || 0,
      totalLines: result.data?.totalLines || 0,
      qualityScore: result.data?.qualityScore || 0,
      confidence: result.data?.confidence || 0,
      cigScore: result.data?.cigScore || 0,
      validated: result.data?.validated || false,
      generationTime: result.data?.generationTime || 0,
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
...
this.app.post('/api/generate/direct', async (req, res) => {
  try {
    ...
    const groqResponse = await groqService.generateCode({
      prompt: `Generate ${language || 'TypeScript'} code for: ${prompt}
...
Generate ONLY the code, no explanations.`,
    ...
    let generatedCode = groqResponse.content;
    ...
    return res.json({
      success: true,
      engine: 'Cognitive Generation (Direct Groq)',
...
this.app.post('/api/v1/generation', async (req, res) => {
  try {
    ...
    // Call Cognitive Generation Engine
   const groqResponse = await groqService.generateCode({
