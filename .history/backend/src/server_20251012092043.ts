...
this.app.post('/api/test/security/scan', async (req, res) => {
  try {
    ...
    // ✅ FIX: Usar SecurityScanRequest corretamente (files, não code)
    const result = await securityEngine.scan({
      ...
      // ✅ CORRETO: scanTypes e complianceStandards
    ...
    return res.json({
      ...
      compliant: result.data?.summary.compliant || true, // ✅ FIX: summary.compliant
      scans: {
        owasp: true,
        compliance: true,
...
this.app.get('/api/test/marketplace/plugins', async (req, res) => {
  try {
    // ✅ FIX: Usar search() ao invés de getPlugins()
    ...
    return res.json({
      success: result.success,
      engine: 'Marketplace',
      plugins: result.data?.listings || [],
      totalPlugins: result.data?.total || 0,
      features: {
        pluginRegistry: true,
        apiStore: true,
        billing: true,
