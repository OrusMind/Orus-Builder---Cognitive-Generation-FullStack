...
groq: true, // GroqService não tem getStatus, sempre ativo
    ...
    // ──────────────────────────────────────────────────────────────────────
    // ✅ FASE 3: AI ENGINES (NEW!)
    // ──────────────────────────────────────────────────────────────────────
    
    // Generation Engine (já existe na rota /api/v1/generation/test)
  ...
  userId: 'test-user',
  prompt,
  language: 'en', // ✅ Correto (idioma do prompt, não linguagem de programação)
  framework: 'none',
  version: 1,
  isDeleted: false,
  createdAt: new Date(),
  updatedAt: new Date()
    ...
    this.app.post('/test/blueprint', async (req, res) => {
      try {
        ...
        if (!content) {
          return res.status(400).json({ success: false, error: 'Blueprint content is required' });
        }
        
        const result = await blueprintEngine.process({
          id: 'test-' + Date.now(),
          requestId: 'test-' + Date.now(),
          userId: 'test-user',
          content,
