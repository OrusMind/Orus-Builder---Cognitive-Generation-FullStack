...
export class ExtensionManager {
  ...
  pluginId,
  userId,
  version: 1,                    // ✅ BaseEntity version (number)
 installedVersion: version || plugin.currentVersion,  // ✅ Fallback se version for undefined
 // ✅ String version do plugin
  status: ExtensionStatus.INSTALLED,
  installedAt: new Date(),
  autoUpdate: true,
  dependencies: [],              // ✅ Array vazio (string[])
  settings: {},
  isDeleted: false,              // ✅ ADICIONAR: BaseEntity property
  createdAt: new Date(),
  updatedAt: new Date()
};
...
// Record download - CORRIGIDO: usar installedVersion (string) não version (number)
await pluginRegistry.recordDownload(
  pluginId, 
  extension.installedVersion,    // ✅ CORRIGIDO: string
  userId
);
...
public async updateExtension(extensionId: string): Promise<InstallationResult> {
  ...
  // ✅ CORRIGIDO: Comparar installedVersion (string) com currentVersion (string)
  if (extension.installedVersion === plugin.currentVersion) {
    return { success: true, extensionId };
  }

  extension.status = ExtensionStatus.UPDATING;
  extension.installedVersion = plugin.currentVersion;  // ✅ CORRIGIDO: string = string
  ...
  plugin: PluginEntry
): Promise<{ resolved: boolean; missing?: string[] }> {  // ✅ CORRIGIDO: tipo correto
  // Simplified dependency resolution
  return { 
    resolved: true,   // ✅ boolean
    missing: []       // ✅ string[] (opcional)
  };
}
