...
this.app.post('/api/test/security/scan', async (req, res) => {
  try {
    ...
    const result = await securityEngine.scan({
      ...
      complianceStandards: [ComplianceStandard.GDPR], // ✅ Usar enum
...
this.app.get('/api/test/marketplace/plugins', async (req, res) => {
  try {
    // ✅ FIX: Usar valores corretos do enum ListingType
    const result = await marketplaceEngine.search({
      type: ['template', 'component'], // ✅ 'template' e 'component' existem no enum
      verifiedOnly: false,
      sortBy: 'popular',
      page: 1,
      pageSize: 10
