    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        return {
          ...
          function: hasFunction ? match[1] : '<anonymous>',
          lineno: parseInt(hasFunction ? match[3] : match[2]),
          colno: parseInt(hasFunction ? match[4] : match[3]),
          inApp: this.isAppCode(hasFunction ? match[2] : match[1])
        };
      }
    }
