...
this.app.post('/api/generate', async (req, res) => {
  try {
    ...
    const result = await cognitiveGenerationEngine.generate({
      ...
      prompt: prompt,
      language: locale || 'en', // ✅ Locale (en/pt-BR/es)
      framework: framework || 'react',
      context: {
        mode: mode || GenerationMode.FROM_PROMPT,
        target: target || GenerationTarget.COMPONENT,
        programmingLanguage: language || 'typescript' // ✅ Separado!
      },
      version: 1,
      isDeleted: false,
      createdAt: new Date(),
