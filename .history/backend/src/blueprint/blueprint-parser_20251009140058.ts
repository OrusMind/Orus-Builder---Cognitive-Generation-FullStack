...
export class BlueprintParser {
  ...
  private async parseContent(
    ...
    const components = options.identifyComponents ? this.identifyComponents(text, sections) : [];
  ...
  private extractMetadata(text: string): BlueprintMetadata {
    ...
    const hashMaster = hashMatch ? hashMatch[0].split(/[:\s]+/)[1] : undefined;

    // Extract Cognitive DNA
    const dnaMatch = text.match(this.ORUS_PATTERNS.cognitiveDNA);
    const cognitiveDNA = dnaMatch ? dnaMatch[0].split(/[:\s]+/).slice(1).join(' ') : undefined;
  ...
  private identifyComponents(text: string, sections: BlueprintSection[]): ComponentSpec[] {
    ...
    for (const line of lines) {
      ...
      // Check for component marker [XXX]
      ...
      if (componentMatch) {
        ...
        components.push({
          ...
          purpose: description,
          dependencies: [],
          type: this.classifyComponentType(fileName, description)
        });
      }
    }
  ...
  private classifyComponentType(fileName: string, description: string): ComponentType {
