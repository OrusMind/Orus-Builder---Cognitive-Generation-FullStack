...
// ══════════════════════════════════════════════════════════════════════
// 🧠 COGNITIVE GENERATION ENGINE - CODE GENERATION
// ══════════════════════════════════════════════════════════════════════
this.app.post('/api/generate', async (req, res) => {
  try {
    const { prompt, mode, target, language } = req.body;
    ...
    const result = await cognitiveGenerationEngine.generate({
      ...
      // Prompt (sem naturalLanguage)
      prompt: prompt,
      
      // Configuration (usar valores corretos dos enums)
      mode: (mode || 'component') as any, // component, api, fullstack
      target: (target || 'frontend') as any, // frontend, backend, fullstack
    ...
    // Response (acessar properties corretas)
    return res.json({
      success: result.success,
      engine: 'Cognitive Generation',
      generationId: result.data?.generationId,
      files: result.data?.files || [], // files ao invés de generatedFiles
      totalFiles: result.data?.files?.length || 0,
