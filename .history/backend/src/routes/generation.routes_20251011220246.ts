...
router.post('/test', async (req, res) => {
  try {
    ...
    console.log('🧪 TEST ENDPOINT - Received request:', { prompt, language, framework });
    ...
    console.log('✅ TEST ENDPOINT - Generation result:', result);
    ...
    console.error('❌ TEST ENDPOINT - Error:', error);  // ← Ver erro completo
    res.status(500).json({
      success: false,
      error: (error as Error).message,
      stack: (error as Error).stack  // ← Stack trace completo
    });
  }
});
