  ...
  try {
    ...
    logger.info('✅ [Routes] Workflow execution completed', {
      ...
      metadata: {
        ...
        stepsCompleted: workflowResult.metadata.stepsCompleted,
        confidence: workflowResult.metadata.confidence,
      }
    });
    ...
    // ─────────────────────────────────────────────────────────────
    const output = workflowResult.result as Record<string, unknown>;
    ...
    return res.status(200).json({
      success: true,
      jobId: workflowResult.executionId,
      status: 'completed',
      result: {
        files,
      ...
      orchestration: {
        stepsCompleted: workflowResult.metadata.stepsCompleted,
        stepsFailed: workflowResult.metadata.stepsFailed,
        confidence: workflowResult.metadata.confidence,
