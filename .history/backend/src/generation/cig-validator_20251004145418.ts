...
import { cigProtocol, CIGValidationResult,ValidationSuggestion } from '../core/cig/cig-protocol';
...
export class CIGValidator {
  ...
  if (issues.some(i => i.type === IssueType.TYPE_ERROR)) {
    suggestions.push({
      type: 'type_safety',
      message: {
        en: 'Add explicit type annotations to improve type safety',
        pt: 'Adicione anotações de tipo explícitas para melhorar a segurança de tipos'
      },
      file: 'current',  // ou nome do arquivo específico
      priority: 'high',
      autoFixable: true,
      fix: {
        description: {
          en: 'Add type annotations to all variables and functions',
          pt: 'Adicionar anotações de tipo a todas as variáveis e funções'
        },
  ...
  if (issues.some(i => i.type === IssueType.DEPENDENCY_ERROR)) {
    suggestions.push({
      type: 'dependencies',
      message: {
        en: 'Review and update package.json dependencies',
        pt: 'Revise e atualize as dependências do package.json'
      },
      file: 'package.json',
      priority: 'high',
      autoFixable: false
  ...
  if (metrics.complexity > 10) {
    suggestions.push({
      type: 'refactor',
      message: {
        en: 'Consider refactoring to reduce complexity',
        pt: 'Considere refatorar para reduzir a complexidade'
      },
      file: 'current',
      priority: 'medium',
      autoFixable: false
  ...
  if (metrics.maintainability < 60) {
    suggestions.push({
      ...
      message: {
        en: 'Improve code maintainability by simplifying logic',
        pt: 'Melhore a manutenibilidade simplificando a lógica'
      },
      file: 'current',
      priority: 'medium',
      autoFixable: false
  ...
  if (metrics.typeCoverage < 80) {
    suggestions.push({
      type: 'type_coverage',
      message: {
        en: 'Increase type coverage to at least 80%',
        pt: 'Aumente a cobertura de tipos para pelo menos 80%'
      },
      file: 'current',
      priority: 'low',
      autoFixable: false
