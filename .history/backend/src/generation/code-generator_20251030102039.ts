...
private normalizeFile(file: any, generatorName: string): GeneratedFile {
  ...
  // ✅ FIX #7.1: Detectar root package.json por nome + path
  if (extractedFilename === 'package.json' && extractedPath === 'src') {
    console.log(`[normalizeFile] 🎯 ROOT FILE DETECTED: ${extractedFilename}`);
    return {
      path: extractedFilename,  // ← APENAS o nome, raiz!
