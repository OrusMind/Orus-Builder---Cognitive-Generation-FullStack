 ...
 * @returns Array of entity names (max 5)
 */
private extractEntitiesFromDescription(description: string): string[] {
  ...
  // ✅ Entity keywords with index signature
 ...
 Object.keys(entityKeywords).forEach(keyword => {
  if (description.toLowerCase().includes(keyword)) {
    const entity = entityKeywords[keyword];
  ...
  for (const match of matches) {
    if (!match[1]) continue; // ✅ Guard clause
  ...
  // ✅ Fallback: Generic e-commerce entities
  if (entities.size === 0) {
