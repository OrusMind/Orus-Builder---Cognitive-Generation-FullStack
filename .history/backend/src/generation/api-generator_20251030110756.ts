...
private generateRoutesIndex(resources: APIResource[]): GeneratedAPIFile {
  // ✅ FIX #11.3: Normalize and deduplicate
  ...
  // ✅ FIX #11.4: Imports SEM caminhos completos
  const imports = normalizedResources
    .map(r => `import ${r.normalizedName.toLowerCase()}Routes from './${r.normalizedName.toLowerCase()}.routes';`)
    .join('\n');

  // Route mounting
  const routes = normalizedResources
    .map(r => `router.use('/${r.normalizedName.toLowerCase()}', ${r.normalizedName.toLowerCase()}Routes);`)
    .join('\n');
...
`;
  ...
  return {
    path: 'backend/src/routes',
    filename: 'index.ts',
    content: content.trim(),  // ✅ NÃO passar por fixImportsInContent()
    type: 'typescript'
  };
}
