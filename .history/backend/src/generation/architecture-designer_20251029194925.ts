...
path: 'backend/src/controllers',
          ...
          content: this.generateControllerCode(entityName, entity)
        }]
      });
...
path: 'backend/src/services',
          ...
          content: this.generateServiceCode(entityName, entity)
        }]
      });
      ...
      if (layers.some(l => l.name === 'Data')) {
        components.push({
          ...
          files: [{
            ...
            content: this.generateRepositoryCode(entityName, entity)
          }]
        });
      }
    });
...
export class ${entityName}Controller {
  ...
  try {
    ...
    const item = await this.${serviceName}.create(req.body);
...
export class ${entityName}Service {
   ...
   * @param filters - Filter criteria (status, priority, category, dateRange, etc)
   * @returns Filtered ${entityName.toLowerCase()}s
   */
