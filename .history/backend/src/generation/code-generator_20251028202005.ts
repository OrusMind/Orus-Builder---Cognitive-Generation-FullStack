...
/**
 * ✅ FIX #4.2.3: Normaliza paths E concatena filename
 */
private normalizeFilePaths(files: any[]): any[] {
  ...
  return files.map((file, index) => {
    ...
    // ✅ FIX: Se não tem filename, tenta extrair do path
    if (!filename || filename.trim() === '') {
      ...
      if (lastPart && lastPart.includes('.')) {
        ...
        console.log(`  [${index + 1}] 🔧 Extracted filename from path: ${filename}`);
      } else {
        console.warn(`  [${index + 1}] ⚠️ No filename found for: ${basePath}`);
      }
    }
    ...
    return {
      ...
      name: filename   // PRESERVA NAME TAMBÉM
    };
  });
}
...
private createFile(
  ...
  return {
    path,
    filename,
    name: filename,  // ← ADICIONAR ESTA LINHA!
    content,
    type,
    size: content.length,
    metadata: {
