...
private extractEntitiesFromDescription(description: string): string[] {
  ...
  const patterns = [
    /gerenciar\s+([a-záàâãéèêíïóôõöúçñ]+)/gi,
    /sistema\s+(?:de|para)\s+([a-záàâãéèêíïóôõöúçñ]+)/gi,
    /plataforma\s+(?:de|para)\s+([a-záàâãéèêíïóôõöúçñ]+)/gi,
    /([A-Z][a-z]+(?:[A-Z][a-z]+)*)/g
  ];
  ...
  patterns.forEach(pattern => {
    ...
    for (const match of matches) {
      ...
      if (entity && entity.length > 2) {
        if (entity.endsWith('s')) entity = entity.slice(0, -1);
