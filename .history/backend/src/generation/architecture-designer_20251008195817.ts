...
if (input.options?.validateWithCIG) {
  try {
    const architectureCode = JSON.stringify(architecture, null, 2);
    
    const cigEngineResult = await cigProtocol.validate({
      requestId: `arch-design-${Date.now()}`,
      code: architectureCode,  // ✅ FIXED: Propriedade obrigatória
      files: [{
    ...
    logger.warn('CIG validation failed', { 
      component: 'ArchitectureDesigner',
      action: 'cigValidation',
