...
private normalizeFile(file: any, generatorName: string): GeneratedFile {
  // ✅ FIX #4.2.4: Garantir que filename sempre existe
  const filename = file.name || file.filename || file.file || 'generated-file.ts';
  const path = file.path || file.directory || 'src';
  
  // Log para debug
  console.log(`[CodeGenerator] Normalizing file:`, {
    original: { name: file.name, filename: file.filename, path: file.path },
    normalized: { filename, path }
  });
  
  return {
    path: path,
    filename: filename,  // ← GARANTIDO!
