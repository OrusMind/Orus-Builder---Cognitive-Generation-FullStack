...
private extractEntitiesFromDescription(description: string): string[] {
  ...
  // ✅ FIX #8: Stopwords PT-BR + entity keywords
  const stopwords = ['sistema', 'de', 'com', 'para', 'e', 'o', 'a', 'os', 'as', 'um', 'uma'];
  
  // Palavras-chave E-COMMERCE (hardcoded para garantir captura)
  ...
  // Pattern 1: Entity keywords (mais importante)
  ...
  // Pattern 2: Após "com" ou "para" (fallback)
  ...
  // ✅ FIX #8: Fallback genérico (NUNCA retorna vazio ou "Sistema")
  if (entities.size === 0) {
    console.log('[APIGenerator] ⚠️ No entities detected, using e-commerce defaults');
