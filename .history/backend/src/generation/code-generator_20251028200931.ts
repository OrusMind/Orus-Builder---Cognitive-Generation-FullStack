...
private normalizeFile(file: any, generatorName: string): GeneratedFile {
  // ✅ FIX #4.2.4: Extrair filename primeiro
  const extractedFilename = file.name || file.filename || file.file || 'generated.ts';
  const extractedPath = file.path || file.directory || 'src';
  
  // Debug log
  if (!file.name && !file.filename) {
    console.warn(`[CodeGenerator] File sem name/filename!`, {
  ...
  return {
    path: extractedPath,
    filename: extractedFilename,  // ← Para GeneratedFile interface
    name: extractedFilename,       // ← Para generation.controller.ts
