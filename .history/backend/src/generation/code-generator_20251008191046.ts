...
import { componentBuilder } from './component-builder'; // @ts-expect-error - Will be used in future feature
...
export class CodeGenerator {
    ...
    try {
      ...
      if (input.requirements.database) {
        const databaseStage = await this.executeStage(
          'Database Design',
          async () => {
            // ✅ FIXED: Convert DatabaseEntity[] to DataRequirement[]
            ...
            return await databaseDesigner.design({
              ...
              // ✅ FIXED: Add type assertion for DatabaseType
              databaseType: input.requirements.database!.type as 'mongodb' | 'postgresql' | 'mysql' | 'sqlite'
            });
          }
        );
        ...
        const apiStage = await this.executeStage(
          ...
          async () => {
            return await apiGenerator.generate({
              ...
              // ✅ FIXED: Add type assertion for APIType
              apiType: (input.requirements.api_type || 'rest') as 'rest' | 'graphql',
              endpoints: [],
              options: {
                // ✅ FIXED: typescript property is now valid (interface updated)
                typescript: true,
  ...
  private convertToFiles(
    // @ts-expect-error - output and generator will be used when implementing file conversion logic
    output: any, 
    generator: string
  ): GeneratedFile[] {
