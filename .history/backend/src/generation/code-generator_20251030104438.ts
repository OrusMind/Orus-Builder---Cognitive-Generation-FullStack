...
private fixImportsInContent(content: string): string {
  ...
  // ✅ FIX #9.1: Remove path prefixes ONLY for local files
  ...
  // ✅ Normalize dots before 'routes' (ex: './catalog./routes' → './catalog.routes')
  fixed = fixed.replace(/\.\/.routes/g, '.routes');
  fixed = fixed.replace(/\.\/([a-zA-Z])/g, '.$1');

  // ❌ NÃO tocar em node_modules imports (express, react, etc)
  // ❌ NÃO fazer replace genérico sem captura de grupo
