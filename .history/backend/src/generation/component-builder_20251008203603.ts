...
`.trim();
  ...
  const propName = firstProp?.name ?? 'Component';  // Opção 1: Nullish coalescing
  // OU
  // const propName = (firstProp.name || 'Component');  // Opção 2: Logical OR
  // OU
  // const propName = firstProp.name!;  // Opção 3: Non-null assertion (se garantir que sempre existe)
