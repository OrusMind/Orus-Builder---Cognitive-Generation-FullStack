...
class BackendGenerator {
  ...
  public async generate(input: BackendGenerationInput): Promise<BackendGenerationResult> {
    ...
    try {
      ...
      // DEDUPLICATE AND NORMALIZE ENTITIES (FIX #1)
const uniqueEntities = [...new Set(
  entities.map((e: string) => this.normalizeName(e))
)] as string[];

// Controllers for each entity
uniqueEntities.forEach((entity: string) => {
...
const entityName = this.normalizeName(entity);  // ✅ FIX
name: `${entityName}.routes.ts`,  // ✅ UserRoutes.ts
...
name: `${entityName}Controller.ts`,  // ✅ UserController.ts
      content: content,
      type: 'typescript'
    };
  }
