...
// ✅ FIX #14: Validar e corrigir path do root package.json
files.forEach((file, index) => {
  // Se é root package.json em local errado
  if (file.filename === 'package.json' && file.path?.includes('src/')) {
    console.log(`[CodeGenerator] 🔧 FIX #14: Moving root package.json from "${file.path}" to "package.json"`);
    files[index] = {
      ...file,
      path: 'package.json',  // ✅ APENAS package.json (root)
      metadata: {
        ...file.metadata,
        isRoot: true
      } as any
...
private fixImportsInContent(content: string): string {
  ...
  // ONLY process backend/src e frontend/src
  // NÃO toque em npm packages!
  fixed = fixed.replace(
  ...
  // Remove .ts extension se existir
  fixed = fixed.replace(
    /from\s+['"](\.[^'"]*?)\.ts['"]/g,
    "from '$1'"
  );
...
private generatePackageJsonConfigs(
  ...
  // ✅ FRONTEND package.json (COM TAILWIND)
  const frontendPkg = {
    ...
    devDependencies: {
      ...
      "eslint": "^8.56.0",
      // ✅ FIX #16.1: Tailwind + PostCSS + Autoprefixer
      "tailwindcss": "^3.4.1",
      "postcss": "^8.4.32",
  ...
  // ✅ BACKEND package.json (SEM MUDANÇAS)
  const backendPkg = {
  ...
  // ✅ ROOT package.json (COM WORKSPACES + CONCURRENTLY)
  const rootPkg = {
    name: "project",
    version: "1.0.0",
    private: true,
