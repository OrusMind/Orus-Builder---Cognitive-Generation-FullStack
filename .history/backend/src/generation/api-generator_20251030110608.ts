...
/**
 * Extract entity names from natural language description
 * @param description Natural language prompt
 * @returns Array of NORMALIZED entity names (max 5, no accents)
 */
private extractEntitiesFromDescription(description: string): string[] {
  ...
  // Pattern 1: Entity keywords (most important)
  ...
  // Pattern 2: Words after "com" or "para" (fallback)
  ...
  // ✅ FIX #11.1: NORMALIZAR TODAS as entidades (remover acentos)
  const normalized = Array.from(entities).map(e => this.normalizeEntityName(e));
  
  // ✅ FIX #11.2: Remove duplicatas após normalização
...
/**
 * Normalize entity name: remove accents, capitalize
 * @param name Entity name (ex: 'Catálogo', 'Carrinho')
 * @returns Normalized name (ex: 'Catalog', 'Cart')
 */
...
private generateRoutesIndex(resources: APIResource[]): GeneratedAPIFile {
  // ✅ FIX #10: Normalize, deduplicate, and sanitize resource names
  const normalizedResources = resources
    .map(r => ({
      ...r,
      // Normalize: remove accents, lowercase, sanitize
      normalizedName: this.normalizeEntityName(r.name || r.id || 'item')
    }))
    // Remove duplicates by normalizedName
    .filter((r, index, self) => 
      index === self.findIndex(t => t.normalizedName === r.normalizedName)
    );

  // Generate imports
  const imports = normalizedResources
    .map(r => `import ${r.normalizedName}Routes from './${r.normalizedName}.routes';`)
    .join('\n');

  // Generate route mounting
  const routes = normalizedResources
    .map(r => `router.use('/${r.normalizedName}', ${r.normalizedName}Routes);`)
    .join('\n');
...
/**
 * Normalize entity name: remove accents, lowercase, sanitize
 * @param name Entity name (ex: 'Catálogo', 'Carrinho', 'Admin')
 * @returns Normalized name (ex: 'catalog', 'cart', 'admin')
 */
private normalizeEntityName(name: string): string {
  return name
    .toLowerCase()
