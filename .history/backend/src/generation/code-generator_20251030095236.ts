...
files.push(
  ...
  // ✅ FIX #6.1: Root package.json com flag especial
  {
    path: 'package.json',  // ← Mantém simples (sem prefixo)
    filename: 'package.json',
    name: 'package.json',
    ...
    metadata: { 
      ...
      isRootFile: true  // ✅ FLAG: Identifica arquivo raiz
    }
  }
);
...
console.log(`✅ [CodeGenerator] Config files generated: 5 files`);
  ...
  /**
   * FIX 5.3 - Fix Import Statements in Generated Code
   */
  private fixImportsInContent(content: string): string {
    // Regex para encontrar imports
    const importRegex = /import\s+(?:\{([^}]*)\}|([\w*]+))\s+from\s+['"]([^'"]+)['"]/g;
    
    return content.replace(importRegex, (match, named, default_, path) => {
      const fixedPath = this.normalizeImportPath(path);
      if (named) {
        return `import { ${named} } from '${fixedPath}'`;
      } else {
        return `import ${default_} from '${fixedPath}'`;
      }
    });
  }
  ...
  private normalizeFile(file: any, generatorName: string): GeneratedFile {
    // ✅ FIX #4.2.7: PRIORIZAR filename sobre name
    const extractedFilename = file.filename || file.name || file.file || 'generated.ts';
    const extractedPath = file.path || file.directory || 'src';
if (file.metadata?.isRootFile) {
    console.log(`[normalizeFile] 🎯 ROOT FILE DETECTED: ${extractedFilename}`);
    return {
      path: extractedFilename,  // ← Apenas o nome, sem prefixo!
    ...
    return {
      ...
      filename: extractedFilename,
      name: extractedFilename,
      content: this.fixImportsInContent(file.content || file.code || ''), // ✅ FIX #5.3
      type: this.detectFileType(extractedFilename),
      size: (file.content || file.code || '').length,
      metadata: {
