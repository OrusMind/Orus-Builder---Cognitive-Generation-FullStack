...
body {
  ...
  private generateHomePage(entities: string[]): GeneratedUIFile {
    const links = entities.map(e => 
      `          <Link to="/${e}s" className="text-blue-600 hover:underline">\n            View ${this.capitalize(e)}s\n          </Link>`
    ).join('\n');
...
export default function HomePage() {
  ...
  const fetchStats = async () => {
    try {
      const response = await api.get('/api/${mainResource}/stats');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-5 w-0 flex-1">
              <dl>
                <dt className="text-sm font-medium text-gray-500 truncate">Total ${mainResource}</dt>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <span className="text-lg font-medium text-blue-900">View All ${mainResource}</span>
          </Link>
          <Link 
            to="/${mainResource}/create" 
            className="flex items-center p-4 bg-green-50 rounded-lg hover:bg-green-100 transition"
          >
            <span className="text-lg font-medium text-green-900">Create New</span>
          </Link>
        </div>
      </div>
    </div>
