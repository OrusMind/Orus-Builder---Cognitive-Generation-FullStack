 ...
 * 🔄 LAST_MODIFIED: 2025-10-08T19:48:00-03:00
 * 🏷️ COMPONENT_HASH: orus.builder.generation.test.20251008.v2.TG050
 * 
 * ═══════════════════════════════════════════════════════════════
 ...
 * - Fixed testCasesCount in TestGenerationResult (duplicate removed from metadata)
 * - Fixed extractFunctions regex to guarantee non-null name
 * - Fixed extractClasses regex to guarantee non-null name
 * - Added null safety for cls.name and cls.extends
 * - Removed unused parameters (input, totalClasses)
 * 
 * ═══════════════════════════════════════════════════════════════
 */
...
export interface TestGenerationResult {
  ...
  testFile: string;
  testCases: TestCase[];
  testCasesCount: number;  // ✅ FIXED: Moved from metadata to root level
  mocks?: string[];
  fixtures?: string[];
...
export class TestGenerator {
  ...
  public async generate(input: TestGenerationInput): Promise<TestGenerationResult> {
    ...
    try {
      ...
      // ✅ FIXED: testCasesCount at root level, also in metadata
      const result: TestGenerationResult = {
        testCode,
        testFile: this.getTestFileName(input.sourceFile, input.framework),
        testCases,
        testCasesCount: testCases.length,  // ✅ FIXED: Added at root level
        mocks,
        fixtures,
        coverage,
        metadata: {
  ...
  private extractFunctions(code: string): FunctionInfo[] {
    ...
    while ((match = functionRegex.exec(code)) !== null) {
      // ✅ FIXED: Ensure name is not undefined
      const name = match[1];
      if (!name) continue;

      functions.push({
        name,  // ✅ Now guaranteed to be string
  ...
  private extractClasses(code: string): ClassInfo[] {
    ...
    while ((match = classRegex.exec(code)) !== null) {
      ...
      classes.push({
        name,  // ✅ Now guaranteed to be string
        extends: match[2],  // Can be undefined
  ...
  private generateTestCases(analysis: CodeAnalysis, _input: TestGenerationInput): TestCase[] {
  ...
  private generateDescribeBlock(testCases: TestCase[], input: TestGenerationInput): string {
    const testCasesCode = testCases.map(tc => this.generateTestCase(tc)).join('\n\n  ');
...
describe('${input.sourceFile}', () => {
  ...
  // ✅ FIXED: Removed unused 'input' parameter
  ...
  private generateFixtures(_analysis: CodeAnalysis): string[] {
  ...
  private estimateCoverage(testCases: TestCase[], analysis: CodeAnalysis): CoverageEstimate {
    ...
    // ✅ FIXED: Removed unused totalClasses
 ...
 * FIX VERSION: v2 - All 6 critical errors resolved
 * ═══════════════════════════════════════════════════════════════
 */
