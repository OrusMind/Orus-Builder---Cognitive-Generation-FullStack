...
try {
  ...
  if (input.requirements.database) {
    const databaseStage = await this.executeStage(
      'Database Design',
     async () => {
  // ✅ USAR a função helper que criamos
  ...
  return await databaseDesigner.design({
    entities: dataRequirements,  // ✅ ADICIONAR
  ...
  if (input.projectType !== ProjectType.FRONTEND_ONLY) {
    const apiStage = await this.executeStage(
      'API Generation',
      async () => {
        // ✅ FIXED: Remove 'options' completely - not in APIGenerationInput interface
       return await apiGenerator.generate({
  entities: input.requirements.features,  // ✅ ADICIONAR (usar features como entities)
