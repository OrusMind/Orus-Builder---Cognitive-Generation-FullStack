...
private async generateWithTemplates(
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 2: Map complexity to valid TemplateCategory
  // ═══════════════════════════════════════════════════════════════
  let templateCategory: 'simple' | 'standard' | 'advanced' = 'standard';
  
  if (complexity === 'minimal' || complexity === 'simple') {
  ...
  if (templateSearch.templates && templateSearch.templates.length > 0) {
    const template = templateSearch.templates[0];
    
    if (template) {
      console.log('  ✅ Found matching template:', template.name);
      
      // Map complexity to template context complexity
      let contextComplexity: 'simple' | 'standard' | 'advanced' = 'standard';
      if (complexity === 'minimal' || complexity === 'simple') {
        contextComplexity = 'simple';
      } else if (complexity === 'complex') {
        contextComplexity = 'advanced';
      }
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 4: No template found → fallback to empty (AI will handle)
  // ═══════════════════════════════════════════════════════════════
  console.log('  ℹ️ No template found, fallback to AI generation');
  return [];
}

/**
 * Map complexity to template category
 */
private mapComplexityToCategory(complexity: 'minimal' | 'simple' | 'moderate' | 'complex'): string {
  const categoryMap = {
    minimal: 'basic',
    simple: 'standard',
    moderate: 'advanced',
  ...
  return categoryMap[complexity] || 'standard';
}
...
private async renderFromTemplate(
  template: any,
  componentSpec: { name: string; type: string },
  designTokens: any,
  context: {
    domain: string;
    personality: string;
...
/**
 * ═══════════════════════════════════════════════════════════════════════
 * GENERATE COMPONENT (v4.0 - WITH TEMPLATE SYSTEM INTEGRATION)
 * ═══════════════════════════════════════════════════════════════════════
 */
private async generateComponent(
  ...
  if (hasPrompt) {
    try {
      ...
      const personality = this.detectPersonality(prompt);
      ...
      if (templateSearch.templates && templateSearch.templates.length > 0) {
        console.log('  ✅ Found template:', templateSearch.templates[0]?.name);
        
        const generatedComponents = await this.renderFromTemplate(
          templateSearch.templates[0],
          componentSpec,
        ...
        if (generatedComponents.length > 0) {
          ...
          return generatedComponents[0]!;
        }
      }
      
      console.log('  ℹ️ No template found, using AI generation');
    } catch (error) {
      console.warn('  ⚠️ Template generation failed, falling back to AI:', (error as Error).message);
    }
  }
  
  // ═══════════════════════════════════════════════════════════════
  // FALLBACK: AI-BASED GENERATION (ORIGINAL METHOD)
  // ═══════════════════════════════════════════════════════════════
  const aiPrompt = this.buildComponentPrompt(componentSpec, specification, request, context);
  ...
  try {
    codeResponse = await this.getProvider().chat([
      {
        role: 'system',
        content: 'You are an expert React/TypeScript code generator. Generate clean, production-ready code with proper file structure.'
      },
      {
        role: 'user',
        content: `${aiPrompt}
...
RULES:
- Start each file with: // src/filename.ext
...
- Just code with file markers`
  ...
  console.log('[COGNITIVE ENGINE] Response:', codeResponse);
  console.log('[COGNITIVE ENGINE] Content:', codeResponse?.content);
  console.log('===============================================');
  ...
  console.log(`[DEBUG] Has "// src/" markers: ${code.includes('// src/')}\n`);
  ...
  if (parsedFiles.length > 1) {
    ...
    console.log(`[INFO] Returning ${parsedFiles.length} files to parent`);
    
    const mainFile = parsedFiles[0]!;
    
    // CRITICAL FIX: Save extra files in "additionalFiles" (not "allFiles")
    (mainFile.metadata as any).additionalFiles = parsedFiles.slice(1);
    (mainFile.metadata as any).isMultiFile = true;
    
    logger.info(`Stored ${parsedFiles.length - 1} extra files in metadata.additionalFiles`, {
      component: 'CognitiveGenerationEngine',
      metadata: { 
        extraFilesCount: parsedFiles.length - 1,
        files: parsedFiles.slice(1).map(f => f.path).join(', ')
      }
    });
  ...
  if (parsedFiles.length === 1) {
    ...
    console.log('[INFO] Single file detected:', firstFile.name);
  ...
  console.warn('[WARN] No file markers detected! AI returned code without "// filename.tsx" format');
