...
private mapComplexityToScope(
  ...
  const scopeMap: Record<string, ScopeResult> = {
    'minimal': {
      type: 'single_component',
      complexity: 'simple',
      confidence: 1.0, // 100% - escolha manual do usuário
      expectedFileCount: { min: 1, max: 3 },
      shouldIncludeFrontend: true,
      shouldIncludeBackend: false,
      shouldIncludeDatabase: false,
  ...
  const result = scopeMap[complexity] || scopeMap['standard'];
  
  console.log(`✅ [Scope] Mapped complexity "${complexity}" to scope:`, JSON.stringify(result, null, 2));
