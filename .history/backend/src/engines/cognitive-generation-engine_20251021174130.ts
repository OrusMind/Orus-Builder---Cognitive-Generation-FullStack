...
finalCode = this.fixTypeScriptForSandpack(finalCode);
        return {
          ...comp,
          code: finalCode,
          validation: {
...
/**
 * Fix TypeScript generics for Sandpack compatibility
 * Sandpack bundler can handle TypeScript, but we clean it to avoid edge cases
 */
private fixTypeScriptForSandpack(code: string): string {
  let fixed = code;
  
  logger.info('🔧 Applying Sandpack compatibility fixes...', { 
    component: 'CognitiveGenerationEngine' 
  } as any);
  ...
  if (catchBlocks) {
    ...
    // Fix 7: REMOVER MARKDOWN FENCES - usando String ao invés de regex literal
  const fencePattern = '```';
  if (fixed.includes(fencePattern)) {
    const fencesCount = (fixed.match(new RegExp(fencePattern, 'g')) || []).length;
    logger.info(`  🧹 Removing ${fencesCount} markdown fences from code`, {
      component: 'CognitiveGenerationEngine'
    } as any);
    
    // Remove fences with language (``````css, etc)
    fixed = fixed.replace(new RegExp(fencePattern + '[a-zA-Z]*\\n?', 'g'), '');
    // Remove plain fences
    fixed = fixed.replace(new RegExp(fencePattern, 'g'), '');
    
    logger.info('  ✅ Markdown fences removed successfully', {
      component: 'CognitiveGenerationEngine'
    } as any);
  }
  ...
  // Fix 8: Remove markdown headers (####, ###, ##) that might be in comments
  const headerMatches = fixed.match(/^#{1,6}\s+/gm);
  if (headerMatches) {
    logger.info(`  🧹 Removing ${headerMatches.length} markdown headers`, {
      component: 'CognitiveGenerationEngine'
    } as any);
    fixed = fixed.replace(/^#{1,6}\s+/gm, '// ');
  }

  logger.info('✅ Sandpack compatibility fixes applied', {
    component: 'CognitiveGenerationEngine'
  } as any);
...
logger.info(`📦 Total components to generate: ${specification.components.length}`, {
 ...
 private async generateComponent(
  ...
  // ✅ ADICIONAR `const codeResponse =` AQUI:
  ...
  const code = codeResponse.content; // ✅ AGORA FUNCIONA
  ...
  if (parsedFiles.length > 1) {
    // ✅ MULTI-FILE DETECTADO → MERGE!
    logger.warn(`⚠️ Multi-file detected (${parsedFiles.length} files), merging...`, {
      component: 'CognitiveGenerationEngine',
      metadata: { files: parsedFiles.map(f => f.name) }
    });
    
    // ✅ MERGE AUTOMÁTICO
    const mergedComponent = this.mergeToSingleFile(parsedFiles);
    
    logger.info(`✅ Merged ${parsedFiles.length} files into ${mergedComponent.name}`, {
      component: 'CognitiveGenerationEngine'
    });
  ...
  // ✅ Fallback: código não parseado
  logger.warn('⚠️ Could not parse files, using original code', {
    component: 'CognitiveGenerationEngine'
  });
...
const filename: string = match[1]!; // ✅ Adicionar !
const content: string = match[2]!.trim(); // ✅ Adicionar !
...
/**
 * Extrai interfaces do código (versão ULTRA-MELHORADA v2)
...
private buildFrontendPrompt(
  ...
  // ✅ Extract context (colors, personality, domain, language)
 ...
 const prompt = `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨🚨🚨 CRITICAL SYSTEM CONSTRAINT - READ THIS FIRST! 🚨🚨🚨
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**THIS IS A SANDBOX ENVIRONMENT WITH ZERO EXTERNAL DEPENDENCIES!**

YOU **MUST** FOLLOW THESE RULES OR THE CODE WILL NOT RUN:

1. ✅ **ONE FILE ONLY** - Do NOT create separate files (Post.ts, Comment.ts, etc)
2. ✅ **MOCK DATA ONLY** - Do NOT use fetch('/api/...') or real API calls
3. ✅ **NO EXTERNAL LIBRARIES** - Do NOT import recharts, axios, lodash, etc
4. ✅ **INLINE EVERYTHING** - Put interfaces, helpers, mock data in ONE file

**IF YOU VIOLATE ANY RULE, THE PREVIEW WILL FAIL! THE USER WILL SEE ERRORS!**

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
❌ WRONG EXAMPLE (WILL CAUSE 404 ERRORS):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

// ❌ Multiple files (WRONG!)
// Post.ts
interface Post { ... }
...
const fetchPosts = async () => {
  const response = await fetch('/api/posts'); // ❌ WILL FAIL WITH 404!
...
// ✅ ONE file with EVERYTHING inline
...
// Main component
function Blog() {
  const [posts, setPosts] = useState(mockPosts); // ✅ Use mock data directly!
  ...
  useEffect(() => {
    setLoading(true);
    setTimeout(() => {
      setPosts(mockPosts); // Set mock data
      setLoading(false);
    }, 500);
  }, []);
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 REMINDER: DO NOT USE fetch('/api/...') - IT WILL CAUSE 404 ERRORS!
🚨 REMINDER: DO NOT CREATE MULTIPLE FILES - MERGE EVERYTHING INTO ONE FILE!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

YOU ARE GENERATING CODE FOR A BROWSER SANDBOX THAT **CANNOT** ACCESS EXTERNAL LIBRARIES!



━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

You are generating a ${framework} TypeScript component named "${component.name}".
...
**Language:** ${language.toUpperCase()} (all user-facing text)
**Personality:** ${personality.toUpperCase()}
**Colors:** Primary ${colorPalette[0]}, Secondary ${colorPalette[1] || colorPalette[0]}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 SANDBOX RESTRICTIONS (MANDATORY - WILL CAUSE RUNTIME ERRORS!)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**❌ EXAMPLE OF WHAT NOT TO DO (WILL CAUSE "ReferenceError"):**

\`\`\`typescript
// ❌ WRONG - This will cause "ReferenceError: LineChart is not defined"
import { LineChart, Line, XAxis, YAxis } from 'recharts';

const Dashboard = () => {
  return <LineChart data={data} />; // ❌ WILL FAIL!
};
\`\`\`

**✅ CORRECT APPROACH - CSS-based chart using divs:**

\`\`\`typescript
// ✅ CORRECT - CSS-based bar chart (works in sandbox!)
const Dashboard = () => {
  const data = [
    { month: 'Jan', value: 65 },
    { month: 'Feb', value: 85 },
  ...
  return (
    <div className="flex items-end h-64 gap-2 p-4 bg-white rounded-lg shadow-lg">
      {data.map((item, idx) => (
...
**IF USER ASKS FOR CHARTS/GRAPHS:**
Create CSS-based visualizations using divs with dynamic heights, widths, or transforms!

**IF USER ASKS FOR DATA FETCHING:**
Use mock data defined in the component (NO axios, NO real API calls)!

\`\`\`typescript
// ✅ CORRECT: Mock data with simulated loading
const mockData = [
  { id: 1, name: 'Item 1' },
  { id: 2, name: 'Item 2' }
];
...
// ❌ WRONG: Real API call (will fail!)
// fetch('/api/data').then(...)
\`\`\`

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 CRITICAL RULE #1: SINGLE-FILE GENERATION (MANDATORY!)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**YOU MUST GENERATE EXACTLY ONE FILE - NO EXCEPTIONS!**

❌ FORBIDDEN - These will cause PREVIEW FAILURE:
  • Separate interface files (models/User.ts, types.ts, interfaces.ts)
  • Separate component files (components/Card.tsx, utils/helper.ts)
  • Index files (index.tsx, index.ts, App.tsx)
  • CSS imports (import './styles.css', import './Component.css')
  • Relative imports (import { X } from './file', import X from '../folder')

✅ REQUIRED - Single-file structure:
  • ALL interfaces defined at top (inline, no exports)
  • ALL helper functions after interfaces (inline, no exports)
  • ALL mock data after helpers (inline)
  • ONE main component
  • ONE export default at the end

**IF YOU GENERATE MORE THAN ONE FILE, YOU FAILED!**
...
\`\`\`typescript
// STEP 1: React imports ONLY (no local imports!)
import React, { useState, useEffect } from 'react';

// STEP 2: TypeScript interfaces (NO "export" keyword!)
interface User {
  id: number;
  name: string;
  email: string;
...
// STEP 3: Helper functions (NO "export" keyword!)
...
const mockUsers: User[] = [
  { id: 1, name: '${language === 'pt-BR' ? 'João Silva' : 'John Smith'}', email: 'user1@example.com' },
  { id: 2, name: '${language === 'pt-BR' ? 'Maria Santos' : 'Mary Johnson'}', email: 'user2@example.com' }
];

// STEP 5: Sub-components (if needed, define as functions)
...
function ${component.name}() {
  // State (NO TypeScript generics in hooks!)
...
// STEP 7: ONLY export (ONE per file!)
export default ${component.name};
\`\`\`

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 CRITICAL RULE #2: TYPESCRIPT BABEL COMPATIBILITY
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**NO GENERICS IN REACT HOOKS** (Babel limitation!)

❌ WRONG (breaks preview):
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎨 DESIGN RULES (TAILWIND CSS MANDATORY)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Layout:**
✅ Use \`min-h-screen\` (NOT h-screen) for full-page layouts
✅ Use \`w-full\` on main containers
✅ Use Flexbox or Grid (NEVER absolute positioning)
✅ Semantic HTML: <nav>, <main>, <article>, <section>, <header>

**Colors (use these EXACT values):**
• Primary: ${colorPalette[0]} → Tailwind: bg-blue-600, text-blue-600
• Secondary: ${colorPalette[1] || colorPalette[0]} → Tailwind: bg-gray-600, text-gray-600
...
**NO CSS imports:** Use ONLY Tailwind classes (inline styling)
...
${language === 'pt-BR' ? `
**Brazilian names:** João Silva, Maria Santos, Pedro Costa, Ana Oliveira
...
` : `
**English names:** John Smith, Mary Johnson, Peter Williams, Anna Davis
**US cities:** New York, Los Angeles, Chicago, Houston
**US phones:** +1 (555) 123-4567, +1 (555) 987-6543
**Date format:** MM/DD/YYYY
**Currency:** $1,234.56 (use Intl.NumberFormat('en-US', {style:'currency', currency:'USD'}))
`}

**Data requirements:**
• Emails: Match names (joao.silva@example.com)
• Dates: Last 7-30 days (new Date with relative offsets)
• Quantity: 10-15 items minimum (show scrolling)
• Status mix: 30% pending, 50% completed, 20% failed

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚡ INTERACTIVITY (MANDATORY FOR RICH UIs)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Required interactive features:**
✅ Toggle states (sidebar, modals, dropdowns)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 COMPONENT RICHNESS (FOR DASHBOARDS/PAGES)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**If generating a Dashboard/Page, MUST include:**

1. **Layout:** Sidebar + Header + Main content + Footer (optional)
2. **Sidebar:** Navigation with 5+ links
3. **Header:** User menu, notifications icon
4. **Stats:** At least 3 StatCards with KPIs
5. **Data:** At least 1 Table with sortable columns
6. **Charts:** CSS-based simple bar charts (NO Recharts!)
7. **Interactivity:** Sidebar toggle, search, sort, filter

**Sub-components (define as functions in same file):**
...
function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  return <aside className="...">{/* navigation */}</aside>;
}

// Header sub-component
function Header({ onMenuClick }: { onMenuClick: () => void }) {
  return <header className="...">{/* top bar */}</header>;
}
...
✅ Generate COMPLETE, PRODUCTION-READY dashboards with rich UIs!
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${domain === 'landing-page' ? `
🎯 LANDING PAGE SPECIFIC RULES (MANDATORY!)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Required sections (minimum 6):**
1. Hero: Gradient bg, bold title, subtitle, CTA button
2. Features: 3-6 cards with icons/emojis
3. How It Works: 3-4 step-by-step
4. Testimonials: 3-6 reviews with avatars
5. Pricing/CTA: Strong call-to-action
6. Footer: Links, social, copyright

**Visual effects:**
• Gradients: bg-gradient-to-br from-blue-600 via-purple-600 to-pink-600
...
• Body: text-lg md:text-xl text-gray-600

**Icons:** Use emojis (🚀 ⚡ 💎 🎯 ✨ 🌟) or colored circles

**CRITICAL:** Landing pages MUST have ALL 6 sections with gradients, animations, and hover effects!
` : ''}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1. ❓ Am I importing Recharts, Chart.js, or any chart library? → If YES, STOP! Use CSS divs instead!
2. ❓ Am I importing axios, Material-UI, lodash, or any external library? → If YES, STOP! Remove it!
3. ❓ Am I using fetch to a real API? → If YES, STOP! Use mock data with setTimeout instead!
4. ❓ Am I creating multiple files? → If YES, STOP! Merge everything into ONE file!
5. ❓ Am I using TypeScript generics in useState/useEffect? → If YES, STOP! Let TypeScript infer types!

**IF YOU ANSWERED YES TO ANY QUESTION, YOUR CODE WILL FAIL! FIX IT BEFORE GENERATING!**
...
✅ ONE file only (no separate files!)
✅ NO local imports (only 'react')
✅ NO external libraries (NO Recharts, NO axios, NO Material-UI!)
✅ ONE export default at end
✅ ALL interfaces inline at top
✅ ALL helpers inline after interfaces
...
🚨 CRITICAL: Return ONLY the TypeScript code. NO explanations, NO markdown blocks, NO file names, NO comments before/after. NO Recharts! NO axios! NO external libraries! Just the raw code starting with "import React" and ending with "export default ${component.name};".
