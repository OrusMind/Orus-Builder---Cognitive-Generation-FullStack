 ...
 * ✅ FIXED: Detecta tanto "// filename.ext" quanto "// FILE: path/filename.ext"
 */
private parseMultiFileCode(code: string, componentSpec: any): GeneratedComponent[] {
  const components: GeneratedComponent[] = [];
  
  // ✅ NOVO REGEX: Detecta múltiplos formatos
  // Formato 1: // filename.ext
  // Formato 2: // FILE: filename.ext
  // Formato 3: // FILE: path/to/filename.ext
  const filePattern = /\/\/\s*(?:FILE:\s*)?(\S+\.(?:ts|tsx|js|jsx|json))\s*\n([\s\S]*?)(?=\/\/\s*(?:FILE:\s*)?\S+\.(?:ts|tsx|js|jsx|json)|$)/gi;
  ...
  matches.forEach((match, index) => {
    if (!match[1] || !match[2]) {
      logger.warn('⚠️ Invalid file match detected, skipping', { index });
      return;
    }

    // ✅ Extrair apenas o filename (remover path se existir)
    const fullPath: string = match[1]!;
    const filename: string = fullPath.split('/').pop() || fullPath;
    const content: string = match[2]!.trim();
    
    // Validar que não está vazio
    if (content.length < 10) {
      logger.warn('⚠️ File content too short, skipping', { filename });
      return;
    }
    
    // Determinar tipo baseado no nome/conteúdo
    let type: 'page' | 'component' | 'service' | 'model' | 'util' | 'config' = 'component';
    let path = '';
    
    // Determinar path baseado no nome do arquivo
    if (filename.includes('App.tsx') || filename.includes('index.tsx')) {
      type = 'page';
      path = `src/${filename}`;
    } else if (filename.includes('types.ts') || filename.includes('interface')) {
      type = 'model';
      path = `src/types/${filename}`;
