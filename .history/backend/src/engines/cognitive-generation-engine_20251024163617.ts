...
import { OptimizationType } from '../generation/code-optimizer'; // ✅ Import normal (não type)
...
const qualityScore = components.reduce((sum: number, c: GeneratedComponent) =>        sum + (c.metadata.qualityScore || 0), 0
  ...
  optimizations: [OptimizationType.PERFORMANCE, OptimizationType.BESTPRACTICES]
};
...
timestamp: new Date(),
    error: {
  code: TrinityErrorCode.COMPONENTUNAVAILABLE,
...
export async function validateGeneration(
  ...
  suggestions: validation.report?.recommendations?.map((r: any) => r.message?.en || 'Suggestion') || []
};
