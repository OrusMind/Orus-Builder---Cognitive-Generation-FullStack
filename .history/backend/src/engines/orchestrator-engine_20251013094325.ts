...
if (promptEngine) {
  try {
    ...
    stylePreferences: promptResult.specification?.stylePreferences || options.style || 'modern',
  ...
  logger.warn('⚠️ [Step 1/7] Prompt analysis failed, using defaults', {
  component: 'OrchestrationEngine',
...
const generationResult = await (cognitiveEngine as any).generate({
 ...
 /**
  ...
  /**
 * ═════════════════════════════════════════════════════════════════════════
 * 🎨 DYNAMIC COLOR PALETTE GENERATION (AI-POWERED!)
 * ═════════════════════════════════════════════════════════════════════════
 */
private async getColorPaletteForContext(
  type: string, 
  prompt: string, 
  entities: any[]
): Promise<string[]> {
  ...
  // ─────────────────────────────────────────────────────────────
  // STEP 1: Análise semântica do prompt
  // ─────────────────────────────────────────────────────────────
  const promptLower = prompt.toLowerCase();
  
  // ─────────────────────────────────────────────────────────────
  // STEP 2: Detectar mood/vibe do app
  // ─────────────────────────────────────────────────────────────
  let mood: 'energetic' | 'calm' | 'professional' | 'playful' | 'dark' | 'vibrant' = 'professional';
  
  if (/energetic|intense|powerful|strong/i.test(promptLower)) {
    mood = 'energetic';
  } else if (/calm|relaxing|peaceful|zen|meditation/i.test(promptLower)) {
    mood = 'calm';
  } else if (/fun|playful|colorful|bright|happy/i.test(promptLower)) {
    mood = 'playful';
  } else if (/dark|night|black|minimal|sleek/i.test(promptLower)) {
    mood = 'dark';
  } else if (/vibrant|colorful|bold|striking/i.test(promptLower)) {
    mood = 'vibrant';
  }

  // ─────────────────────────────────────────────────────────────
  // STEP 3: Detectar cores específicas mencionadas no prompt
  // ─────────────────────────────────────────────────────────────
  const colorMentions = this.extractColorMentions(promptLower);
  if (colorMentions.length > 0) {
    return colorMentions;  // ← USA CORES DO PRÓPRIO PROMPT!
  }

  // ─────────────────────────────────────────────────────────────
  // STEP 4: Gerar paleta baseada em TYPE + MOOD
  // ─────────────────────────────────────────────────────────────
  const palettes: Record<string, Record<string, string[]>> = {
    fitness: {
      energetic: ['#FF3B30', '#FF9500', '#000000'],  // Vermelho + Laranja + Preto
      calm: ['#32ADE6', '#34C759', '#FFFFFF'],       // Azul + Verde suave
      dark: ['#1C1C1E', '#00D084', '#0A84FF'],       // Dark + Verde neon
      vibrant: ['#FF2D55', '#FFD60A', '#5E5CE6'],    // Rosa + Amarelo + Roxo
      playful: ['#FF9500', '#30D158', '#5E5CE6'],    // Laranja + Verde + Roxo
      professional: ['#00D084', '#0A84FF', '#34C759'], // Verde + Azul (padrão)
    },
    
    ecommerce: {
      energetic: ['#FF3B30', '#FFD60A', '#000000'],
      calm: ['#007AFF', '#FFFFFF', '#F2F2F7'],
      dark: ['#1C1C1E', '#007AFF', '#FFD60A'],
      vibrant: ['#FF2D55', '#007AFF', '#30D158'],
      playful: ['#FF9500', '#5E5CE6', '#FF2D55'],
      professional: ['#007AFF', '#34C759', '#8E8E93'],
    },
    
    // ... mais tipos ...
  };

  return palettes[type]?.[mood] || palettes[type]?.['professional'] || ['#007AFF', '#5856D6', '#34C759'];
}

/**
 * Extrair cores mencionadas explicitamente no prompt
 */
private extractColorMentions(promptLower: string): string[] {
