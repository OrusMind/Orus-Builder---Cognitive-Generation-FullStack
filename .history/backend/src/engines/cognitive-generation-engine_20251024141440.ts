      ...
      for (const component of components) {
        ...
        if (this.config.enableQualityAnalysis) {
          const qualityInput: QualityAnalysisInput = {
            ...
            depth: 'comprehensive' as AnalysisDepth,
            context: {
              framework: component.framework,
              projectType: 'web-app'
            }
        ...
        const optimizationInput: OptimizationInput = {
          ...
          level: 'standard' as OptimizationType,
        ...
        component.metadata.optimizations = optimized.optimizations.map(o => o.type);
...
export class ContextBuilder {
  ...
  private async invokeTrinity(request: GenerationRequest): Promise<TrinityResponse<any>> {
    try {
      ...
      const trinityRequest: TrinityRequest<any> = {
      ...
      const result = await trinity.executeRequest(trinityRequest);
      return result;
...
// Named exports (classes only, interfaces already exported)
export {
