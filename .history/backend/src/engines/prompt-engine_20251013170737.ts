...
export class PromptEngine {
  ...
  async analyze(request: PromptRequest): Promise<EngineResult<PromptAnalysisResult>> {
    ...
    try {
      ...
      const detectedDomain = contextAnalysis.domain.domain;
      
      logger.info('✅ Domain detected', {
        component: 'PromptEngine',
        metadata: { 
          domain: detectedDomain,
          confidence: contextAnalysis.domain.confidence 
        }
      });
      ...
      const context = {
        ...
        domainPatterns: contextAnalysis.domain.suggestedTechnologies || []
      };
...
Return as JSON matching the schema.`;
  ...
  private getColorPaletteForDomain(domain: Domain | string): string[] {
    ...
    return palettes[domainKey] || palettes['GENERAL'];
  }
  ...
  private getPersonalityForDomain(domain: Domain | string): string {
    ...
    return personalities[domainKey] || personalities['GENERAL'];
  }
