...
export class CIGProtocolEngine {
 ...
 private collectErrors(
  syntax: { errors: TSCompilerSyntaxError[] },
  types: { errors: TSCompilerTypeError[] },
  dependencies: { circularDependencies?: Array<{ cycle: string[] }> },
  contracts: { violations?: Array<{ type: string; message: string }> }
): CompilationError[] {
  const errors: CompilationError[] = [];
  
  // Collect syntax errors
  syntax.errors.forEach((err: TSCompilerSyntaxError) => {
    errors.push({
      code: 'SYNTAX_ERROR',
      message: err.message,
  ...
  // Collect type errors
  types.errors.forEach((err: TSCompilerTypeError) => {
  ...
  if (dependencies.circularDependencies && dependencies.circularDependencies.length > 0) {
    dependencies.circularDependencies.forEach((dep) => {
      errors.push({
        code: 'CIRCULAR_DEPENDENCY',
        message: `Circular dependency detected: ${dep.cycle.join(' -> ')}`,
        file: 'unknown',
        line: 0,
        column: 0,
        severity: 'error',
  ...
  if (contracts.violations && contracts.violations.length > 0) {
...
private collectWarnings(
  types: { warnings?: Array<{ message: string; file?: string }> },
  dependencies: { unusedDependencies?: string[] },
  performance: { optimizations?: Array<{ type: string; description: string }> }
): Array<{ code: string; message: string; file: string; severity: 'warning' }> {
  const warnings: Array<{ code: string; message: string; file: string; severity: 'warning' }> = [];
  
  // Type warnings
  if (types.warnings) {
    types.warnings.forEach((warn) => {
      warnings.push({
        code: 'TYPE_WARNING',
        message: warn.message,
        file: warn.file || 'unknown',
        severity: 'warning'
      });
    });
  }
  ...
  // Dependency warnings
  if (dependencies.unusedDependencies) {
