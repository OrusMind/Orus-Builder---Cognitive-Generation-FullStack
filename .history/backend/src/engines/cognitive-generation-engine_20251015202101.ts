...
/**
 * ✅ BUILD FRONTEND GENERATION PROMPT - COMPLETE VERSION
 * Generates context-aware prompt for AI code generation
 */
private buildFrontendPrompt(
  ...
  const prompt = `Generate a ${framework} TypeScript ${component.type} component named ${component.name}.
...
**RULE #1: ONE FILE ONLY - NO EXCEPTIONS**
✅ Generate EXACTLY ONE component in ONE single file
❌ DO NOT create separate files (NO "// models/User.ts", NO "// api/auth.ts")
❌ DO NOT suggest file structure or folders
❌ DO NOT create utility files, helper files, or type files
✅ ALL interfaces, functions, helpers, and component code MUST be in THE SAME FILE

**RULE #2: IMPORTS - ONLY REACT & EXTERNAL LIBRARIES**
✅ ALLOWED: import React, { useState, useEffect } from 'react';
❌ FORBIDDEN: import { User } from './models/User';
❌ FORBIDDEN: import { api } from './services/api';
❌ FORBIDDEN: import Component from './components/Component';
❌ FORBIDDEN: import { helper } from './utils/helper';
✅ Only import from: 'react', 'react-dom', external npm packages
✅ NEVER import from relative paths ('./', '../')

**RULE #3: EXPORTS - ONLY DEFAULT EXPORT AT END**
✅ ONLY ONE export at the very end: export default ${component.name};
❌ DO NOT export functions: export function helper() { ... }
❌ DO NOT export interfaces: export interface User { ... }
❌ DO NOT use named exports: export { Component, helper };
✅ Keep all helpers, interfaces internal to the file

**RULE #4: SELF-CONTAINED & COMPLETE**
✅ Define ALL TypeScript interfaces at the top of the file
✅ Define ALL helper functions after interfaces
✅ Define ALL mock data after helper functions
✅ Define main component after all definitions
✅ Use ONLY inline mock data (NO API calls to external services)
✅ Component must work standalone without any external dependencies
**RULE #5: RICH COMPONENT WITH SUB-COMPONENTS**
✅ If building a Dashboard/Page, include ALL necessary sub-components:
...
✅ ALL sub-components must be defined in THE SAME file
✅ Sub-components are just functions, NOT separate files
...
// Main Component (uses all sub-components)
      ...
      <div className="flex-1">
        ...
        <main className="p-6">
          <div className="grid grid-cols-3 gap-6">
            <StatCard title="Vendas" value="R$ 12.345" icon="💰" />
            <StatCard title="Clientes" value="1.234" icon="👥" />
            <StatCard title="Pedidos" value="456" icon="📦" />
          </div>
        </main>
      </div>
    </div>
...
❌ WRONG: Generate only a simple component with 3 cards
✅ CORRECT: Generate a COMPLETE dashboard with sidebar, header, nav, cards, tables, charts
...
\`\`\`typescript
// ✅ STEP 1: IMPORTS (React + external libraries only)
import React, { useState, useEffect } from 'react';

// ✅ STEP 2: TYPE DEFINITIONS (NO export!)
interface User {
  id: number;
  username: string;
  email: string;
...
// ✅ STEP 3: MOCK DATA (realistic, language-appropriate)
const mockUsers: User[] = [
  { id: 1, username: 'João Silva', email: 'joao.silva@example.com' },
  { id: 2, username: 'Maria Santos', email: 'maria.santos@example.com' },
  { id: 3, username: 'Pedro Costa', email: 'pedro.costa@example.com' }
];

// ✅ STEP 4: HELPER FUNCTIONS (NO export!)
...
function formatDate(date: Date): string {
  return new Intl.DateTimeFormat('${language === 'pt-BR' ? 'pt-BR' : 'en-US'}').format(date);
}
...
// ✅ STEP 6: DEFAULT EXPORT (ONLY export in entire file)
export default ${component.name};
\`\`\`

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎨 DESIGN & STYLING RULES (MANDATORY)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**LAYOUT ARCHITECTURE:**
✅ ALWAYS use \`min-h-screen\` for full-page layouts (NEVER \`h-screen\`)
✅ ALWAYS use \`w-full\` on main containers
✅ ALWAYS use Flexbox or CSS Grid (NEVER absolute positioning)
✅ ALWAYS add \`overflow-auto\` to scrollable content areas
✅ Use semantic HTML: <nav>, <main>, <article>, <section>, <header>, <footer>

**COLOR PALETTE (USE THESE EXACT COLORS):**
🎨 Primary Color: ${colorPalette[0]} 
   - Use for: buttons, headers, important UI elements
   - Tailwind: bg-blue-600, text-blue-600, border-blue-600
   
🎨 Secondary Color: ${colorPalette[1] || colorPalette[0]}
   - Use for: accents, secondary actions, subtle elements
   - Tailwind: bg-gray-600, text-gray-600, border-gray-600

${colorPalette[2] ? `🎨 Accent Color: ${colorPalette[2]}
   - Use for: highlights, success states, special elements
   - Tailwind: bg-green-600, text-green-600, border-green-600` : ''}
...
✅ Border styles: border border-2 border-gray-200

**COMPONENT PATTERNS:**

📊 Dashboard Layout:
- Sticky header with \`sticky top-0 z-50\`
- Sidebar with \`min-h-screen\` and grid layout
- Main content with \`overflow-auto\`
- Cards with \`bg-white rounded-xl shadow-lg p-6\`
...
- Sortable headers with cursor-pointer
- Zebra striping with odd:bg-gray-50
- Status badges with colored backgrounds

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 MOCK DATA QUALITY (REALISTIC & CONTEXTUAL)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**LANGUAGE-APPROPRIATE NAMES:**
${language === 'pt-BR' ? `
✅ Brazilian names: "João Silva", "Maria Santos", "Pedro Costa", "Ana Oliveira", "Carlos Ferreira"
✅ Brazilian cities: "São Paulo", "Rio de Janeiro", "Brasília", "Belo Horizonte"
✅ Brazilian phones: "+55 11 98765-4321", "+55 21 99876-5432"
✅ Brazilian CPF: "123.456.789-00" (format)
` : ''}
${language === 'en' ? `
✅ English names: "John Smith", "Mary Johnson", "Peter Williams", "Anna Davis", "Michael Brown"
✅ US cities: "New York", "Los Angeles", "Chicago", "Houston"
✅ US phones: "+1 (555) 123-4567", "+1 (555) 987-6543"
✅ US SSN: "123-45-6789" (format)
` : ''}

**REALISTIC DATA:**
✅ Emails: Match names (joao.silva@example.com, maria.santos@example.com)
✅ Dates: Last 7-30 days (use new Date() with relative offsets)
✅ Amounts: Realistic values with proper formatting
   - ${language === 'pt-BR' ? 'Brazilian: R$ 1.234,56 (use Intl.NumberFormat)' : 'US: $1,234.56'}
✅ Status distribution: 30% pending, 50% approved, 20% rejected
✅ Quantity: 5-15 items minimum (enough to show scrolling/pagination)
...
✅ Toggle states: sidebar open/closed, modals, dropdowns
✅ Hover effects: All clickable elements must have hover states
✅ Focus states: focus:ring-2 focus:ring-blue-500
✅ Loading states: Show spinners or skeletons during actions
✅ Form validation: Real-time validation with error messages
✅ Search/filter: If showing lists, add search functionality
✅ Sort: If showing tables, make headers sortable
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚫 TYPESCRIPT BABEL COMPATIBILITY (CRITICAL!)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**RULE: NO GENERICS IN REACT HOOKS**

❌ WRONG (will break in Babel):
const [users, setUsers] = useState<User[]>([]);
const [loading, setLoading] = useState<boolean>(false);
const [count, setCount] = useState<number>(0);

✅ CORRECT (Babel-compatible):
const [users, setUsers] = useState(mockUsers);
const [loading, setLoading] = useState(false);
const [count, setCount] = useState(0);

**REASON:** Babel standalone (used in browser preview) does not support TypeScript generics in hooks. Let TypeScript infer types from initial values.
...
✅ Titles, headings, descriptions: ${language}
✅ Button labels: ${language}
✅ Form labels and placeholders: ${language}
✅ Error messages: ${language}
✅ Success messages: ${language}
✅ Comments and JSDoc: ${language}

**DOMAIN CONTEXT:** ${domain.toUpperCase()}
${domain === 'fitness' ? `
🏃 FITNESS DOMAIN:
- Use energetic, motivating language
- Focus on: achievements, progress, goals, workouts, health
- Colors: vibrant oranges, energetic reds, motivating greens
- Tone: Encouraging, positive, action-oriented
` : ''}
${domain === 'ecommerce' ? `
🛒 E-COMMERCE DOMAIN:
- Use persuasive, trust-building language
- Focus on: products, prices, cart, checkout, reviews
- Colors: trustworthy blues, conversion-focused oranges
- Tone: Professional, secure, customer-focused
` : ''}
${domain === 'landing-page' ? `
🎯 LANDING PAGE DOMAIN:
- Use compelling CTAs and value propositions
- Focus on: hero sections, benefits, testimonials, CTAs
- Colors: high-contrast, attention-grabbing
- Tone: Persuasive, benefit-driven, conversion-optimized
` : ''}
${domain === 'dashboard' ? `
📊 DASHBOARD DOMAIN:
- Use clear, data-focused language
- Focus on: metrics, charts, KPIs, analytics, insights
- Colors: professional blues, neutral grays
- Tone: Analytical, informative, actionable
` : ''}

**PERSONALITY:** ${personality.toUpperCase()}
${personality === 'professional' ? `
💼 PROFESSIONAL PERSONALITY:
- Colors: Blues (#3B82F6), grays (#6B7280), clean whites
- Typography: Sans-serif, medium weights, clear hierarchy
- Language: Formal, business-appropriate, trustworthy
- Design: Clean lines, structured layouts, corporate feel
` : ''}
${personality === 'energetic' ? `
⚡ ENERGETIC PERSONALITY:
- Colors: Vibrant oranges, bold reds, electric blues
- Typography: Bold weights, dynamic sizing, eye-catching
- Language: Motivational, action-oriented, enthusiastic
- Design: Dynamic layouts, bold CTAs, high energy
` : ''}
${personality === 'minimalist' ? `
✨ MINIMALIST PERSONALITY:
- Colors: Subtle grays, pure whites, accent blacks
- Typography: Light weights, generous spacing, refined
- Language: Concise, elegant, essential
- Design: Ample whitespace, clean lines, refined simplicity
` : ''}
...
✅ Semantic HTML: Use proper tags (<nav>, <main>, <article>, <button>)
...
✅ ONE file only (no separate models/api/utils files)
✅ NO imports from local files (only React and external libraries)
✅ ONE export default at the end
✅ ALL interfaces defined at top
✅ ALL helpers defined after interfaces
✅ Mock data is realistic and language-appropriate
✅ Uses correct color palette (${colorPalette[0]})
✅ NO TypeScript generics in hooks (useState, useEffect, etc)
✅ All text in ${language}
✅ Personality matches ${personality}
✅ Domain appropriate for ${domain}
✅ Accessible (ARIA, semantic HTML, keyboard nav)
✅ Responsive (mobile-first, works on all screen sizes)
✅ Interactive (hover states, loading states, toggles)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 COMPONENT RICHNESS (MANDATORY FOR DASHBOARDS/PAGES)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**IF GENERATING A DASHBOARD OR PAGE, MUST INCLUDE:**
...
2. **Data Display:**
   ✅ At least 3 StatCards showing KPIs
   ✅ At least 1 DataTable with sortable columns
   ✅ At least 1 Chart (can be placeholder with <div>)
...
4. **Navigation:**
   ✅ Sidebar links: Dashboard, Pedidos, Clientes, Relatórios, Configurações
   ✅ Header user menu with Perfil, Sair

5. **Mock Data:**
   ✅ At least 10 items in tables (to show scrolling)
   ✅ Realistic data with ${language} names
   ✅ Mix of statuses (pending, approved, rejected)

**EXAMPLE OUTPUT FOR "dashboard de vendas":**

✅ Full layout: Sidebar + Header + Main content
✅ 4 StatCards: Vendas Totais, Pedidos, Clientes, Taxa de Conversão
✅ DataTable com 10+ pedidos
✅ Chart placeholder
✅ Search bar
✅ User menu
✅ All interactive (sidebar toggle, sort, filter)

❌ DO NOT generate simple components with only 3 cards!
✅ Generate COMPLETE, PRODUCTION-READY dashboards!

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚠️  CRITICAL REMINDER: Return ONLY THE CODE. No explanations, no markdown blocks, no file names. Just the raw TypeScript code ready to be executed.`;
...
/**
 * ✅ AUTO-FIX DE CÓDIGO - TODOS OS 7 FIXES
 * Corrige automaticamente problemas de compatibilidade com Babel in-browser
 */
...
// ✅ ADICIONAR: Remove type annotations complexas (Error | null, string | null, etc)
fixed = fixed.replace(/:\s*([A-Z]\w+\s*\|\s*null)/g, '');  // Error | null
fixed = fixed.replace(/:\s*([a-z]+\s*\|\s*null)/g, '');    // string | null
fixed = fixed.replace(/:\s*([A-Z]\w+\s*\|\s*undefined)/g, ''); // Error | undefined
fixed = fixed.replace(/:\s*([a-z]+\s*\|\s*undefined)/g, '');   // string | undefined

// ✅ ADICIONAR: Remove arrays com tipo (string[], number[], etc)
fixed = fixed.replace(/:\s*string\[\]/g, '');
fixed = fixed.replace(/:\s*number\[\]/g, '');
fixed = fixed.replace(/:\s*boolean\[\]/g, '');
fixed = fixed.replace(/:\s*any\[\]/g, '');

// ✅ ADICIONAR: Remove type annotations de objetos ({ key: value })
...
logger.debug('Applied Fix 3: Removed all type annotations (aggressive mode)', {
  ...
  logger.debug('Applied Fix 3: Removed type annotations', {
    component: 'CognitiveGenerationEngine'
  });
  ...
  // ✅ (BABEL IN-BROWSER NÃO SUPORTA TEMPLATE STRINGS ANINHADAS!)
  // ✅ ═══════════════════════════════════════════════════════════════════
  
  // ❌ PROBLEMA: fetch(`https://api.com/${id}`) quebra no Babel
  // ✅ SOLUÇÃO: fetch('https://api.com/' + id)
  
  const fetchTemplateRegex = /fetch\s*\(\s*`([^`]*)\$\{([^}]+)\}([^`]*)`\s*\)/g;
  ...
  while ((fetchMatch = fetchTemplateRegex.exec(fixed)) !== null) {
    const original = fetchMatch[0]; // Toda a expressão fetch(...)
    const beforeVar = fetchMatch[1]; // Parte antes da variável
    const variable = fetchMatch[2];   // Nome da variável
    const afterVar = fetchMatch[3];   // Parte depois da variável
    
    // Construir versão concatenada
  ...
  if (fetchReplacements.length > 0) {
    logger.info(`Applied Fix 6: Converted ${fetchReplacements.length} fetch template strings to concatenation`, {
      component: 'CognitiveGenerationEngine'
    });
  }

  // ✅ ═══════════════════════════════════════════════════════════════════
  // ✅ FIX 7: REMOVER IMPORTS LOCAIS (TUDO DEVE SER UM ARQUIVO SÓ!)
  // ✅ (BABEL IN-BROWSER NÃO SUPORTA FILE SYSTEM!)
  // ✅ ═══════════════════════════════════════════════════════════════════

  // ❌ PROBLEMA: import Component from './Component' quebra no preview
  // ✅ SOLUÇÃO: Remover imports locais e garantir código único

  const localImportRegex = /import\s+.*?from\s+['"]\.\/.+?['"]\s*;?\s*\n?/g;
  ...
  if (localImportMatches && localImportMatches.length > 0) {
    ...
    // Remover todos os imports locais (que começam com './')
    fixed = fixed.replace(localImportRegex, '');
    
    logger.info(`Applied Fix 7: Removed ${localImportMatches.length} local imports from code`, {
      component: 'CognitiveGenerationEngine'
    });
  }

  // ✅ ═══════════════════════════════════════════════════════════════════
  // ✅ FIM DOS FIXES - RETORNAR CÓDIGO CORRIGIDO
  // ✅ ═══════════════════════════════════════════════════════════════════
  ...
  logger.info('✅ Auto-fix completed successfully', {
    ...
    metadata: {
      ...
      fixesApplied: [
        ...
        `Removed ${localImportMatches ? localImportMatches.length : 0} local imports`
      ]
    }
  });
