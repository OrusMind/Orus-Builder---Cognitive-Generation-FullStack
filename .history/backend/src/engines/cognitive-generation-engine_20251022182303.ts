...
// Reconstruct code
finalCode = (enhancedResult as any).enhancedFiles
  .map((f: any) => `// ${f.path}\n\n${f.content}`)
  .join('\n\n');
 ...
 */
/**
 * Call AI Provider (Wrapper for compatibility)
 */
private async callAIProvider(prompt: string, provider: string): Promise<string> {
  ...
  try {
    ...
    const result = await this.aiProvider.chat([
      { role: 'system', content: 'You are an expert TypeScript code generator. Generate production-ready code following all specifications.' },
