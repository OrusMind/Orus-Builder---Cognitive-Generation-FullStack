      ...
      if (this.config.enableLearning) {
        await learningEngine.recordEvent(
          ...
          PatternType.SUCCESS_PATTERN,
          {
            ...
            context: specification as Record<string, unknown>
          },
          {
  ...
  private async prepareSpecification(request: GenerationRequest): Promise<TechnicalSpecification> {
    switch (request.mode) {
      case GenerationMode.FROM_PROMPT:
        ...
        return analysis.data.specification;
...
components: [TrinityComponent.CEREBRO, TrinityComponent.ALMA],
...
layers: enhancement.data.synthesized.architecture.layers.map(l => l.name)
      };
    }
  }
