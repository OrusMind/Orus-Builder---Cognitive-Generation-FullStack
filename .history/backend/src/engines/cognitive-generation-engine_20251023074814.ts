...
private parseMultiFileCode(code: string, componentSpec: any): GeneratedComponent[] {
  ...
  // Regex atualizado: aceita "// src/..." OU "// FILE: src/..."
  const fileMarkerRegex = /^\/\/\s*(?:FILE:\s*)?src\/([^\s]+)\s*$/gm;
  
  console.log('[DEBUG] parseMultiFileCode: Starting parse...');
  console.log('[DEBUG] Code length:', code.length);
  
  const matches = Array.from(code.matchAll(fileMarkerRegex));
  console.log('[DEBUG] Found', matches.length, 'file markers');
  
  if (matches.length === 0) {
    console.log('[DEBUG] No markers found, returning empty array');
    return [];
  }
  ...
  matches.forEach((match, idx) => {
    ...
    const endPos = nextMatch ? nextMatch.index! : code.length;
    const fileCode = code.substring(startPos, endPos).trim();
    
    console.log(`[DEBUG] File ${idx}: ${fullPath} (${fileCode.split('\n').length} lines)`);
    
    const fileName = match[1]!.split('/').pop()!;
    
    files.push({
      id: `${componentSpec.name}-${idx}`,
      name: fileName.replace(/\.(tsx?|jsx?|css)$/, ''),
      type: this.getFileType(fileName),
      path: fullPath,
      code: fileCode,
      dependencies: this.extractDependencies(fileCode),
      metadata: {
        linesOfCode: fileCode.split('\n').length,
        complexity: this.calculateComplexity(fileCode),
        coverage: 0
      }
    });
  });
  
  console.log('[DEBUG] Returning', files.length, 'parsed files');
  return files;
}

private getFileType(fileName: string): string {
  if (fileName.endsWith('.tsx') || fileName.endsWith('.jsx')) return 'component';
  if (fileName.endsWith('.ts')) return 'utility';
