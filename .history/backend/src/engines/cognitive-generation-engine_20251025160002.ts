...
/**
 * ═══════════════════════════════════════════════════════════════
 * SPLIT AI CODE INTO COMPONENTS - VERSÃO FINAL v4.0
 * ═══════════════════════════════════════════════════════════════
 * FEATURES:
 * - 5 formatos de parsing (component:Name, comments, markdown, React, JSON)
 * - Entity name extraction (Button, Card, Modal)
 * - Auto-fix naming (Item → Button)
...
private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
  ...
  if (components.length === 0) {
    ...
    const tripleBacktick = '```';
const pattern = `${tripleBacktick}(?:typescript|tsx|jsx)\\n([\\s\\S]*?)${tripleBacktick}`;  // TUDO EM UMA LINHA!
  ...
  if (components.length === 0) {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        // Process arrays: controllers, services, middleware, models, routes, config, utils, validators
  ...
  // ═══════════════════════════════════════════════════════════════
  // FALLBACK: Código completo em um único arquivo
  // ═══════════════════════════════════════════════════════════════
  if (components.length === 0) {
  ...
  // ═══════════════════════════════════════════════════════════════
  // POST-PROCESSING: VALIDATE & AUTO-FIX NAMING
  // ═══════════════════════════════════════════════════════════════
 ...
 * ═══════════════════════════════════════════════════════════════
 * Examples:
 * - "Crie um componente de botão..." → "Button"
 * - "Create a card component..." → "Card"
 * - "Build a modal dialog..." → "Modal"
 */
private extractMainEntity(prompt: string): string {
  const promptLower = prompt.toLowerCase();
  
  // Keywords mapping (PT + EN)
 ...
 * - "Item" → "Button" (if prompt mentioned "button")
 * - "Component" → actual entity name
 * - Updates: name, path, code content, files
 */
private validateAndFixNaming(components: any[], expectedEntity: string): void {
  ...
  components.forEach((comp, idx) => {
    ...
    if (genericNames.includes(currentName)) {
      ...
      // Fix code content (replace all occurrences)
      if (comp.code) {
...
private extractComponentName(filePath: string, code: string): string {
  ...
  // Try to extract from code (export statements)
...
private detectScopeFromIntent(
  ...
  // ═══════════════════════════════════════════════════════════════
  // PRIORITY 0: SINGLE_COMPONENT (NOVO! DETECTAR ANTES DE TUDO)
  // ═══════════════════════════════════════════════════════════════
...
complexity: 'complex',      confidence: 0.95,
...
complexity: 'moderate',
  ...
  // ═══════════════════════════════════════════════════════════════
  // PRIORITY 4: PAGE/FEATURE (DEFAULT)
  // ═══════════════════════════════════════════════════════════════
