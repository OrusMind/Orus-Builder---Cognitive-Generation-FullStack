...
class ContextBuilder {
  ...
  private async analyzePrompt(request: GenerationRequest): Promise<PromptAnalysisResult> {
    try {
      // Use PromptProcessor (8-stage pipeline) - CORRIGIDO: 1 argumento apenas
      const processed = await promptProcessor.process(request.prompt);

      // Mapear ProcessingResult para PromptAnalysisResult
      return {
        ...
        requirements: Array.isArray(processed.requirements) 
          ? processed.requirements 
          : (processed.requirements ? [processed.requirements] : []),
        ambiguities: processed.ambiguity ? [processed.ambiguity] : [],
        context: {
          domain: processed.context?.domain || request.context?.domain || 'general',
          complexity: processed.context?.complexity || 'standard',
          stylePreferences: processed.context?.stylePreferences || request.context?.stylePreferences,
          colorPalette: processed.context?.colorPalette || request.context?.colorPalette,
          personality: processed.context?.personality || request.context?.personality
        },
        specification: processed.specification,
        confidence: processed.confidence || 50
      };
