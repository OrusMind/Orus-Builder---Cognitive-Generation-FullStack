...
private mergeToSingleFile(parsedFiles: GeneratedComponent[]): GeneratedComponent {
  ...
  parsedFiles.forEach(file => {
    const interfaces = this.extractInterfaces(file.code);
  ...
  parsedFiles.forEach(file => {
    const helpers = this.extractHelpers(file.code);
  ...
  parsedFiles.forEach(file => {
    if (file.path.endsWith('.tsx')) {
      const cleanComponent = this.cleanComponent(file.code);
      if (cleanComponent) {
        ...
        if (file.name.toLowerCase().includes('app')) {
          const match = cleanComponent.match(/const\s+(\w+):\s*React\.FC/);
 ...
 */
private extractHelpers(code: string): string[] {
 ...
 */
private cleanComponent(code: string): string {
  let cleaned = code
    // Remove imports
