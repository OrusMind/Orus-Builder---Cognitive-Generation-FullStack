 ...
 */
  ...
  private async stagePrepare(request: GenerationRequest): Promise<PipelineStageResult> {
    ...
    const userComplexity = request.options?.complexity; // Extrair do request
const scope = this.detectScopeFromIntent(analysis, request.prompt, userComplexity);

console.log(`✅ [Pipeline] Scope detected: ${scope.type} (confidence: ${scope.confidence})`);
if (userComplexity) {
  console.log(`🎚️ [Pipeline] User override applied: ${userComplexity}`);
}
...
`;
    ...
    if (scope.type === ScopeType.FULLSTACK) {
      ...
      try {
        ...
        console.error('❌ [Pipeline] CODE-GENERATOR FAILED! Falling back to AI Provider...');

        const aiResponse = await this.aiProvider.generateCode({
          prompt: enrichedPrompt, // ✅ USA O PROMPT COM FILE COUNT INSTRUCTION
          maxTokens: 8000,
          temperature: 0.7
        });
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * SCOPE DETECTION FROM INTENT - VERSION 3.0 (USER OVERRIDE SUPPORT)
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * Detecta scope preciso baseado no intent classificado pelo PromptProcessor
 * 
 * NEW FEATURE: Aceita userComplexity do frontend como OVERRIDE
 * 
 * PRIORITY ORDER:
 * 0. USER_OVERRIDE (manual complexity selection) ← ALWAYS FIRST!
 * 1. SINGLE_COMPONENT
 * 2. FULLSTACK
 * 3. BACKEND
 * 4. LANDING_PAGE
 ...
 * @param userComplexity - User-selected complexity (optional OVERRIDE)
 * @returns ScopeDetectionResult with confidence and file count expectations
 */
private detectScopeFromIntent(
  analysis: PromptAnalysisResult,
  originalPrompt: string,
  userComplexity?: 'minimal' | 'standard' | 'feature_rich' | 'enterprise'
): ScopeDetectionResult {
  const promptLower = originalPrompt.toLowerCase();
  const detectedKeywords: string[] = [];

  // ═══════════════════════════════════════════════════════════════════════════
  // PRIORITY 0: USER OVERRIDE (ALWAYS RESPECTED!)
  // ═══════════════════════════════════════════════════════════════════════════
  if (userComplexity) {
    console.log(`🎚️ [Scope] ═════════════════════════════════════════════════`);
    console.log(`🎚️ [Scope] USER OVERRIDE DETECTED: "${userComplexity}"`);
    console.log(`🎚️ [Scope] Ignoring prompt-based detection`);
    console.log(`🎚️ [Scope] ═════════════════════════════════════════════════`);
    
    detectedKeywords.push(`user_override_${userComplexity}`);
    
    // ✅ MAPA DE COMPLEXIDADE PARA SCOPE
    const complexityMap: Record<string, ScopeDetectionResult> = {
      'minimal': {
        type: ScopeType.SINGLE_COMPONENT,
        complexity: 'simple' as ComplexityLevel,
        confidence: 1.0, // 100% - escolha manual
        expectedFileCount: { min: 1, max: 3 },
        shouldIncludeFrontend: true,
        shouldIncludeBackend: false,
        shouldIncludeDatabase: false,
        detectedKeywords: [...detectedKeywords, 'minimal_mvp']
      },
      'standard': {
        type: ScopeType.SINGLE_COMPONENT,
        complexity: 'moderate' as ComplexityLevel,
        confidence: 1.0,
        expectedFileCount: { min: 4, max: 8 },
        shouldIncludeFrontend: true,
        shouldIncludeBackend: false,
        shouldIncludeDatabase: false,
        detectedKeywords: [...detectedKeywords, 'standard_component']
      },
      'feature_rich': {
        type: ScopeType.FEATURE,
        complexity: 'high' as ComplexityLevel,
        confidence: 1.0,
        expectedFileCount: { min: 15, max: 25 },
        shouldIncludeFrontend: true,
        shouldIncludeBackend: true,
        shouldIncludeDatabase: false,
        detectedKeywords: [...detectedKeywords, 'feature_rich_set']
      },
      'enterprise': {
        ...
        detectedKeywords: [...detectedKeywords, 'enterprise_fullstack']
      }
    };
    
    // ✅ RETORNAR SCOPE MAPEADO
    const mappedScope = complexityMap[userComplexity];
    
    if (mappedScope) {
      console.log(`✅ [Scope] Mapped to: ${JSON.stringify(mappedScope, null, 2)}`);
      return mappedScope;
    }
    
    // ⚠️ FALLBACK: Se userComplexity inválido
    console.log(`⚠️ [Scope] Invalid userComplexity: "${userComplexity}"`);
    console.log(`⚠️ [Scope] Falling back to auto-detection...`);
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // AUTO-DETECTION FALLBACK (se não houver override)
  // ═══════════════════════════════════════════════════════════════════════════
  
  console.log(`🔍 [Scope] No user override, starting auto-detection...`);
  ...
  const hasFrontend =
    promptLower.includes('frontend') || promptLower.includes('react');
  const hasBackend =
    promptLower.includes('backend') ||
  ...
  if (hasFullstack || (hasFrontend && hasBackend)) {
    ...
    return {
      ...
      confidence: 0.95,
      expectedFileCount: { min: 30, max: 45 },
      shouldIncludeFrontend: true,
      shouldIncludeBackend: true,
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // PRIORITY 5: FEATURE (DEFAULT FALLBACK)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log(`⚠️ [Scope] AUTO-DETECTED: FEATURE (default fallback)`);
  console.log(`⚠️ [Scope] Unable to classify precisely, using generic feature scope`);
...
/**
 * Map user complexity to scope configuration
 * @param complexity - User-selected complexity level
 */
private mapComplexityToScope(
  complexity: 'minimal' | 'standard' | 'feature_rich' | 'enterprise'
): ScopeDetectionResult {
  
  // ✅ MAPA DE COMPLEXIDADE PARA SCOPE (TIPAGEM CORRETA)
  const complexityMap: Record<string, ScopeDetectionResult> = {
    'minimal': {
      type: ScopeType.SINGLE_COMPONENT, // ✅ Usar ScopeType.SINGLE_COMPONENT (não string)
      complexity: 'simple' as ComplexityLevel,
      confidence: 1.0,
      expectedFileCount: { min: 1, max: 3 },
      shouldIncludeFrontend: true,
      shouldIncludeBackend: false,
      shouldIncludeDatabase: false,
      detectedKeywords: ['user_override_minimal']
    },
    'standard': {
      type: ScopeType.SINGLE_COMPONENT, // ✅ ENUM
      complexity: 'moderate' as ComplexityLevel,
      confidence: 1.0,
      expectedFileCount: { min: 4, max: 8 },
      shouldIncludeFrontend: true,
      shouldIncludeBackend: false,
      shouldIncludeDatabase: false,
      detectedKeywords: ['user_override_standard']
    },
    'feature_rich': {
      type: ScopeType.FEATURE, // ✅ ENUM (não 'feature_set')
      complexity: 'complex' as ComplexityLevel, // ✅ 'complex' (não 'high')
      confidence: 1.0,
      expectedFileCount: { min: 15, max: 25 },
      shouldIncludeFrontend: true,
      shouldIncludeBackend: true,
      shouldIncludeDatabase: false,
      detectedKeywords: ['user_override_feature_rich']
    },
    'enterprise': {
      type: ScopeType.FULLSTACK, // ✅ ENUM
      complexity: 'complex' as ComplexityLevel, // ✅ 'complex' (não 'very_high')
      confidence: 1.0,
      expectedFileCount: { min: 30, max: 60 },
      shouldIncludeFrontend: true,
      shouldIncludeBackend: true,
      shouldIncludeDatabase: true,
      detectedKeywords: ['user_override_enterprise']
    }
  };
  
  // ✅ RETORNAR SCOPE MAPEADO
  const mappedScope = complexityMap[complexity];
  
  if (mappedScope) {
    console.log(`✅ [Scope] Mapped complexity "${complexity}" to scope:`, JSON.stringify(mappedScope, null, 2));
    return mappedScope;
  }
  
  // ⚠️ FALLBACK: Se userComplexity inválido, retornar standard
  console.log(`⚠️ [Scope] Invalid userComplexity: "${complexity}", falling back to standard`);
  
  // ✅ GARANTIR RETORNO (não pode ser undefined)
  const standardScope = complexityMap['standard'];
  if (standardScope) {
    return standardScope;
  }
  
  // ✅ ÚLTIMO FALLBACK (caso extremo - nunca deve acontecer)
  return {
    type: ScopeType.SINGLE_COMPONENT,
    complexity: 'moderate' as ComplexityLevel,
    confidence: 0.5,
    ...
    shouldIncludeBackend: false,
    shouldIncludeDatabase: false,
    detectedKeywords: ['fallback_standard']
  };
}
