  ...
  if (propMatch[1] && propMatch[3]) {
    properties.push({
      ...
      optional: !!propMatch[2],
            ...
            if (match[1]) {
  entities.push({ name: match[1], type: 'interface', properties });
}
          }
        ...
        if (options.extractServices) {
          ...
          while ((match = serviceRegex.exec(content)) !== null) {
            ...
            while ((methodMatch = methodRegex.exec(body)) !== null) {
              methods.push({
                name: methodMatch[1],
                isAsync: content.includes(`async ${methodMatch[1]}`),
                isCRUD: /create|get|update|delete/i.test(methodMatch[1]),
              });
            }
           if (match[1]) {
  services.push({ name: match[1], type: 'class', methods });
}
          }
        }
      ...
      logger.info('✅ [Blueprint Engine] Code parsing completed', {
        component: 'BlueprintEngine',
        metadata: { entities: entities.length, services: services.length, components: components.length }
      });
