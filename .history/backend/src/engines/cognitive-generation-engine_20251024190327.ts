...
private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
  ...
  if (components.length === 0) {
    console.log('🔍 [Pipeline] Tentativa 2: Formato // component:Name.tsx:tsx:path');
    const format2Regex = /``````/gi;
  ...
  // ✅ FORMATO 3: ```
  if (components.length === 0) {
    console.log('🔍 [Pipeline] Tentativa 3: Formato // path/to/file.tsx');
    const format3Regex = /```(?:typescript|tsx|jsx|javascript|ts|js)?\s*\n(?:\/\/\s*(.+?\.(tsx?|jsx?))\s*)?\n([\s\S]*?)```
  ...
  if (components.length === 0) {
    ...
    const componentRegex = /(?:export\s+default\s+function\s+(\w+)|export\s+function\s+(\w+)|const\s+(\w+)\s*:\s*React\.FC)/g;
