...
private buildFrontendPrompt(
  ...
  const prompt = `You are generating a ${framework} TypeScript component named "${component.name}".
...
You are generating code for a BROWSER SANDBOX with LIMITED capabilities!

AVAILABLE LIBRARIES (ONLY THESE!):
✅ React (useState, useEffect, useCallback, useMemo, useRef, useContext)
✅ ReactDOM
✅ Standard JavaScript (Array, Object, Math, Date, JSON, Promise, etc)
✅ Standard HTML elements (div, button, input, form, table, etc)
✅ Tailwind CSS classes (inline only)

FORBIDDEN LIBRARIES (WILL BREAK PREVIEW!):
❌ Recharts, Chart.js, Victory, D3, any chart library
❌ Axios (use fetch with mock data instead)
❌ Material-UI, Ant Design, Chakra UI (any UI framework)
❌ Lodash, Moment.js, date-fns (any utility library)
❌ React Router, Next.js Link (any routing)
❌ Framer Motion, React Spring (any animation library)
❌ TanStack Query, SWR (any data fetching library)
❌ Zustand, Redux, Jotai (any state management library)

**IF USER ASKS FOR CHARTS/GRAPHS:**
Create CSS-based visualizations using divs with dynamic heights!
...
return (
  <div className="flex items-end h-64 gap-2 p-4 bg-white rounded-lg">
    {data.map(item => (
...
**IF USER ASKS FOR DASHBOARDS:**
Use stats cards with numbers, tables, and CSS-based simple charts!

**IF USER ASKS FOR DATA FETCHING:**
Use mock data defined in the component (NO axios, NO real APIs)!
\`\`\`typescript
// ✅ CORRECT: Mock data
const mockData = [
  { id: 1, name: 'Item 1' },
...
**CRITICAL:** If you generate code using forbidden libraries, the preview WILL FAIL!
Users can only preview components with React + Tailwind + mock data!
...
6. **Charts:** Placeholder with <div> (or simple bar chart)
...
🚨 CRITICAL: Return ONLY the TypeScript code. NO explanations, NO markdown blocks, NO file names, NO comments before/after. Just the raw code starting with "import React" and ending with "export default ${component.name};".
