...
private fixTypeScriptForSandpack(code: string): string {
  ...
  // Fix 7: REMOVER MARKDOWN FENCES (VERSÃO ULTRA-AGRESSIVA)
logger.info('🧹 [FIX 7] Starting ULTRA-AGGRESSIVE markdown fence removal', {
  component: 'CognitiveGenerationEngine'
} as any);

let totalRemoved = 0;

// Lista completa de linguagens que AI pode usar
const languages = ['tsx', 'jsx', 'ts', 'js', 'typescript', 'javascript', 'css', 'html', 'json', 'md', 'markdown'];
...
languages.forEach(lang => {
  ...
  // Loop até não encontrar mais
  while (fixed.includes(pattern) && count < 50) {
    fixed = fixed.split(pattern).join(''); // Método mais agressivo que replace
    count++;
  }
  
  if (count > 0) {
    totalRemoved += count;
    logger.info(`  🔥 Removed ${count}x "${pattern}"`, {
      component: 'CognitiveGenerationEngine'
    } as any);
  }
});
...
// Remove ```
let plainBackticksCount = 0;
let maxIterations = 100;
while (fixed.includes('```') && maxIterations > 0) {
fixed = fixed.split("```  plainBackticksCount++;
  maxIterations--;
}

totalRemoved += plainBackticksCount;

if (plainBackticksCount > 0) {
  logger.info(`  🔥 Removed ${plainBackticksCount}x plain backticks`, {
    component: 'CognitiveGenerationEngine'
  } as any);
}

// Remove headers markdown (##, ###, ####)
const headersRemoved = (fixed.match(/^#{1,6}\s+.+$/gm) || []).length;
fixed = fixed.replace(/^#{1,6}\s+.+$/gm, '');
if (headersRemoved > 0) {
  logger.info(`  🔥 Removed ${headersRemoved} markdown headers`, {
    component: 'CognitiveGenerationEngine'
  } as any);
}

// Remove linhas vazias em excesso (mais de 2 consecutivas)
fixed = fixed.replace(/\n{3,}/g, '\n\n');

// VERIFICAÇÃO FINAL
if (fixed.includes('```')) {
  logger.error('⚠️⚠️⚠️ CRITICAL: MARKDOWN FENCES STILL PRESENT AFTER FIX 7!', {
    component: 'CognitiveGenerationEngine',
    metadata: { 
      snippet: fixed.substring(fixed.indexOf('``````') + 100)
    }
  } as any);
} else {
  logger.info(`✅ [FIX 7] ULTRA-AGGRESSIVE removal COMPLETE - ${totalRemoved} total removals`, {
    component: 'CognitiveGenerationEngine'
  } as any);
}
...
private buildFrontendPrompt(
  ...
  const prompt = `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 DESIGN-FIRST GENERATION - ${component.name.toUpperCase()}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**DOMAIN:** ${domain.toUpperCase()}
${domain === 'fitness' ? '→ Energetic colors, motivational copy, progress tracking' : ''}
${domain === 'ecommerce' ? '→ Conversion-focused, CTAs, product showcases' : ''}
${domain === 'landing' ? '→ Hero impact, 6+ sections, gradient backgrounds' : ''}
${domain === 'dashboard' ? '→ Data visualization, metrics, tables' : ''}

**PERSONALITY:** ${personality.toUpperCase()}
${personality === 'professional' ? '→ Clean layout, neutral tones (blue/gray), minimal' : ''}
${personality === 'casual' ? '→ Rounded corners, gradients, vibrant colors' : ''}
${personality === 'motivational' ? '→ Bold CTAs, inspiring copy, energetic design' : ''}

**COLOR PALETTE:**
• Primary: ${colorPalette[0]} (use as bg-blue-600/text-blue-600 equivalent)
• Secondary: ${colorPalette[1] || colorPalette[0]} (use as bg-gray-600 equivalent)
• Apply consistently: buttons, headers, cards, links, active states

**LANGUAGE:** ${language} (all text, names, dates, currency)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 SANDBOX CONSTRAINTS (CRITICAL - VIOLATIONS CAUSE ERRORS)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✅ ALLOWED: React (useState, useEffect), Tailwind CSS, inline interfaces
❌ FORBIDDEN: External libraries (Recharts, axios, Material-UI, lodash)
❌ FORBIDDEN: API calls (fetch, axios) - use mock data with setTimeout
❌ FORBIDDEN: Multiple files - merge ALL into ONE file
❌ FORBIDDEN: TypeScript generics in hooks - let TS infer types
🚨 ABSOLUTE BAN ON CHART LIBRARIES - AI KEEPS TRYING TO IMPORT THEM!

IF USER ASKS FOR CHARTS/GRAPHS/ANALYTICS:
- Do NOT import: Recharts, Chart.js, Victory, Nivo, react-chartjs
- Do NOT create: <Chart>, <LineChart>, <BarChart>, <PieChart> components
- INSTEAD: Use CSS-based visualization with <div> bars

Example working chart:
const chartData = [{ label: 'Jan', value: 65 }, { label: 'Feb', value: 85 }];
return (
  <div className="flex items-end h-64 gap-2 bg-white p-4">
    {chartData.map((item) => (
...
IF AI GENERATES import { Chart } FROM ANY LIBRARY, THE PREVIEW WILL FAIL!
...
// 1. Interfaces (inline, no exports)
interface User { id: number; name: string; }

// 2. Helpers (inline, ${language} formatting)
...
// 3. Mock data (realistic ${language} content)
const mockData = [
  { id: 1, name: '${language === 'pt-BR' ? 'João Silva' : 'John Smith'}' }
];

// 4. Sub-components (if needed)
function Card({ title }: { title: string }) {
  return <div className="bg-white p-4 rounded-lg shadow">{title}</div>;
}
...
**Layout:**
• Full-page: min-h-screen w-full (NOT h-screen)
• Flexbox/Grid only (NO absolute positioning)
• Semantic HTML: <nav>, <main>, <article>, <section>

**Colors (from palette):**
• Buttons: bg-[${colorPalette[0]}] hover:bg-[${colorPalette[0]}]/90
• Text: text-[${colorPalette[0]}]
• Borders: border-[${colorPalette[1] || colorPalette[0]}]
...
Required states:
• Toggle (sidebar, modals): const [isOpen, setIsOpen] = useState(false);
• Selection: const [selectedId, setSelectedId] = useState(null);
• Search/Filter: const [searchTerm, setSearchTerm] = useState('');
...
Required sections (6 minimum):
1. Hero: bg-gradient-to-br from-[${colorPalette[0]}] to-[${colorPalette[1] || colorPalette[0]}]
   • Title: text-5xl md:text-7xl font-extrabold
   • CTA: button with hover:shadow-2xl
2. Features: 3-6 cards with emojis/icons
...
Visual effects:
• Gradients on hero + CTAs
• Animations: hover:-translate-y-2 transform
• Shadows: shadow-2xl
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
` : domain === 'dashboard' ? `
...
Required components:
1. Sidebar: Navigation with 5+ links
2. Header: User menu, notifications
3. Stats: 3+ metric cards with icons
4. Data Table: Sortable columns
5. Charts: CSS-based bars/progress (NO Recharts!)

Chart example (CSS bars):
<div className="flex items-end h-64 gap-2">
  {data.map((item, i) => (
    <div key={i} className="flex-1 flex flex-col items-center">
      <div 
        className="w-full bg-[${colorPalette[0]}] rounded-t"
        style={{ height: \`\${item.value}%\` }}
      />
...
✅ ONE file only (no imports except 'react')
✅ Mock data in ${language} with realistic names
✅ Colors ${colorPalette[0]} applied consistently
✅ Personality ${personality} reflected in design
✅ Interactive (hover, loading, toggle states)
✅ Responsive (mobile-first with md: breakpoints)
✅ Accessible (ARIA labels, semantic HTML)
✅ NO external libraries (NO Recharts, NO axios!)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 CRITICAL: PREVENT UNDEFINED .map() ERRORS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ALWAYS initialize state with mock data arrays:

❌ WRONG (causes undefined error):
const [features, setFeatures] = useState();
{features.map(f => ...)} // ❌ ERROR: Cannot read 'map' of undefined

✅ CORRECT (works immediately):
const mockFeatures = [{ id: 1, title: 'Feature 1' }];
const [features, setFeatures] = useState(mockFeatures);
{features.map(f => ...)} // ✅ Works!

Generate production-ready ${framework} TypeScript component now.
