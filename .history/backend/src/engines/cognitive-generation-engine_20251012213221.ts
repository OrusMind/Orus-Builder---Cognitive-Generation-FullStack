...
// backend/src/engines/cognitive-generation-engine.ts
// MANTER O CÓDIGO ORIGINAL (NÃO TROCAR):
...
private async generateWithAI(
  ...
  type: 'component',
  purpose: request.prompt || 'Component functionality',
...
private async validateWithCIG(files: GeneratedFile[]): Promise<{ score: number; errors: string[]; warnings: string[] }> {
  ...
  component: 'CognitiveGenerationEngine',
  metadata: { error: (error as Error).message }  // ✅ Mover para metadata
});
...
private extractComponentNameFromPrompt(prompt: string | undefined): string {
  if (!prompt) return 'Component';
