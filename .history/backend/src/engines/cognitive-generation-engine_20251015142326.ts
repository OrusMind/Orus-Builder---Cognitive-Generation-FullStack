...
1. **NO Duplicate Declarations:**
   ...
   - Check for duplicates before adding new declarations

2. **Compatible with In-Browser Babel:**
   - AVOID complex generics in useState: Use \`useState([])\` NOT \`useState<Product[]>([])\`
   - AVOID type assertions with 'as': Use type inference
   - Use simple TypeScript syntax
   - Prefer explicit types over complex inference

3. **Code Structure:**
   - Declare all interfaces/types at the TOP
   - Then declare helper functions
   - Then declare main component
...
// 4. MAIN COMPONENT (use different name than interfaces!)
function ShoppingApp() {
  // AVOID: const [products, setProducts] = useState<Product[]>([]);
