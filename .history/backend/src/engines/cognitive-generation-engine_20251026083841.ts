...
private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
  ...
  try {
    ...
    while ((match = format1Pattern.exec(aiCode)) !== null) {
      const componentName = match ? match.trim() : '';[1]
      const language = match ? match.trim() : 'typescript';[2]
      const filePath = match ? match.trim() : '';[3]
      const code = match ? match.trim() : '';[4]
    ...
    if (components.length === 0) {
      ...
      while ((match = format3Pattern.exec(aiCode)) !== null) {
        const filePath = match ? match.trim() : '';[1]
        let code = match ? match.trim() : '';[2]
    ...
    } // ✅ FECHA o IF de components.length === 0 (TENTATIVA 4)
    ...
    } // ✅ FECHA o IF de components.length === 0 (TENTATIVA 5)
    ...
    } // ✅ FECHA o IF de components.length === 0 (FALLBACK)
    ...
    return components; // ✅ RETURN PRINCIPAL

  } catch (error) {
    console.error('[Pipeline] ❌ Erro no splitCodeIntoComponents:', error);
    return components; // ✅ RETURN EM CASO DE ERRO
  }
} // ✅ FECHA O MÉTODO splitCodeIntoComponents
