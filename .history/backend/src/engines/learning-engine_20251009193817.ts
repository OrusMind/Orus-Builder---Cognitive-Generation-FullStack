  ...
  private async generateInsights(): Promise<void> {
    ...
    if (successPatterns.length >= 5) {
      ...
      const insight: LearningInsight = {
        ...
        insightId,
        type: 'pattern',
        title: 'Strong Success Patterns Identified',
        description: `Found ${successPatterns.length} reliable success patterns`,
        evidence: successPatterns.map(p => p.name),
        confidence: 85,
        actionable: true,
        suggestedAction: 'Apply these patterns to new generations for better results',
        version: 1,
        isDeleted: false,
        createdAt: now,
        updatedAt: now
