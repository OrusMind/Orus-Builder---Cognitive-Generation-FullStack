...
/**
 * Limpa componente removendo imports, exports e comentários (versão SAFE)
 */
private cleanComponentCode(code: string): string {
  let cleaned = code
    // Remove imports de bibliotecas externas
    .replace(/import\s+.*?from\s+['"](?:recharts|axios|lodash|moment|chart\.js|victory|@mui|antd)['"];?\s*/g, '')
    // Remove imports de paths relativos
    .replace(/import\s+.*?from\s+['"]\..*?['"]\s*;?\s*/g, '')
    // Remove comentários de linha de arquivo (// todo.ts, // TodoList.tsx)
    .replace(/^\s*\/\/\s+[\w\/]+\.(ts|tsx)\s*$/gm, '')
    // Remove blocos de comentários JSDoc COMPLETOS (não parciais!)
    .replace(/\/\*\*[\s\S]*?\*\/\s*/g, '')
    // Remove comentários de linha simples no início (// Import, // Define, etc)
    .replace(/^\s*\/\/\s+[A-Z].*$/gm, '')
  ...
  // ✅ VERIFICAÇÃO FINAL: Se código ficou muito pequeno, retornar vazio
