...
private fixTypeScriptForSandpack(code: string): string {
  ...
  // Fix 7: REMOVER MARKDOWN FENCES (``````css, ```
  const fenceMatches = fixed.match(/```[a-zA-Z]*\n?/g);
  if (fenceMatches) {
    logger.info(`  🧹 Removing ${fenceMatches.length} markdown fences from code`, {
      component: 'CognitiveGenerationEngine',
      metadata: { fences: fenceMatches.slice(0, 5) } // Log apenas primeiros 5 para não poluir
    } as any);
    
    // Remove ALL markdown fences (``````css, ``````)
    fixed = fixed
      .replace(/```
