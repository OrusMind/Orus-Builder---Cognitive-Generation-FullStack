...
private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
  ...
  console.log('[Pipeline] AI code preview:', aiCode.substring(0, 300));
  
  const components: any[] = [];
  let match: RegExpExecArray | null;
  
  console.log('🔍 [Pipeline] Tentativa 1: Formato component:Name:tsx:path');
  const codeBlockPattern = '```component:([^:]+):([^:]+):([^' + String.fromCharCode(92) + 'n]+)' + String.fromCharCode(92) + 'n([' + String.fromCharCode(92) + 's' + String.fromCharCode(92) + 'S]*?)```';
  const format1Regex = new RegExp(codeBlockPattern, 'gi');
  
  while ((match = format1Regex.exec(aiCode)) !== null) {
    const componentName = (match && typeof match === 'string') ? match.trim() : '';
    const language = (match && typeof match === 'string') ? match.trim() : 'typescript';
    const filePath = (match && typeof match === 'string') ? match.trim() : '';
    const code = (match && typeof match === 'string') ? match.trim() : '';
  ...
  if (components.length === 0) {
    console.log('🔍 [Pipeline] Tentativa 2: Formato // component:');
    const slashN = String.fromCharCode(92) + 'n';
    const slashS = String.fromCharCode(92) + 's';
    const slashSlash = String.fromCharCode(92) + '/';
    const format2Pattern = '```(?:typescript|tsx|jsx|javascript|ts|js)?' + slashS + '*' + slashN + slashSlash + slashSlash + slashS + '*component:([^:]+):([^:]+):([^' + slashN + ']+)' + slashN + '([' + slashS + slashS.toUpperCase() + ']*?)```';
    const format2Regex = new RegExp(format2Pattern, 'gi');
    
    while ((match = format2Regex.exec(aiCode)) !== null) {
      const fileName = (match && typeof match === 'string') ? match.trim() : '';
      const language = (match && typeof match === 'string') ? match.trim() : 'typescript';
      const filePath = (match && typeof match === 'string') ? match.trim() : '';
      const code = (match && typeof match === 'string') ? match.trim() : '';
  ...
  if (components.length === 0) {
    console.log('🔍 [Pipeline] Tentativa 3: Detectar múltiplos blocos markdown');
    const n = String.fromCharCode(92) + 'n';
    const s = String.fromCharCode(92) + 's';
    const slash = String.fromCharCode(92) + '/';
    const format3Pattern = '```(?:typescript|tsx|jsx|javascript|ts|js)?' + s + '*' + n + '(?:' + slash + slash + s + '*(.+?.(tsx?|jsx?))' + s + '*)?' + n + '([' + s + s.toUpperCase() + ']*?)```';
    const format3Regex = new RegExp(format3Pattern, 'gi');
    
    while ((match = format3Regex.exec(aiCode)) !== null) {
      const filePath = (match && typeof match === 'string') ? match.trim() : '';
      const code = (match && typeof match === 'string') ? match.trim() : '';
  ...
  if (components.length === 0) {
    ...
    const componentPattern = '(?:export' + s + '+default' + s + '+function' + s + '+(' + w + '+)|export' + s + '+function' + s + '+(' + w + '+)|const' + s + '+(' + w + '+)' + s + '*:' + s + '*React' + String.fromCharCode(92) + '.FC)';
    const componentRegex = new RegExp(componentPattern, 'g');
    const matches: Array<{ name: string; start: number }> = [];
    
    while ((match = componentRegex.exec(aiCode)) !== null) {
      const nameValue = match || match || match;
      const name = (nameValue && typeof nameValue === 'string') ? nameValue : '';
    ...
    console.log(`[Pipeline] Componentes detectados: ${matches.length}`);
...
private extractComponentName(filePath: string, code: string): string {
  ...
  const s = String.fromCharCode(92) + 's';
  const w = String.fromCharCode(92) + 'w';
  const namePattern = '(?:export' + s + '+default' + s + '+(?:function|const)' + s + '+(' + w + '+)|export' + s + '+(?:function|const)' + s + '+(' + w + '+)|const' + s + '+(' + w + '+)' + s + '*:' + s + '*React' + String.fromCharCode(92) + '.FC|class' + s + '+(' + w + '+))';
  const nameRegex = new RegExp(namePattern);
  const nameMatch = code.match(nameRegex);
  
  if (nameMatch) {
    return nameMatch || nameMatch || nameMatch || nameMatch || 'Component';
  }
