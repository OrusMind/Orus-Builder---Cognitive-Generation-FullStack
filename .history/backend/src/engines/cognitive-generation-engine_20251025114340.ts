  ...
  for (const section of sections) {
    ...
    if (pathMatch && pathMatch[1]) {
      ...
      if (codeMatch && codeMatch[1]) {
        ...
        // Remove markdown code fences - Usando replace de string simples
...
if (components.length === 0) {
  ...
  // Usando RegExp constructor para evitar problemas com backticks
  const tripleBacktick = '```';
  const format3Pattern = new RegExp(
    tripleBacktick + '(?:typescript|tsx|jsx|javascript|ts|js)?\\s*\\n(?:\\/\\/\\s*(.+?\\.(?:tsx?|jsx?))\\s*)?\\n?([\\s\\S]*?)' + tripleBacktick,
    'gi'
  );
...
if (components.length === 0) {
  ...
  // Padrões de limpeza usando RegExp constructor
  const tripleBacktick = '```
  const markdownPattern1 = new RegExp(tripleBacktick + '(?:typescript|tsx|jsx|javascript|ts|js)?\\s*\\n?', 'g');
  const markdownPattern2 = new RegExp(tripleBacktick + '\\s*$', 'g');
