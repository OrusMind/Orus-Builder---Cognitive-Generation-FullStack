...
import { 
  cigValidator,           // Singleton instance
  CodeLanguage,           // Enum de linguagens
  ValidationInput,        // Interface de input
  ExtendedValidationResult // Interface de resultado
...
describe('${component.name}', () => {
  ...
  /**
 * ✅ Auto-fix de código baseado em resultados da validação CIG
 */
private autoFixCode(code: string, validation: ExtendedValidationResult): string {
  ...
  // Fix 1: Remover generics TypeScript de hooks (ERRO ORIGINAL!)
  fixed = fixed.replace(/useState<[^>]+>/g, 'useState');
  ...
  if (!fixed.includes('export default')) {
    const componentName = this.extractComponentName(fixed);
    if (componentName) {
