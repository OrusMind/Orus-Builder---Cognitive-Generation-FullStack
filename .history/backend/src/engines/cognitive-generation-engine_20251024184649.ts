...
private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
  ...
  console.log('[Pipeline] AI code length:', aiCode.length);
  console.log('[Pipeline] AI code preview:', aiCode.substring(0, 200));
  
  const components: any[] = [];
  
  // ✅ MÉTODO 1: Detecta blocos markdown com file path
  const codeBlockRegex = /``````/gi;
  let match;
  
  console.log('🔍 [Pipeline] Tentativa 1: Detectar blocos markdown...');
  ...
  while ((match = codeBlockRegex.exec(aiCode)) !== null) {
    ...
    if (code.length > 10) {
      ...
      components.push({
        ...
        type: 'component',
        code: code,
        path: filePath || `src/${componentName}.tsx`,
        files: [{
          path: filePath || `src/${componentName}.tsx`,
          content: code,
          language: 'typescript'
      ...
      console.log(`[Pipeline] ✅ Found markdown block: ${componentName} (${code.length} chars)`);
    }
  }
  
  console.log(`[Pipeline] Blocos markdown encontrados: ${components.length}`);
  
  // ✅ MÉTODO 2: Se não tem markdown, tenta detectar componentes React direto
  if (components.length === 0) {
    console.log('🔍 [Pipeline] Tentativa 2: Detectar componentes React...');
    
    // Detecta: export default function ComponentName
    const componentRegex = /(?:export\s+default\s+function\s+(\w+)|export\s+function\s+(\w+)|const\s+(\w+)\s*=\s*\(\)?\s*=>)/g;
    const matches: any[] = [];
    ...
    console.log(`[Pipeline] Componentes detectados via regex: ${matches.length}`);
    
    // Se encontrou múltiplos componentes
    if (matches.length > 1) {
      for (let i = 0; i < matches.length; i++) {
        ...
        if (code.length > 50) {
          ...
          console.log(`[Pipeline] ✅ Added component: ${matches[i].name} (${code.length} chars)`);
        }
      }
    }
    // Se encontrou apenas 1 componente
    else if (matches.length === 1) {
      components.push({
        name: matches[0].name,
        type: 'component',
        code: aiCode.trim(),
        path: `src/${matches[0].name}.tsx`,
        files: [{
          path: `src/${matches[0].name}.tsx`,
          content: aiCode.trim(),
          language: 'typescript'
      ...
      console.log(`[Pipeline] ✅ Added single component: ${matches[0].name}`);
    }
  }
  
  // ✅ FALLBACK: Usa todo o código como um componente
  if (components.length === 0) {
    console.log('[Pipeline] ⚠️ Usando fallback - código completo como App');
    
    const componentName = (
      analysis?.entities?.[0]?.value || 
      (analysis?.originalPrompt?.split(' ').filter((w: string) => w.length > 0)[0]) ||
      'App'
    ) as string;
  ...
  components.forEach((comp, idx) => {
    console.log(`[Pipeline] Component ${idx + 1}: ${comp.name} | Files: ${comp.files?.length || 0} | Code: ${comp.code.length} chars`);
  });
...
private extractComponentName(filePath: string, code: string): string {
  ...
  return nameMatch?.[1] || nameMatch?.[2] || nameMatch?.[3] || 'Component';
}
