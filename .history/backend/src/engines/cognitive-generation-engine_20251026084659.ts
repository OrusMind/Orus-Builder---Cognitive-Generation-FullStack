...
if (components.length === 0) {
  ...
  // ✅ CORRIGIDO: String literal terminada corretamente
  ...
  while ((match = format3Pattern.exec(aiCode)) !== null) {
    // ✅ CORRIGIDO: Acesso seguro ao match com nullish coalescing[1]
    const filePath = (match ?? '').trim();[1]
    let code = (match ?? '').trim();[2]

    if (code.length > 50) {
      const componentName = filePath 
        ? this.extractComponentName(filePath, code) 
        : this.extractComponentName('', code);
