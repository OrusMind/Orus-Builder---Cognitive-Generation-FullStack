...
import { 
  promptProcessor, 
  PromptProcessor 
} from '../prompt/prompt-processor';import { intentClassifier } from '../prompt/intent-classifier';
...
entities: processingResult.requirements.entities?.map((e: any) => e.name) || [],
...
console.log('[Pipeline] Intent detected:', (analysis.intent as any).intent || analysis.intent.type);
...
private async stageValidate(
  ...
  ...(validation.codeIssues?.map(issue => issue.message) || [])
];
      ...
      component.metadata.cigCompliant = validation.cigCompliant || false;
