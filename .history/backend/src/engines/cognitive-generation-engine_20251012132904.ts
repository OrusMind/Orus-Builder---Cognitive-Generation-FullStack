...
import { UIEnhancementOptions } from './types';
...
export class CognitiveGenerationEngine {
  ...
  async generate(request: GenerationRequest): Promise<EngineResult<GeneratedCode>> {
    ...
    try {
      ...
      // ═══════════════════════════════════════════════════════════════
      // ✨ STAGE 5.5: UI ENHANCEMENT (NEW!)
      // ═══════════════════════════════════════════════════════════════
      logger.info('🎨 [Stage 5.5] Enhancing UI with automatic styling...', {
        component: 'CognitiveGenerationEngine',
        metadata: { filesCount: optimizedFiles.length }
      });

      const uiOptions: UIEnhancementOptions = {
        addTailwind: true,
        addAnimations: true,
        addResponsive: true,
        style: 'modern', // Can be configured per request
        colorScheme: 'auto',
        addAccessibility: true,
        enhancementLevel: 'standard',
      };

      const uiResult = await uiEnhancementEngine.enhance(optimizedFiles, uiOptions);
      const enhancedFiles = [...uiResult.enhancedFiles, ...uiResult.configFiles];

      logger.info('✅ [Stage 5.5] UI Enhancement completed', {
        component: 'CognitiveGenerationEngine',
        metadata: {
          totalFiles: enhancedFiles.length,
          addedClasses: uiResult.statistics.addedClasses,
          addedAnimations: uiResult.statistics.addedAnimations,
          processingTime: `${uiResult.statistics.processingTime}ms`
        }
      });
      // ═══════════════════════════════════════════════════════════════
      
      // Stage 6: Validate with CIG if enabled (usar enhancedFiles agora)
      let cigScore = 100;
      ...
      // Stage 7: Calculate quality metrics (usar enhancedFiles)
      ...
      // Stage 8: Build result (usar enhancedFiles)
      const generatedCode: GeneratedCode = {
        id: request.requestId,
        requestId: request.requestId,
        generationId: this.generateGenerationId(),
        files: enhancedFiles, // ← MUDOU AQUI
        architecture: this.extractArchitecture(specification),
        dependencies: this.extractDependencies(enhancedFiles), // ← E AQUI
        qualityScore,
        confidence,
        cigScore,
        ...
        validationErrors,
        validationWarnings,
        totalLines: this.countTotalLines(enhancedFiles), // ← E AQUI
        totalFiles: enhancedFiles.length, // ← E AQUI
        estimatedComplexity: this.estimateComplexity(enhancedFiles), // ← E AQUI
      ...
      // Stage 9: Cache result if enabled
      if (this.config.enableCaching) {
        await this.cacheGeneration(request, generatedCode);
      }
      
      // Stage 10: Learn from generation
      if (this.config.enableLearning) {
        await this.learnFromGeneration(specification, generatedCode);
      }
      
      // Record metrics
      metricsAggregator.recordHistogram('generation.time', generatedCode.generationTime, {
        mode: request.mode,
        language: request.language
      });
      
      metricsAggregator.setGauge('generation.quality_score', qualityScore);
      metricsAggregator.setGauge('generation.cig_score', cigScore);
      
      logger.info('✅ Cognitive code generation completed', {
        component: 'CognitiveGenerationEngine',
        action: 'generate',
        metadata: {
          requestId: request.requestId,
          filesCount: generatedCode.totalFiles,
          linesCount: generatedCode.totalLines,
          qualityScore,
          cigScore,
          generationTime: generatedCode.generationTime
        }
      });
      ...
      logger.error('❌ Cognitive code generation failed', {
        component: 'CognitiveGenerationEngine',
        action: 'generate',
        metadata: {
          requestId: request.requestId,
      ...
      metricsAggregator.increment('generation.errors', 1, {
  ...
  /**
   * Generate With AI - REAL GROQ INTEGRATION
   */
    ...
    try {
      for (const component of specification.components) {
        ...
        const groqResponse = await groqService.generateCode({
          ...
          language: this.mapToSupportedLanguage(request.language),
          maxTokens: 4096,
          temperature: 0.3
        });

        const generatedCode = this.extractCodeFromResponse(groqResponse.content, request.language);
      ...
      logger.error('AI generation failed, falling back to template', error as Error, {
        component: 'CognitiveGenerationEngine',
        action: 'generateWithAI'
      });
...
private async learnFromGeneration(
  ...
  try {
    await learningEngine.learn({
      id: `learn-${Date.now()}`,
      source: LearningSource.CODE_GENERATION,
      data: {
        specification,
        generatedCode,
        qualityScore: generatedCode.qualityScore,
        cigScore: generatedCode.cigScore
      },
      pattern: PatternType.SUCCESS_PATTERN,
      confidence: generatedCode.confidence / 100,
      version: 1,
      isDeleted: false,
      createdAt: new Date(),
      updatedAt: new Date()
    ...
    logger.debug('Learning completed from generation', {
      component: 'CognitiveGenerationEngine',
      metadata: { generationId: generatedCode.generationId }
    });
  } catch (error) {
    logger.error('Learning from generation failed', error as Error, {
      component: 'CognitiveGenerationEngine'
    });
  }
}
