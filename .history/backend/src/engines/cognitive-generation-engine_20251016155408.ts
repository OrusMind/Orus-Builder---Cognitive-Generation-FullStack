...
/**
 * Limpa componente removendo imports, exports e comentários (versão ULTRA-SAFE)
 */
private cleanComponentCode(code: string): string {
  let cleaned = code;
  
  // FIX 1: Remove imports de bibliotecas externas (SAFE)
  ...
  // FIX 2: Remove imports de paths relativos (SAFE)
  cleaned = cleaned.replace(/import\s+.*?from\s+['"]\..*?['"]\s*;?\s*/g, '');
  
  // FIX 3: Remove APENAS comentários de linha de arquivo (MUITO ESPECÍFICO!)
  // Match: // todo.ts
  // Match: // TodoList.tsx
  // NÃO Match: código normal
  cleaned = cleaned.replace(/^\/\/\s+[a-zA-Z0-9_-]+\.(ts|tsx|js|jsx)\s*$/gm, '');
  
  // FIX 4: Remove blocos JSDoc COMPLETOS (não parciais!)
  cleaned = cleaned.replace(/\/\*\*[\s\S]*?\*\/\s*/g, '');
  
  // FIX 5: Remove comentários de seção (// STEP 1, // Import, etc)
  cleaned = cleaned.replace(/^\/\/\s+(STEP|Import|Define|TODO|FIXME|NOTE)\s+.*$/gm, '');
  
  // FIX 6: Remove export statements (SAFE - não afeta código)
  cleaned = cleaned
  ...
  // FIX 7: Remove múltiplas linhas vazias
  cleaned = cleaned.replace(/\n{3,}/g, '\n\n').trim();
  
  // FIX 8: Verificação final
...
private fixTypeAnnotations(code: string): string {
  // Fix: event.FormEvent → event: React.FormEvent
  return code
    .replace(/\((\w+)\.(\w+)/g, '($1: $2') // Fix parameter types
    .replace(/: (React\.)?FormEvent/g, ': React.FormEvent') // Ensure React. prefix
    .replace(/: (React\.)?ChangeEvent/g, ': React.ChangeEvent')
    .replace(/: (React\.)?MouseEvent/g, ': React.MouseEvent');
}
