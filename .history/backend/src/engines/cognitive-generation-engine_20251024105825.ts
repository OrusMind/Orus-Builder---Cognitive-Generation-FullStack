...
// ═══════════════════════════════════════════════════════════════
// CIG PROTOCOL & COMPONENTS IMPORTS
// ═══════════════════════════════════════════════════════════════

// 1. CIG Protocol (Orquestradora)
import {
  CIGProtocolEngine,
  type CIGProtocolConfig,
  type CIGValidationRequest,
  type CIGValidationResult,
  ValidationStatus
} from '../core/cig/cig-protocol';

// 2. Dependency Graph Intelligence
import {
  DependencyGraphIntelligenceEngine,
  type DependencyGraph,
  type CycleDetectionResult,
  type GenerationOrder
} from '../core/cig/dependency-graph-intelligence';

// 3. Progressive Type Inference
import {
  ProgressiveTypeInferenceEngine,
  type TypeInferenceRequest,
  type TypeInferenceResult,
  type InferredType
} from '../core/cig/progressive-type-inference';

// 4. Contract Evolution Tracking
import {
  ContractEvolutionTrackingEngine,
  type ChangeDetectionResult,
  type MigrationPath,
  type VersionedContract
} from '../core/cig/contract-evolution-tracking';

// 5. Type Coverage Metrics
import {
  TypeCoverageMetricsEngine,
  type CoverageAnalysisRequest,
  type CoverageAnalysisResult,
  type TypeSafetyScore
} from '../core/cig/type-coverage-metrics';

// 6. Project Context Awareness
import {
  ProjectContextAwarenessEngine,
  type ProjectAnalysisRequest,
  type ProjectAnalysisResult,
  AnalysisDepth
} from '../core/cig/project-context-awareness';

// 7. Cognitive Learning Loop
import {
  CognitiveLearningLoopEngine,
  type LearningSession,
  type FeedbackSample,
  type LearningResult
} from '../core/cig/cognitive-learning-loop';

// 8. IDE Integration Protocol (Optional)
import {
  IDEIntegrationEngine,
  type IDEIntegrationProtocol
} from '../core/cig/ide-integration-protocol';

// 9. Multi-Project Intelligence (Optional)
import {
  MultiProjectIntelligenceEngine,
  type MultiProjectKnowledge
} from '../core/cig/multi-project-intelligence';
...
/**
 * ═══════════════════════════════════════════════════════════════════════
 * ANALYZE USER PROMPT - Extract intent and requirements
 * ═══════════════════════════════════════════════════════════════════════
 */
private analyzeUserPrompt(userPrompt: string): {
  complexity: 'minimal' | 'simple' | 'moderate' | 'complex';
  components: string[];
  features: string[];
  scope: 'single-component' | 'page' | 'feature' | 'application';
} {
  const lower = userPrompt.toLowerCase();
  
  // Detect scope
  let scope: 'single-component' | 'page' | 'feature' | 'application' = 'single-component';
  
  if (
    lower.includes('just a') ||
    lower.includes('only a') ||
    lower.includes('single') ||
    lower.match(/^create an? \w+ (card|button|input|modal)/)
  ) {
    scope = 'single-component';
  } else if (
    lower.includes('page') ||
  ...
  // Detect complexity
  let complexity: 'minimal' | 'simple' | 'moderate' | 'complex' = 'simple';
  ...
  const componentKeywords = [
    'card', 'button', 'form', 'table', 'list', 'modal', 
    'navbar', 'header', 'footer', 'sidebar', 'menu',
    'dashboard', 'profile', 'settings', 'chart', 'grid'
  ];
  ...
  const featureKeywords = [
    'search', 'filter', 'sort', 'pagination', 'auth',
    'crud', 'upload', 'download', 'export', 'import',
    'real-time', 'chat', 'notifications', 'analytics'
  ];
  ...
  return { complexity, components, features, scope };
}
