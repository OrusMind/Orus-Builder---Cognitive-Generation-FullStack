...
class AlmaConnector {
  ...
  constructor() {
    // Provider will be created on demand (lazy loading)
  }
...
Extract relevant patterns, concepts, and examples from the knowledge base.
...
Focus on design patterns, best practices, and concrete code examples.
...
Analyze context and recommend the best architectural decision.
Return JSON with this EXACT structure:
...
Communicate with the user in a ${tone} tone.
Return JSON with this EXACT structure:
...
Be clear, helpful, and ${tone}. Ask clarifying questions if needed.
Return ONLY valid JSON, no explanations.`;
      ...
      logger.info('✅ Voz: Communication sent', {
        component: 'VozConnector',
        metadata: {
          responseLength: comm.message?.length || 0
        }
      });
