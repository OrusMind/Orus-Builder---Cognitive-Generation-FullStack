 ...
 */
/**
 * Limpa componente removendo imports, exports e bibliotecas indisponíveis
 */
private cleanComponentCode(code: string): string {
  let cleaned = code
    // Remove imports de bibliotecas não disponíveis no sandbox
    .replace(/import\s+.*?from\s+['"](?:recharts|axios|lodash|moment|chart\.js|victory)['"];?\s*/g, '')
    // Remove imports normais de paths
