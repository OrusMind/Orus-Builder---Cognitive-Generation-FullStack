...
async initialize(config: EngineConfig): Promise<unknown> {
  ...
  try {
    // ✅ Initialize ALL 7 sub-engines (não apenas 2!)
    await this.nlpProcessor.initialize();
    await this.intentClassifier.initialize();
    await this.entityExtractor.initialize();
    await this.requirementDecomposer.initialize();
    await this.ambiguityResolver.initialize();
    ...
    logger.error('Failed to initialize Prompt Engine', {
      component: 'PromptEngine',
      action: 'initialize',
      error: error.message
    });
