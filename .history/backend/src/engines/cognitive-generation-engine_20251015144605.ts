...
private fixTypeScriptForBabel(code: string): string {
  ...
  // Fix 3: Remove problematic type assertions
...
🚨 CRITICAL BABEL COMPATIBILITY RULES (VIOLATIONS WILL BREAK CODE):

**RULE 1: NO GENERICS IN HOOKS**
...
❌ WRONG: const [error, setError] = useState<string | null>(null);
✅ CORRECT: const [error, setError] = useState(null);

**RULE 2: TYPE INFERENCE ONLY**
Let TypeScript infer types from initial values. The in-browser Babel transformer CANNOT parse generic syntax.

**RULE 3: NO DUPLICATE DECLARATIONS**
Each interface/type must be declared ONLY ONCE.
If you need a type and a component with same name, use different names:
- Type: \`Product\` (interface)
- Component: \`ProductCard\` or \`ProductItem\` (function)

**RULE 4: MANDATORY CODE STRUCTURE**
\`\`\`typescript
// 1. IMPORTS
import React, { useState } from 'react';

// 2. TYPE DECLARATIONS (only once each!)
interface Product {
  id: number;
  name: string;
  price: number;
...
function ShoppingApp() {
  // ✅ CORRECT: No generics, let TypeScript infer from initial value
...
**CRITICAL REMINDER: DO NOT USE <TYPE> SYNTAX IN ANY useState CALL. THIS WILL CAUSE BABEL SYNTAX ERROR.**
