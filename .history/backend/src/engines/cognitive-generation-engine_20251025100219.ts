...
private async stagePrepare(request: GenerationRequest): Promise<PipelineStageResult> {
 ...
 // ═══════════════════════════════════════════════════════════════
// TRANSFORMAR ProcessingResult → PromptAnalysisResult
// ═══════════════════════════════════════════════════════════════
analysis = {
  ...
  // ✅ CORRIGIR: intent
  intent: {
    type: processingResult.classification?.primary?.intent || 'CREATE_APP',
    description: processingResult.classification?.primary?.description || 'Create application',
    confidence: processingResult.classification?.primary?.confidence || 60,
    subIntents: processingResult.classification?.secondary?.map((s: any) => ({
      intent: s.intent,
      confidence: s.confidence
    })) || []
  },
  
  // ✅ CORRIGIR: entities (verificar se existe)
  entities: processingResult.requirements?.extracted?.entities?.map((e: any) => e.name) || 
            processingResult.requirements?.entities?.map((e: any) => 
              typeof e === 'string' ? e : e.name
            ) || [],
  
  // ✅ CORRIGIR: requirements
  ...
  // ✅ CORRIGIR: context
  context: {
    domain: typeof processingResult.context?.domain === 'string' 
      ? processingResult.context.domain 
      : (processingResult.context?.domain as any)?.name || 'general',
    
    complexity: processingResult.context?.complexity || 
...
console.log('[Pipeline] Intent detected:', analysis.intent.type);
