...
if (parsedFiles.length > 1) {
  ...
  logger.warn(`⚠️ Multi-file detected (${parsedFiles.length} files), merging into single file...`, {
    component: 'CognitiveGenerationEngine',
    metadata: { files: parsedFiles.map(f => f.name) }
  });
  
  // ✅ MERGE TUDO EM 1 ARQUIVO (função agora existe!)
  const mergedComponent = this.mergeToSingleFile(parsedFiles);
  
  logger.info(`✅ Successfully merged ${parsedFiles.length} files into ${mergedComponent.name}`, {
    component: 'CognitiveGenerationEngine'
  });
  ...
  // ✅ Single-file já → retorna direto
  logger.info(`✅ Single file detected: ${parsedFiles[0].name}`, {
    component: 'CognitiveGenerationEngine'
  });
...
// ✅ Fallback: código não foi parseado, retornar código original
...
private mergeToSingleFile(parsedFiles: GeneratedComponent[]): GeneratedComponent {
  ...
  parsedFiles.forEach(file => {
    ...
    helpers.forEach(helper => {
      // Remover 'export' das classes/funções
  ...
  parsedFiles.forEach(file => {
    if (file.path.endsWith('.tsx')) {
      ...
      if (cleanComp) {
        ...
        // Extrair dependencies
        if (file.dependencies) {
          file.dependencies.forEach(dep => allDependencies.add(dep));
        }
        
        // Detectar componente principal (geralmente App ou o último)
  ...
  // PASSO 4: Export default do componente principal
  mergedCode += `export default ${mainComponentName};\n`;

  // PASSO 5: Retornar componente merged
  const firstFile = parsedFiles[0];
  return {
    id: firstFile.id,
...
private cleanComponentCode(code: string): string {
  let cleaned = code
    ...
    // Remove comentários de arquivo
    .replace(/\/\/\s+[\w\/]+\.(ts|tsx)\s*\n/g, '')
    // Remove comentários de documentação do topo
    .replace(/^\/\*\*[\s\S]*?\*\/\s*/g, '')
