...
if (parsedFiles.length > 0) {
  // Código contém múltiplos arquivos, retornar o primeiro
  // Os outros serão adicionados no generateComponents()
...
// Código único, retornar normalmente
return {
...
private parseMultiFileCode(code: string, componentSpec: any): GeneratedComponent[] {
  ...
  if (matches.length === 0) {
    // Código único, retorna como está
    return [];
  }
...
✅ Apply color palette: ${JSON.stringify(context.colorPalette || {})}
✅ Match personality: ${context.personality || 'professional'}
✅ Use TypeScript with strict typing
✅ Follow ${framework} best practices
✅ Include proper error handling
✅ Ensure accessibility (aria-labels, semantic HTML)
✅ Make it responsive (mobile-first with Tailwind)
