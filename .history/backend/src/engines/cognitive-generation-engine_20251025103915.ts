  ...
  try {
    ...
    if (scope.type === ScopeType.BACKEND) {
      ...
      try {
        ...
        generatedCode = backendResult.code || backendResult;
      ...
      try {
        const fullstackResult = await codeGenerator.generate({
          ...
          requirements: {
            entities: analysis.entities || [],
            features: analysis.requirements || [],
            includeAuth: analysis.originalPrompt.toLowerCase().includes('auth'),
            includePrisma: analysis.originalPrompt.toLowerCase().includes('database') || 
                           analysis.originalPrompt.toLowerCase().includes('prisma')
          },
          options: {
            frontend: {
              framework: 'react',
        ...
        generatedCode = fullstackResult.code || fullstackResult;
      ...
      // ✅ OUTROS SCOPES (SINGLE_COMPONENT, PAGE, FEATURE, LANDING_PAGE) → AI Provider
