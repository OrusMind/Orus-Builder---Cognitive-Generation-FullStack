...
{
  "message": "Main message to the user",
...
Return ONLY valid JSON, no explanations.`;
    ...
    // ✅ CORRETO: Usar 'message' ao invés de 'response'
    return {
      message: comm.message || 'Message processed successfully',
...
async orchestrate(request: TrinityRequest): Promise<TrinityResult> {
  ...
  try {
    logger.info('🔱 Trinity: Orchestrating AI-powered generation', {
      component: 'TrinityOrchestrator'
    });

    // ALMA: Knowledge retrieval
    const almaResult = await this.alma.retrieveKnowledge(
      request.prompt,
      request.context
    );

    // CEREBRO: Decision making (✅ CORRETO - 2 parâmetros)
    const cerebroResult = await this.cerebro.makeDecision(
      request.context || {},
      [request.prompt]
    );

    // VOZ: Communication (✅ CORRETO - passar apenas message)
    ...
    logger.info('✅ Trinity: Orchestration complete', {
      ...
      metadata: {
        processingTime,
        confidence: (almaResult.confidence + cerebroResult.confidence) / 2
      }
    });
