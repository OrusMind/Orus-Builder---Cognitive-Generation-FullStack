  ...
  public async generate(request: GenerationRequest): Promise<GenerationResult> {
    ...
    try {
      ...
      return {
        ...
        // ✅ Adicionar propriedades esperadas pelo controller
        data: {
          ...
          generationTime: processingTime, // ✅ ADICIONAR
      ...
      return {
        ...
        packageJson: '',
        readme: '',
        // ✅ Também retornar estrutura vazia em caso de erro
        data: {
          components: [],
          files: [],
          structure: {}
          ...
          generationTime: Date.now() - startTime, // ✅ ADICIONAR
          linesOfCode: 0,
          complexity: 0
        },
        qualityReport: {
...
const qualityScore = components.reduce((sum: number, c: GeneratedComponent) =>        sum + (c.metadata.qualityScore || 0), 0
 ...
 /**
 * Stage 2: Generate
 * - Use AI + specialized generators to create MULTIPLE components
 */
private async stageGenerate(prepareData: any): Promise<PipelineStageResult> {
  logger.info('Pipeline Stage 2: Generate (ENHANCED)', { component: 'GenerationPipeline' });
  
  try {
    const { analysis, specification, templates } = prepareData;
    const components: GeneratedComponent[] = [];
    
    // ✅ 1. GERAR COMPONENTES UI (React)
    logger.info('Generating UI components...');
    try {
      ...
      // UIGenerator v2.0 retorna .files[] array
      if (uiResult.files && uiResult.files.length > 0) {
        uiResult.files.forEach((file: any) => {
          components.push({
        ...
        logger.info(`✅ Generated ${uiResult.files.length} UI files`);
      }
    } catch (uiError) {
      logger.warn('UI generation failed', { error: (uiError as Error).message });
    }
    
    // ✅ 2. GERAR APIS BACKEND (Express)
    logger.info('Generating backend APIs...');
    try {
      ...
      // BackendGenerator retorna .files[] array
      if (backendResult.files && backendResult.files.length > 0) {
        backendResult.files.forEach((file: any) => {
          components.push({
        ...
        logger.info(`✅ Generated ${backendResult.files.length} backend files`);
      }
    } catch (backendError) {
      logger.warn('Backend generation failed', { error: (backendError as Error).message });
    }
    
    // ✅ 3. SE AINDA NÃO GEROU NADA, USAR FALLBACK
    if (components.length === 0) {
      logger.warn('No components generated, using fallback');
      
      // Gerar componente básico
      components.push({
        name: 'App',
        type: 'component',
        code: this.generateFallbackComponent(analysis.originalPrompt),
        path: 'src/App.tsx',
        language: 'typescript',
        framework: 'react',
        dependencies: ['react'],
