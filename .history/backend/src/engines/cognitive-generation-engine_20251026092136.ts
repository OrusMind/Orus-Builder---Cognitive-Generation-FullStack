 ...
 * 
 * PRIORITY ORDER:
 * 0. USER_OVERRIDE (manual complexity selection)
 * 1. SINGLE_COMPONENT
 * 2. FULLSTACK
 * 3. BACKEND
 * 4. LANDING_PAGE
...
private detectScopeFromIntent(
  ...
  // ═══════════════════════════════════════════════════════════════
  if (userComplexity) {
    console.log(`🎚️ [Scope] USER OVERRIDE: ${userComplexity}`);
    detectedKeywords.push(`user_override_${userComplexity}`);
    
    // ✅ TIPAGEM CORRETA: usar 'simple' | 'moderate' | 'high' | 'very_high'
    const complexityMap: Record<string, ScopeDetectionResult> = {
      'minimal': {
        type: ScopeType.SINGLE_COMPONENT,
        complexity: 'simple' as ComplexityLevel, // ✅ CAST EXPLÍCITO
        confidence: 1.0,
      ...
      'standard': {
        type: ScopeType.SINGLE_COMPONENT,
        complexity: 'moderate' as ComplexityLevel, // ✅ CAST EXPLÍCITO
        confidence: 1.0,
      ...
      'feature_rich': {
        ...
        complexity: 'high' as ComplexityLevel, // ✅ CAST EXPLÍCITO
        confidence: 1.0,
      ...
      'enterprise': {
        ...
        complexity: 'very_high' as ComplexityLevel, // ✅ CAST EXPLÍCITO
        confidence: 1.0,
    ...
    return complexityMap[userComplexity];
  }
  ...
  const fullstackKeywords = [
    ...
    'aplicação completa', 'aplicacao completa',
    'frontend e backend', 'frontend + backend',
    'react e express', 'react + express',
    'complete application',
    'sistema completo'
  ];

  const hasFullstack = fullstackKeywords.some(keyword => promptLower.includes(keyword));
  const hasFrontend = promptLower.includes('frontend') || promptLower.includes('react');
  const hasBackend = promptLower.includes('backend') || promptLower.includes('express') || promptLower.includes('api');
  const hasDatabase = promptLower.includes('database') || promptLower.includes('prisma') || promptLower.includes('mongodb');
  ...
  const backendKeywords = [
    'api rest', 'restful api', 'backend', 'express',
    'endpoints', 'routes', 'controllers', 'middleware'
  ];

  const hasBackendKeyword = backendKeywords.some(keyword => promptLower.includes(keyword));
  ...
  const landingPageKeywords = [
    'landing page', 'landing-page', 'página de destino',
    'hero section', 'call to action', 'cta', 'testimonials'
  ];

  const hasLandingPageKeyword = landingPageKeywords.some(keyword => promptLower.includes(keyword));
