...
private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
  ...
  // Regex para: ```
  const codeBlockPattern = '```component:([^:]+):([^:]+):([^\\n]+)\\n([\\s\\S]*?)```
  ...
  // ✅ FORMATO 2: ```typescript \n // component:Name.tsx:tsx:path
  if (components.length === 0) {
    ...
    while ((match = format2Regex.exec(aiCode)) !== null) {
      const fileName = match[1] ? match[1].trim() : '';
  ...
  // ✅ FORMATO 4: Detectar múltiplos componentes React sem markdown
  if (components.length === 0) {
    console.log('🔍 [Pipeline] Tentativa 4: Detectar componentes React direto');
    
    const componentPattern = '(?:export\\s+default\\s+function\\s+(\\w+)|export\\s+function\\s+(\\w+)|const\\s+(\\w+)\\s*:\\s*React\\.FC)';
    ...
    while ((match = componentRegex.exec(aiCode)) !== null) {
      const name = match[1] || match || match;
  ...
  // ✅ FALLBACK: Código completo como um componente
  if (components.length === 0) {
    console.log('[Pipeline] ⚠️ Usando fallback - código completo');
    
    let componentName = 'App';
    
    if (analysis?.entities && analysis.entities && analysis.entities.value) {
      componentName = analysis.entities.value;
    } else if (analysis?.originalPrompt) {
...
private extractComponentName(filePath: string, code: string): string {
  ...
  const nameMatch = code.match(/(?:export\s+default\s+(?:function|const)\s+(\w+)|export\s+(?:function|const)\s+(\w+)|const\s+(\w+)\s*:\s*React\.FC|class\s+(\w+))/);
  
  if (nameMatch) {
    return nameMatch[1] || nameMatch || nameMatch || nameMatch || 'Component';
  }
