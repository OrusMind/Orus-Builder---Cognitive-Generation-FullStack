...
const aiAnalysis = await this.aiProvider.analyze(enhancedPrompt, 'intent');
...
private enhancePromptForProfessionalOutput(
  originalPrompt: string, 
  context: Partial<Context>  // ← MUDOU AQUI (de Context para Partial<Context>)
): string {
  // ✅ Se já é um prompt detalhado (>300 chars), não modificar
  ...
  // ✅ 1. Design System (safe access com ?.)
  ...
  // ✅ 8. Domain-specific enhancements (safe access com ?.)
  const domain = context.domain || 'GENERAL';
  
  if (domain === 'E_COMMERCE' || domain === 'ECOMMERCE') {
    enhancements.push('\n**E-commerce Specific:**');
