...
private mergeToSingleFile(parsedFiles: GeneratedComponent[]): GeneratedComponent {
  ...
  // PASSO 1: Extrair todas as interfaces
...
/**
 * Extrai interfaces do código (melhorado)
 */
private extractInterfacesFromCode(code: string): string[] {
  const interfaces: string[] = [];
  
  // Regex melhorada para capturar interface completa
  const interfacePattern = /(?:export\s+)?interface\s+\w+\s*\{[^}]*\}/gs;
  const matches = code.match(interfacePattern);
  
  if (matches) {
    matches.forEach(match => {
      // Remove 'export' da interface
      const cleanInterface = match.replace(/export\s+/g, '');
...
/**
 * Extrai classes e funções helper do código
 */
private extractHelpersFromCode(code: string): string[] {
  ...
  helpers.push(...classes);
  
  // Funções (fora de componentes React)
  const functionPattern = /(?:export\s+)?function\s+(?!(?:App|Component)\b)\w+\([^)]*\)[^{]*\{[\s\S]*?\n\}/g;
  const functions = code.match(functionPattern) || [];
  helpers.push(...functions);
  
  return helpers;
}

/**
 * Limpa componente removendo imports e exports
 */
/**
 * Limpa componente removendo imports, exports e comentários
 */
private cleanComponentCode(code: string): string {
  let cleaned = code
    // Remove imports
    .replace(/import\s+.*?from\s+['"].*?['"]\s*;?\s*/g, '')
    // Remove comentários de arquivo (// models/Post.ts)
    .replace(/\/\/\s+[\w\/]+\.(ts|tsx)\s*\n/g, '')
    // Remove comentários JSDoc do topo
    .replace(/^\/\*\*[\s\S]*?\*\/\s*/g, '')
    // Remove export statements (export interface, export const, etc)
    .replace(/export\s+interface\s+\w+[\s\S]*?\n\}/g, '') // Remove export interface completo
    .replace(/export\s+type\s+\w+[\s\S]*?;/g, '') // Remove export type
    .replace(/export\s+const\s+/g, 'const ') // Remove export de const
    .replace(/export\s+function\s+/g, 'function ') // Remove export de function
    .replace(/export\s+class\s+/g, 'class ') // Remove export de class
    .replace(/export\s+default\s+\w+\s*;?\s*/g, '') // Remove export default
    .replace(/export\s+\{[^}]*\}\s*;?\s*/g, '') // Remove export { ... }
    .replace(/export\s*;?\s*/g, '') // Remove export órfão (FIX CRÍTICO!)
