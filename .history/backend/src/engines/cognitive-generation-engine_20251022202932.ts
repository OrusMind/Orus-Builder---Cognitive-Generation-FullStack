...
private async generateComponent(
  ...
  // ✅ SYSTEM PROMPT v2.0 COM INSTRUÇÕES MULTI-FILE
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 CRITICAL: MULTI-FILE FORMAT REQUIREMENT (MANDATORY)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

YOU MUST generate code in MULTI-FILE format using this EXACT pattern:

// src/ComponentName.tsx
[TypeScript/React code here]
...
MANDATORY RULES:
✅ Start EVERY file block with: // src/path/filename.ext
✅ Use full path with "src/" prefix (e.g., src/components/Button.tsx)
✅ One blank line after marker, then code
✅ NO markdown code blocks (no \`\`\`typescript or \`\`\`css)
✅ NO explanations or comments before/after code
✅ Separate each file with 2 blank lines
...
⚠️ FAILURE TO USE THIS FORMAT WILL BREAK THE BUILD SYSTEM!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Return ONLY code with file markers, nothing else. No explanations.`
    },
    {
      role: 'user',
      content: prompt
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ✅ ETAPA 2: SE MULTI-FILE → RETORNAR TODOS OS ARQUIVOS!
  // ═══════════════════════════════════════════════════════════════════════
  if (parsedFiles.length > 1) {
    ...
    console.log('\n✅ ═══════════════════════════════════════════════════════════');
    console.log('✅ RETURNING MULTIPLE FILES TO PARENT:');
    console.log('✅ Total files:', parsedFiles.length);
    parsedFiles.forEach((f, idx) => {
      console.log(`   [${idx + 1}] ${f.path} (${f.code.split('\n').length} lines)`);
    });
    console.log('✅ ═══════════════════════════════════════════════════════════\n');
    
    const mainFile = parsedFiles[0]!;
    
    // ✅ Salvar TODOS os arquivos no metadata (backup)
    (mainFile.metadata as any).allFiles = parsedFiles;
    (mainFile.metadata as any).isMultiFile = true;
    
    // ✅ CRÍTICO: Salvar arquivos extras em _extraFiles para generateComponents() pegar
    (this as any)._extraFiles = parsedFiles.slice(1);
    
    logger.info(`📦 Stored ${parsedFiles.length - 1} extra files in _extraFiles`, {
      component: 'CognitiveGenerationEngine',
      metadata: { extraFilesCount: parsedFiles.length - 1 }
    });
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ✅ FALLBACK: Nenhum marcador detectado
  // ═══════════════════════════════════════════════════════════════════════
...
private parseMultiFileCode(code: string, componentSpec: any): GeneratedComponent[] {
  ...
  // ✅ REGEX v5.0: Detecta qualquer "// path/to/file.ext" (com ou sem src/)
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    // ✅ NORMALIZAR PATH: Se não começar com "src/", adicionar!
  ...
  sections.forEach((section) => {
    ...
    if (filename.includes('App.tsx') || filename.includes('index.tsx')) {
      ...
      type = 'model';
    } else if (section.path.includes('/context/') || section.path.includes('/hooks/')) {
      type = 'service';
    } else if (section.path.includes('/utils/')) {
      type = 'util';
...
export function UserCard({ user }: { user: User }) {
  return <div className="bg-white p-4 ${radius.md} ${shadows.md}">{user.name}</div>;
}
...
**CRITICAL RULES:**
✅ Start EVERY file with: // src/frontend/path/filename.ext
✅ NO code blocks (no \`\`\`typescript)
✅ Generate 3-8 files minimum
✅ Use provided design tokens
✅ Export interfaces from type files
✅ Import between files using relative paths
...
1. **Types** (src/frontend/types/*.ts)
   - Define all interfaces and types
   - Export everything
   - Use TypeScript strict mode

2. **Components** (src/frontend/components/*.tsx)
   - Functional React components
   - Import types from ../types
   - Use design tokens
   - Include props interface

3. **Utils/Helpers** (src/frontend/utils/*.ts)
   - Pure functions
   - Data formatters
   - Validators

4. **Main App** (src/frontend/App.tsx)
   - Root component
   - Import all sub-components
   - Setup state management
   - Mock data

5. **Constants** (src/frontend/constants/*.ts)
   - Mock data arrays
   - Configuration
   - Enums
...
1. Create 3-8 files in src/frontend/ structure
...
**OUTPUT FORMAT:**
Start each file with: // src/frontend/path/filename.ext
NO markdown blocks, NO explanations!

**CHECKLIST:**
✅ All files start with // src/frontend/
