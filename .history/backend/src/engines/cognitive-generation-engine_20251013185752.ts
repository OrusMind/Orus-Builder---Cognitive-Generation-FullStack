...
/**
 * Gets darker shade for hover states
 */
private getDarkerShade(tailwindClass: string): string {
  ...
  return shadeMap[tailwindClass] || tailwindClass.replace('-500', '-600');
}
...
const code = codeResponse.content;
 ...
 private buildComponentPrompt(
  ...
  const primaryTailwindColor = this.mapColorToTailwind(colors[0]);
  const secondaryTailwindColor = this.mapColorToTailwind(colors[1]);
...
ARCHITECTURE:
- Style: ${specification?.architecture?.style || 'component-based'}
- Patterns: ${specification?.architecture?.patterns?.join(', ') || 'functional'}
