...
class AlmaConnector {
  ...
  constructor() {
    // Provider criado sob demanda (lazy loading)
  }
...
{
  "knowledge": [
    { "concept": "Component Architecture", "description": "...", "relevance": 95 }
  ],
  "patterns": [
    { "name": "Factory Pattern", "applicability": "...", "examples": ["..."] }
  ],
  "examples": [
    { "code": "const x = ...", "explanation": "..." }
  ],
  "confidence": 85
}
...
Return ONLY valid JSON, no explanations.`;
      ...
      logger.info('✅ Alma: Knowledge retrieved', {
        component: 'AlmaConnector',
        metadata: {
          knowledgeCount: knowledge.knowledge?.length || 0,
          patternsCount: knowledge.patterns?.length || 0,
          examplesCount: knowledge.examples?.length || 0,
...
Your job: Analyze context and recommend the best architectural decision.
Return JSON with this EXACT structure:
...
Return ONLY valid JSON, no explanations.`;
    ...
    const response = await this.getProvider().chat([
      ...
      { 
        role: 'user', 
        content: `Context: ${JSON.stringify(context)}\n\nOptions: ${options.join(', ')}\n\nWhat's the best architectural decision?` 
      }
    ], {
      temperature: 0.3,
      maxTokens: 1500
    ...
    // ✅ CORRETO: Mapear para CerebroResult
    return {
      architecture: {
        style: decision.architecture?.style || 'Layered',
        layers: decision.architecture?.layers || ['Presentation', 'Business', 'Data'],
    ...
    return {
      ...
      reasoning: [{
        decision: options[0] || 'default',
        justification: 'Fallback decision due to error',
        tradeoffs: ['Limited analysis due to error']
      }],
      confidence: 50
    };
  }
...
Your job: Communicate with the user in a ${tone} tone.
Return JSON with this EXACT structure:

{
  "response": "Main message to the user",
...
Return ONLY valid JSON, no explanations.`;
      ...
      logger.info('✅ Voz: Communication sent', {
        component: 'VozConnector',
        metadata: {
          responseLength: comm.response?.length || 0,
          clarificationsCount: comm.clarifications?.length || 0,
          suggestionsCount: comm.suggestions?.length || 0
        }
      });

      return {
        response: comm.response || 'Message processed successfully',
        clarifications: comm.clarifications || [],
      ...
      return {
        response: 'Communication system temporarily unavailable. Your request has been received.',
        clarifications: [],
        suggestions: ['Try again in a moment', 'Check your input format'],
        tone: tone
      };
    }
  }
