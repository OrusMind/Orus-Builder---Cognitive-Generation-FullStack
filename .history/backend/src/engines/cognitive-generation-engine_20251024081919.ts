...
import { TemplateType } from '../engines/template-engine';
...
/**
 * ═════════════════════════════════════════════════════════════════════════
 * GENERATE WITH TEMPLATE SYSTEM (NEW!)
 * ═════════════════════════════════════════════════════════════════════════
 */
private async generateWithTemplates(
  ...
  console.log('🎨 Using Template System for generation');
  ...
  const complexity = this.detectComplexity(context);
  
  console.log('  Detected:', { domain, personality, complexity });
  
  // ═══════════════════════════════════════════════════════════════
  // STEP 2: Search for matching templates
  // ═══════════════════════════════════════════════════════════════
  const templateSearch = await templateManager.searchTemplates({
    keyword: context,
    category: this.mapComplexityToCategory(complexity),
    framework: 'React' as any,
    limit: 1
  });
  
  if (templateSearch.templates.length > 0) {
    console.log('  ✅ Found matching template:', templateSearch.templates[0].name);
    
    // Use template
    const template = templateSearch.templates[0];
    return await this.renderFromTemplate(template, componentSpec, designTokens, {
      domain,
      personality,
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 3: No template found → Generate with AI + context
  // ═══════════════════════════════════════════════════════════════
  console.log('  ℹ️ No template found, generating with context-aware AI');
  ...
  const rendered = await templateEngine.render({
    ...
    context: {
      ...
      complexity: complexity as any,
      colorPalette: [
        designTokens.primaryColor || '#007bff',
        designTokens.secondaryColor || '#6c757d'
      ],
      stylePreferences: designTokens.theme || 'modern'
    },
    formatCode: true,
    validateOutput: true,
    version: 1,
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 4: Convert rendered template to GeneratedComponent format
  // ═══════════════════════════════════════════════════════════════
  return [{
    id: componentSpec.name,
    name: componentSpec.name,
    type: 'page' as const,
    path: `${componentSpec.name}.tsx`,
    code: rendered.data.content,
    files: [],
    dependencies: [],
    metadata: {
      linesOfCode: rendered.data.content.split('\n').length,
      complexity: 1,
      coverage: 0
    }
  }];
...
private detectDomain(prompt: string): string {
  ...
  if (lower.includes('fitness') || lower.includes('workout') || lower.includes('exercise')) {
    return 'fitness';
  }
...
/**
 * Detect personality from prompt
 */
private detectPersonality(prompt: string): string {
  const lower = prompt.toLowerCase();
  
  if (lower.includes('motivational') || lower.includes('energetic') || lower.includes('inspiring')) {
    return 'motivational';
  }
 ...
 */
private detectComplexity(prompt: string): 'minimal' | 'simple' | 'moderate' | 'complex' {
  const lower = prompt.toLowerCase();
  
  // Minimal: single component requests
  if (
    lower.includes('just a') ||
    lower.includes('only a') ||
    lower.includes('single') ||
    lower.match(/^create an? \w+ card$/) ||
    lower.match(/^create an? \w+ button$/)
  ) {
    return 'minimal';
  }
  ...
  // Moderate: multi-page or feature-rich
  if (
...
/**
 * ═════════════════════════════════════════════════════════════════════════
 * GENERATE COMPONENT - WITH FULL TEMPLATE SYSTEM INTEGRATION v2.0
 * ═════════════════════════════════════════════════════════════════════════
 * Now uses template-manager, component-library, style-generator, etc.
 */
private async generateComponent(
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log('[COGNITIVE ENGINE v2.0] Starting TEMPLATE-BASED generation');
  console.log('[COGNITIVE ENGINE] Component:', componentSpec.name);
  console.log('[COGNITIVE ENGINE] Type:', componentSpec.type);
  console.log('═══════════════════════════════════════════════════════');
  
  // ═════════════════════════════════════════════════════════════════
  // STEP 1: Detect domain, personality, complexity from request
  // ═════════════════════════════════════════════════════════════════
  const domain = request.context?.domain || this.detectDomain(request.prompt);
  const personality = request.context?.personality || this.detectPersonality(request.prompt);
  const complexity = request.context?.complexity || this.detectComplexity(request.prompt);
  
  console.log('[COGNITIVE ENGINE] Context:', { domain, personality, complexity });
  
  // ═════════════════════════════════════════════════════════════════
  // STEP 2: Search for matching template using template-manager
  // ═════════════════════════════════════════════════════════════════
  let templateCode: string | null = null;
  
  try {
    const { templateManager } = await import('../templates/template-manager');
    
    const searchResult = await templateManager.searchTemplates({
      keyword: componentSpec.name,
      category: this.mapComplexityToCategory(complexity),
      framework: (request.framework || 'react') as any,
      limit: 1
    });
    
    if (searchResult.templates.length > 0) {
      const template = searchResult.templates[0];
      console.log(`[COGNITIVE ENGINE] ✅ Found template: ${template!.name}`);
      
      // ✅ Use template-customizer to customize
      const { templateCustomizer } = await import('../templates/template-customizer');
      
      const customized = await templateCustomizer.customize(
        template!.content,
        {
          variables: {
            componentName: componentSpec.name,
            primaryColor: request.designTokens?.primaryColor || '#007bff',
            secondaryColor: request.designTokens?.secondaryColor || '#6c757d',
            ...request.designTokens
          },
          slots: {
            content: componentSpec.purpose
          },
          theme: request.designTokens || {}
        }
      );
      
      if (customized.success && customized.data) {
        templateCode = customized.data.content;
        console.log('[COGNITIVE ENGINE] ✅ Template customized successfully');
      }
    } else {
      console.log('[COGNITIVE ENGINE] ℹ️ No template found, will use AI generation');
    }
  } catch (error) {
    console.warn('[COGNITIVE ENGINE] ⚠️ Template system unavailable, using AI:', error);
  }
  
  // ═════════════════════════════════════════════════════════════════
  // STEP 3: If no template, try component-library for pre-built components
  // ═════════════════════════════════════════════════════════════════
  if (!templateCode) {
    try {
      const { componentLibrary } = await import('../templates/component-library');
      
      const component = await componentLibrary.getComponent({
        name: componentSpec.name,
        category: this.mapTypeToCategory(componentSpec.type),
        framework: (request.framework || 'react') as any
      });
      
      if (component) {
        templateCode = component.code;
        console.log('[COGNITIVE ENGINE] ✅ Found pre-built component in library');
      }
    } catch (error) {
      console.warn('[COGNITIVE ENGINE] ⚠️ Component library unavailable:', error);
    }
  }
  
  // ═════════════════════════════════════════════════════════════════
  // STEP 4: If still no code, generate with AI (but context-aware!)
  // ═════════════════════════════════════════════════════════════════
  let code: string;
  
  if (templateCode) {
    code = templateCode;
    console.log('[COGNITIVE ENGINE] ✅ Using template-based code');
  } else {
    console.log('[COGNITIVE ENGINE] 🤖 Generating with AI (context-aware)...');
    
    // ✅ Build CONTEXT-AWARE prompt (not generic!)
    const prompt = this.buildContextAwarePrompt(
      componentSpec,
      specification,
      request,
      { domain, personality, complexity }
    );
    ...
    try {
      const codeResponse = await this.getProvider().chat([
        {
          role: 'system',
          content: `You are an expert ${request.framework || 'React'}/TypeScript code generator. 
Generate clean, production-ready code optimized for ${domain} domain with ${personality} personality.
CRITICAL: Return ONLY code with file markers like: // src/ComponentName.tsx`
        },
        {
          role: 'user',
          content: prompt
      ...
      console.log('[COGNITIVE ENGINE] ✅ AI generation completed');
  ...
  // ═════════════════════════════════════════════════════════════════
  // STEP 5: Apply style-generator for consistent styling
  // ═════════════════════════════════════════════════════════════════
  try {
    const { styleGenerator } = await import('../templates/style-generator');
    
    const styled = await styleGenerator.generate({
      id: crypto.randomUUID(),
      requestId: request.projectId,
      component: componentSpec.name,
      type: 'tailwind' as any,
      theme: {
        colors: {
          primary: request.designTokens?.primaryColor || '#007bff',
          secondary: request.designTokens?.secondaryColor || '#6c757d'
        },
        spacing: 'standard' as any,
    ...
    if (styled.success && styled.data) {
      console.log('[COGNITIVE ENGINE] ✅ Styles generated and applied');
      // Merge styles into code (se necessário)
    }
  } catch (error) {
    console.warn('[COGNITIVE ENGINE] ⚠️ Style generator unavailable:', error);
  }
  
  // ═════════════════════════════════════════════════════════════════
  // STEP 6: Parse multi-file code (já existente)
  // ═════════════════════════════════════════════════════════════════
  console.log('\n[DEBUG] Parsing generated code...');
  ...
  // ═════════════════════════════════════════════════════════════════
  // STEP 7: Generate tests (já existente)
  // ═════════════════════════════════════════════════════════════════
  ...
  // ═════════════════════════════════════════════════════════════════
  // STEP 8: Extract dependencies and calculate metadata
  // ═════════════════════════════════════════════════════════════════
  ...
  // ═════════════════════════════════════════════════════════════════
  // STEP 9: Return appropriate format (multi-file or single)
  // ═════════════════════════════════════════════════════════════════
...
/**
 * ✅ NEW: Build context-aware prompt (not generic!)
 */
private buildContextAwarePrompt(
  componentSpec: { name: string; type: string; purpose: string; responsibilities: string[] },
  specification: TechnicalSpecification,
  request: GenerationRequest,
  context: { domain: string; personality: string; complexity: string }
): string {
  
  const { domain, personality, complexity } = context;
  
  // ✅ ADAPTIVE file count based on complexity
  const fileCountMap = {
    'minimal': '1-3 files',
    'simple': '3-6 files',
    'moderate': '6-12 files',
  ...
  const fileCount = fileCountMap[complexity as keyof typeof fileCountMap] || '3-8 files';
  
  return `
Generate a production-ready ${request.framework || 'React'} + TypeScript component for:

**COMPONENT SPECIFICATION:**
- Name: ${componentSpec.name}
- Type: ${componentSpec.type}
- Purpose: ${componentSpec.purpose}
- Responsibilities: ${componentSpec.responsibilities.join(', ')}

**CONTEXT (CRITICAL - ADAPT CODE TO THIS!):**
- Domain: ${domain} (adjust features and terminology accordingly)
- Personality: ${personality} (adjust tone, copy, and interactions)
- Complexity: ${complexity} (generate ${fileCount})

**USER REQUEST:**
"${request.prompt}"

**DESIGN TOKENS:**
- Primary Color: ${request.designTokens?.primaryColor || '#007bff'}
- Secondary Color: ${request.designTokens?.secondaryColor || '#6c757d'}
- Theme: ${request.designTokens?.theme || 'modern'}
- Font: ${request.designTokens?.fontFamily || 'Inter, sans-serif'}

**MANDATORY REQUIREMENTS:**
${complexity === 'minimal' ? `
1. Generate ONLY ${fileCount} (keep it simple!)
2. Single main component + types
3. Minimal mock data (3-5 items)
4. NO unnecessary features
` : complexity === 'simple' ? `
1. Generate ${fileCount}
2. Main component + 1-2 supporting components
3. Basic mock data
4. Add search/filter ONLY if user mentioned it
` : complexity === 'moderate' ? `
1. Generate ${fileCount}
2. Multiple components with clear separation
3. Full mock data set
4. Interactive features as requested
5. Proper state management
` : `
1. Generate ${fileCount}
2. Complete application structure
3. Comprehensive mock data
4. All features requested + best practices
...
✅ Add ${personality} personality to copy/UI
✅ Optimize for ${domain} domain
✅ NO markdown code blocks
✅ Just raw code with file markers

Generate the complete code now, tailored to the ${domain} domain with ${personality} personality:
`;
}

/**
 * ✅ NEW: Map complexity to template category
 */
private mapComplexityToCategory(complexity: string): string {
...
/**
 * ✅ NEW: Map component type to library category
 */
private mapTypeToCategory(type: string): string {
  ...
  return map[type.toLowerCase()] || 'components';
}

/**
 * ✅ NEW: Detect domain from prompt
 */
private detectDomain(prompt: string): string {
  const lower = prompt.toLowerCase();
...
/**
 * ✅ NEW: Detect personality from prompt
 */
private detectPersonality(prompt: string): string {
  const lower = prompt.toLowerCase();
  
  if (lower.includes('motivational') || lower.includes('energetic')) return 'motivational';
  if (lower.includes('professional') || lower.includes('business')) return 'professional';
  if (lower.includes('friendly') || lower.includes('casual')) return 'friendly';
...
/**
 * ✅ NEW: Detect complexity from prompt
 */
private detectComplexity(prompt: string): string {
  const lower = prompt.toLowerCase();
  
  // Minimal
  if (lower.match(/^create an? \w+ (card|button)$/)) return 'minimal';
  if (lower.includes('just') || lower.includes('only') || lower.includes('single')) return 'minimal';
  
  // Complex
  if (lower.includes('complete') || lower.includes('full-stack')) return 'complex';
  if (lower.includes('application') || lower.includes('system')) return 'complex';
  
  // Moderate
  if (lower.includes('dashboard') || lower.includes('multiple')) return 'moderate';
  if (lower.includes('features') || lower.includes('crud')) return 'moderate';
