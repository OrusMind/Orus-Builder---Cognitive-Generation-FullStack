...
private parseMultiFileCode(code: string, componentSpec: any): GeneratedComponent[] {
  ...
  // ✅ CRIAR UM ÚNICO GeneratedComponent COM TODOS OS ARQUIVOS DENTRO:
  ...
  // ✅ RETORNAR UM ÚNICO COMPONENTE COM .files:
  components.push({
    id: componentSpec.name || 'App',
    name: componentSpec.name || 'App',
    type: 'page' as const,
    path: `${componentSpec.name || 'App'}.tsx`,
    code: '', // Vazio porque os arquivos estão em .files
    files: files,  // ✅ TODOS OS ARQUIVOS AQUI!
    dependencies: [],
    metadata: {
