...
code = uiResult.component; // UIGenerationResult retorna .component, não .code
...
code = backendResult.files[0]?.content || ''; 

        } else {
          // Use generic Code Generator - RETORNA OBJETO
       const codeResult = await codeGenerator.generate({
  prompt: `Generate ${componentSpec.type}: ${componentSpec.name}`,
  framework: this.context!.framework,
...
code = codeResult.generatedCode || ''; 
  ...
  code: component.code,
  language: 'ts' as any, // CIG usa 'ts', não 'typescript'
  ...
  code: component.code,
  language: 'ts' as any, // QualityAnalyzer usa 'ts'
  framework: component.framework
});
component.metadata.qualityScore = quality.overallScore;
component.metadata.coverage = quality.metrics?.testCoverage || 0; // está em .metrics.testCoverage
...
class ContextBuilder {
  ...
  private async analyzePrompt(request: GenerationRequest): Promise<PromptAnalysisResult> {
    try {
      // PromptProcessor retorna string, não ProcessingResult
      // Usar fallback direto por enquanto
  ...
  private async searchTemplates(
    ...
    try {
      ...
      // TemplateSearchResult pode ser array ou objeto com .templates
...
const trinity = TrinityOrchestrator.getInstance(); 
  ...
  private generateFallbackTrinity(): TrinityResponse<any> {
    return {
...
// Converter ExtendedValidationResult para ValidationResult
return {
...
/**
 * Named exports (SEM DUPLICAÇÃO)
 * GenerationRequest e GenerationResult já são exportados como interfaces acima
 */
export {
