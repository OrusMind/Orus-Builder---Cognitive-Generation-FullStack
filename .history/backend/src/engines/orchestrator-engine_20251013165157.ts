...
const detectedContext = promptResult.data.context;

logger.info('✅ [Step 1/7] Prompt analyzed', {
...
if (promptEngine) {
  ...
  } catch (error) {  // ✅ ADICIONAR } catch (error) {
...
const generationResult = await (cognitiveEngine as any).generateFromPrompt({
