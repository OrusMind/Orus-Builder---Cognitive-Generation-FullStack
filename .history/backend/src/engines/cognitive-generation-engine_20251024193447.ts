...
constructor(config?: Partial<PipelineConfig>) {
  ...
  private splitCodeIntoComponents(aiCode: string, analysis: any): any[] {
    ...
    while ((match = format1Regex.exec(aiCode)) !== null) {
      const componentName = (match[1] && typeof match[1] === 'string') ? match[1].trim() : '';
      const language = (match[2] && typeof match[2] === 'string') ? match[2].trim() : 'typescript';
      const filePath = (match[3] && typeof match[3] === 'string') ? match[3].trim() : '';
      const code = (match[4] && typeof match[4] === 'string') ? match[4].trim() : '';
    ...
    if (components.length === 0) {
      ...
      while ((match = format2Regex.exec(aiCode)) !== null) {
        const fileName = (match[1] && typeof match[1] === 'string') ? match[1].trim() : '';
        const language = (match[2] && typeof match[2] === 'string') ? match[2].trim() : 'typescript';
        const filePath = (match[3] && typeof match[3] === 'string') ? match[3].trim() : '';
        const code = (match[4] && typeof match[4] === 'string') ? match[4].trim() : '';
    ...
    if (components.length === 0) {
      ...
      const format3Pattern = '``````';
      const format3Regex = new RegExp(format3Pattern, 'gi');
      
      while ((match = format3Regex.exec(aiCode)) !== null) {
        const filePath = (match[1] && typeof match[1] === 'string') ? match[1].trim() : '';
        const code = (match[3] && typeof match[3] === 'string') ? match[3].trim() : '';
  ...
  private extractComponentName(filePath: string, code: string): string {
    ...
    if (nameMatch) {
      return nameMatch[1] || nameMatch[2] || nameMatch[3] || nameMatch[4] || 'Component';
    }
