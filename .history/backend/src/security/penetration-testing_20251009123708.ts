...
private assessImpact(attack: AttackResult): ExploitImpact {
  const impactMap: Record<AttackVector, ExploitImpact> = {
    ...
    [AttackVector.XXE]: {
      confidentiality: 'high',
      integrity: 'low',  // ✅ CORRIGIDO: "medium" → "low"
  availability: 'low',  // ✅ CORRIGIDO: "medium" → "low"
...
`;
  ...
  const remediationMap: Record<AttackVector, string> = {
    ...
    [AttackVector.COMMAND_INJECTION]: 'Avoid system calls. Use safe APIs. Validate and sanitize all inputs. Implement principle of least privilege.',
    ...
    [AttackVector.PATH_TRAVERSAL]: 'Validate file paths. Use whitelists for allowed files. Implement chroot jails. Never use user input directly in file paths.'
  };
