...
export class PromptHistory {
  ...
  public getStatistics() {
    return {
      ...
      oldestEntry: this.historyEntries.length > 0
        ? this.historyEntries.reduce((oldest, e) => 
            e.timestamp < oldest ? e.timestamp : oldest,
            this.historyEntries[0].timestamp
          )
        : null,
      newestEntry: this.historyEntries.length > 0
        ? this.historyEntries.reduce((newest, e) => 
            e.timestamp > newest ? e.timestamp : newest,
            this.historyEntries[0].timestamp
          )
        : null
    };
