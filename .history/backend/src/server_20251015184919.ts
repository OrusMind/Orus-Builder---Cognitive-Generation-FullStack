...
this.app.post('/api/generate', async (req: Request, res: Response) => {
  try {
    const { prompt, framework, locale } = req.body;
    
    this.logger.info('📥 Generate endpoint called', {
      component: 'Server',
    ...
    // ✅ MODIFICAR APENAS ESTAS LINHAS (adicionar options: {})
    const result = await cognitiveGenerationEngine.generate({
      ...
      options: {},  // ✅ ADICIONAR SOMENTE ESTA LINHA
      version: 1,
      isDeleted: false,
      createdAt: new Date(),
      updatedAt: new Date()
    ...
    this.logger.error('❌ Error in generate endpoint', {
      error: error.message,
      stack: error.stack,
      component: 'Server'
    } as any);
