...
this.app.post('/api/test/testing/generate', async (req, res) => {
  try {
    ...
    // ✅ ESTRUTURA CORRETA do TestGenerationRequest
    const result = await testingEngine.generateTests({
      ...
      sourceFiles: [{
        ...
        content: code,
        language: 'typescript',
        type: 'component' as any // ✅ 'component' | 'service' | 'util' | 'controller'
      }],
      
      // Configuration
      testTypes: testType ? [testType] : [TestType.UNIT], // ✅ Array de TestType[]
      framework: TestFramework.JEST, // ✅ Usar enum correto
      targetCoverage: 85,
    ...
    // ✅ result é EngineResult<TestSuite>, acessar via result.data
    res.json({
...
this.app.post('/api/test/deployment/deploy', async (req, res) => {
  try {
    ...
    const result = await deploymentEngine.deploy({
      ...
      // Source
      sourceType: 'local', // ✅ 'git' | 'local' | 'generated'
      localPath: projectPath || './dist',
      
      // Configuration
      strategy: 'blue-green' as any, // ✅ DeploymentStrategy enum
    ...
    // ✅ result é EngineResult<DeploymentResult>, acessar via result.data
    res.json({
      success: result.success,
      engine: 'Deployment',
      deploymentId: result.data?.deploymentId,
      deploymentUrl: result.data?.deploymentUrl, // ✅ Propriedade existe!
      url: result.data?.url,
      status: result.data?.status,
      buildTime: result.data?.buildDuration, // ✅ buildDuration existe!
      deployTime: result.data?.deployDuration,
      totalTime: result.data?.totalDuration,
...
this.app.get('/api/test/monitoring/metrics', async (req, res) => {
  try {
    // ✅ FIX: MonitoringEngine não tem getMetrics, usar recordMetric
