...
function processMultiFileCode(code: string): { 
  ...
  // Limpar markdown ANTES de processar
  ...
  // Detectar multi-file REAL (mais de 1 separador)
  ...
  if (!isMultiFile) {
    ...
    // Remover exports com regex multiline robusto
    processedCode = processedCode.replace(/^export\s+(interface|type|const|function|class|enum|default)\s+/gm, '$1 ');
  ...
  // Ordenar: types primeiro, App por último
  files.sort((a, b) => {
  ...
  const processedFiles = files.map(file => {
    ...
    // Remover TODOS os imports (incluindo React, completamente)
    ...
    // NÃO re-adicionar React aqui! Será adicionado uma vez no topo do arquivo final
    
    // Remover exports
    content = content.replace(/^export\s+(interface|type|const|function|class|enum|default)\s+/gm, '$1 ');
    content = content.replace(/^export\s*\{[^}]+\}\s*;?/gm, '');
    
    return `\n// ============ ${file.path} ============\n${content}\n`;
  });

  // Adicionar React UMA VEZ no topo + adicionar hooks necessários
