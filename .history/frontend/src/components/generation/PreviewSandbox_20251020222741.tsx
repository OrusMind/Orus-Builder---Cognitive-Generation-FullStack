...
/**
 * Remove TypeScript syntax de forma minimalista e segura
 * Remove apenas o essencial - interfaces/types ficam (Babel ignora)
 */
function removeTypeScriptSyntax(code: string): string {
  ...
  // === PASSO 1: REMOVER IMPORTS (LOOP ATÉ NÃO TER MAIS!) ===
  ...
  // === PASSO 3.5: REMOVER TYPE CASTS (as Type) ===
...
function processMultiFileCode(code: string): { 
  ...
  while ((match = filePattern.exec(cleanedCode)) !== null) {
    ...
    // ✅ FILTRO MELHORADO: Apenas arquivos JS/TS E verificar se tem código real
    const isJsFile = /\.(tsx?|jsx?)$/i.test(filePath);
    const isCssFile = /\.css$/i.test(filePath);
    const hasRealCode = /(?:const|function|class|interface|type)\s+\w+/.test(fileContent);
    
    if (isJsFile && hasRealCode) {
      files.push({ path: filePath, content: fileContent });
      console.log('✅ Incluindo arquivo:', filePath);
    } else if (isCssFile) {
      console.log('⏭️ Ignorando CSS:', filePath);
    } else {
      console.log('⏭️ Ignorando (não JS ou sem código):', filePath);
    }
  }

  // Se não encontrou NENHUM arquivo válido, abortar multi-file
  if (files.length === 0) {
    console.log('⚠️ Nenhum arquivo JS válido encontrado! Processando como single-file');
    return {
      isMultiFile: false,
