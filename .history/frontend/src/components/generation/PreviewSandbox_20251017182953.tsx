...
// ═══════════════════════════════════════════════════════════════
// ❌ PASSO 1: DESABILITADO (causava perda de componentes)
// ═══════════════════════════════════════════════════════════════
/*
...
*/
console.log('✅ Step 1: SKIPPED (multi-file support enabled)');
console.log('🔧 Step 1.5: Removing file separator comments...');
cleaned = cleaned
...
console.log('✅ Step 1.5: File separator comments removed');
...
// ═══════════════════════════════════════════════════════════════
// ⭐ PASSO 2.7: REMOVER 'export' DE CONST/FUNCTION/CLASS
// ═══════════════════════════════════════════════════════════════
// Transforma:
//   export const ProductCard = ... → const ProductCard = ...
//   export const CartComponent = ... → const CartComponent = ...
//   export class CartUtils ... → class CartUtils ...
cleaned = cleaned.replace(/export\s+(const|function|class)\s+/g, '$1 ');
console.log('✅ Step 2.7: Removed "export" keyword from named declarations');
...
// ═══════════════════════════════════════════════════════════════
// ✅ PASSO 5.5: REMOVER GENERICS ISOLADOS (MELHORADO)
// ═══════════════════════════════════════════════════════════════
// Remove generics que ficaram órfãos após remover React.FC:
//   const Post<PostProps> = (...) → const Post = (...)
//   const Component <Props> = (...) → const Component = (...)
//   const BlogPost <{ post }> = (...) → const BlogPost = (...)

// ⭐ VERSÃO MELHORADA - Pega TODOS os casos
cleaned = cleaned
  // Caso 1: Generics com espaço (mais comum)
  .replace(/(\w+)\s*<\{[^}]+\}>\s*=/g, '$1 =')     // Component <{ prop }> = ...
  .replace(/(\w+)\s*<[A-Z]\w+>\s*=/g, '$1 =')      // Component <Props> = ...
  
  // Caso 2: Generics sem espaço
  .replace(/(\w+)<\{[^}]+\}>\s*=/g, '$1 =')        // Component<{ prop }> = ...
  .replace(/(\w+)<[A-Z]\w+>\s*=/g, '$1 =')         // Component<Props> = ...
  
  // Caso 3: Generics em functions
  .replace(/(\w+)\s*<[A-Z]\w+>\s*\(/g, '$1(')      // function<Props>(...)
  .replace(/(\w+)\s*<\{[^}]+\}>\s*\(/g, '$1(');    // function<{ prop }>(...) 

console.log('✅ Step 5.5: Removed ALL isolated generics');
