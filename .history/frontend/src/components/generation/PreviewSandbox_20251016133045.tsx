...
/**
 * PreviewSandbox V4.0 - Universal React Preview
 * 
 * ✅ HANDLES: Single component, multiple files, TypeScript, CSS
 * ✅ FIXES: TypeScript generics, imports, multi-file code
 * ✅ WORKS: With ANY generated code
 */
const PreviewSandbox: React.FC<PreviewSandboxProps> = ({ code, className = '' }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // ✅ MERGE ALL TYPESCRIPT/TSX BLOCKS INTO ONE
      const tsxMatches = code.matchAll(/``````/g);
      let mergedCode = Array.from(tsxMatches).map(m => m[1]).join('\n\n');

      // ✅ CLEAN TYPESCRIPT CODE FOR BABEL
      mergedCode = mergedCode
        // Remove ALL imports (they won't work in iframe anyway)
        .replace(/import\s+.*?from\s+['"].*?['"];?\n?/g, '')
        // Remove export keywords from interfaces/types (keep them inline)
        .replace(/export\s+(interface|type)\s+/g, '$1 ')
        // Fix TypeScript generic syntax: React.FC<{ post: Post }> → React.FC
        .replace(/:\s*React\.FC<{[^}]+}>/g, '')
        // Remove export from const components, will export at end
        .replace(/export\s+const\s+(\w+)/g, 'const $1')
        // Remove export default (we'll handle main component separately)
        .replace(/export\s+default\s+/g, '')
        .trim();

      // ✅ FIND MAIN COMPONENT TO RENDER
      const componentNames = [];
      const componentRegex = /const\s+(\w+)(?:\s*:\s*React\.FC)?(?:<[^>]+>)?\s*=\s*\(/g;
      let match;
      while ((match = componentRegex.exec(mergedCode)) !== null) {
        componentNames.push(match[1]);
      }

      // Last component is usually the main one (App, or the parent component)
    ...
    if (rootElement) {
      try {
        ...
        if (MainComponent) {
          ...
          throw new Error('Component ${mainComponent} not found');
        }
      } catch (err) {
        console.error('Render error:', err);
        rootElement.innerHTML = \`
          <div style="padding: 20px; background: #fee; border: 1px solid red; border-radius: 8px;">
            <h2 style="color: red; margin-bottom: 10px;">❌ Preview Error</h2>
            <pre style="color: #c00; font-size: 12px; overflow-x: auto;">\${err.toString()}</pre>
          </div>
        \`;
      }
    }
      ...
      if (root) {
        root.innerHTML = \`
          <div style="padding: 20px; background: #fee; border: 1px solid red; border-radius: 8px;">
            ...
            <pre style="color: #c00; font-size: 12px; overflow-x: auto;">\${message}</pre>
            <p style="margin-top: 10px; font-size: 12px;">Line: \${lineno}, Column: \${colno}</p>
          </div>
        \`;
      }
      return true;
    ...
    <div className={`preview-sandbox ${className}`} style={{ position: 'relative', width: '100%', height: '100%' }}>
      {isLoading && (
        <div style={{
          ...
          <div style={{ fontSize: '14px', color: '#666' }}>⏳ Loading preview...</div>
        </div>
      )}
      ...
      {error && (
        <div style={{
          ...
          <h3 style={{ marginBottom: '10px' }}>❌ Preview Setup Error</h3>
