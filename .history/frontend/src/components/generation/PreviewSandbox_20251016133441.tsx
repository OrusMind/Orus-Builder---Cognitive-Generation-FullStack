...
/**
 * PreviewSandbox V6.0 - Multi-File Component Preview (FINAL)
 * 
 * ✅ HANDLES: Multiple export default in one file
 * ✅ KEEPS: All components accessible in global scope
 * ✅ RENDERS: Last component (usually App)
 */
const PreviewSandbox: React.FC<PreviewSandboxProps> = ({ code, className = '' }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // ✅ FIND ALL COMPONENTS WITH export default
      const componentNames: string[] = [];
      const exportDefaultRegex = /export\s+default\s+(\w+)/g;
      let match;
      while ((match = exportDefaultRegex.exec(mergedCode)) !== null) {
        componentNames.push(match[1]);
      }

      // Last component is usually the main one (App)
      const mainComponent = componentNames[componentNames.length - 1] || 'App';

      console.log('🎯 Components found:', componentNames);
      console.log('🎯 Main component:', mainComponent);

      // ✅ CLEAN CODE FOR BABEL (BUT KEEP ALL COMPONENTS ACCESSIBLE)
      mergedCode = mergedCode
        // Remove ALL imports (won't work in iframe)
        .replace(/import\s+.*?from\s+['"].*?['"];?\n?/g, '')
        // Remove export keywords from interfaces/types
        .replace(/export\s+(interface|type)\s+/g, '$1 ')
        // Fix TypeScript generic syntax
        .replace(/:\s*React\.FC<{[^}]+}>/g, '')
        // Remove "export" but KEEP "default" and component name for now
        .replace(/export\s+default\s+(\w+);?/g, '// Component: $1')
        // Remove standalone "export" from const/function
        .replace(/export\s+(const|function)\s+/g, '$1 ')
        .trim();

      console.log('📦 Processed code length:', mergedCode.length);
...
<body>
  ...
  <script type="text/babel">
    ...
    if (rootElement) {
      try {
        // ✅ ALL COMPONENTS ARE NOW IN SCOPE (Header, Home, Sobre, Planos, Footer, App)
        const MainComponent = ${mainComponent};
        
        if (typeof MainComponent !== 'function') {
          throw new Error('Component "${mainComponent}" is not a function. Found: ' + typeof MainComponent);
        }
        ...
        rootElement.innerHTML = \`
          <div style="padding: 20px; background: #fef2f2; border: 2px solid #dc2626; border-radius: 12px; max-width: 800px; margin: 20px;">
            <div style="display: flex; align-items: center; margin-bottom: 16px;">
              <span style="font-size: 24px; margin-right: 12px;">❌</span>
              <h2 style="color: #dc2626; margin: 0; font-size: 18px;">Preview Error</h2>
            </div>
            
            <div style="background: white; padding: 16px; border-radius: 8px; margin-bottom: 16px;">
              <pre style="color: #991b1b; font-size: 13px; overflow-x: auto; margin: 0; white-space: pre-wrap;">\${err.toString()}</pre>
            </div>
            
            <details style="font-size: 12px; color: #7f1d1d;">
              <summary style="cursor: pointer; font-weight: 600; padding: 8px; background: white; border-radius: 4px; margin-bottom: 12px;">
                🔍 Debug Information
              </summary>
              <div style="background: white; padding: 16px; border-radius: 8px; margin-top: 8px;">
                <p style="margin-bottom: 8px;"><strong>Target Component:</strong> ${mainComponent}</p>
                <p style="margin-bottom: 8px;"><strong>Error Message:</strong> \${err.message}</p>
                <p style="margin-bottom: 8px;"><strong>All Components Found:</strong> ${componentNames.join(', ')}</p>
                <p style="margin-bottom: 12px;"><strong>Stack Trace:</strong></p>
                <pre style="font-size: 11px; overflow-x: auto; background: #f9fafb; padding: 12px; border-radius: 4px;">\${err.stack || 'No stack trace available'}</pre>
              </div>
            </details>
          </div>
        \`;
  ...
  <script>
    ...
    console.log('✅ Preview sandbox loaded');
  </script>
</body>
</html>
      `;
    ...
    <div className={`preview-sandbox ${className}`} style={{ position: 'relative', width: '100%', height: '100%' }}>
      {isLoading && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '16px', color: '#374151', marginBottom: '8px', fontWeight: 600 }}>
              ⏳ Loading Preview
            </div>
      ...
      {error && (
        <div style={{
          ...
          <h3 style={{ marginBottom: '12px', color: '#dc2626', fontSize: '18px' }}>
            ❌ Setup Error
          </h3>
          <pre style={{
            fontSize: '12px',
