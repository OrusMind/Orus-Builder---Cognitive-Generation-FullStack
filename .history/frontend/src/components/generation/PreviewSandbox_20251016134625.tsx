...
const PreviewSandbox: React.FC<PreviewSandboxProps> = ({ code, className = '' }) => {
  ...
  useEffect(() => {
    ...
    if (!iframeDoc) {
      setErrorMsg('Failed to access iframe document');
      return;
    }
    ...
    try {
      ...
      const exportLines = processedCode.split('\n').filter(line => line.includes('export default'));
      const componentName = exportLines.length > 0
        ? exportLines[exportLines.length - 1].replace('export', '').replace('default', '').replace(';', '').trim()
        : 'App';

      processedCode = processedCode
        .split('\n')
        .map(line => {
          if (line.includes('export default')) return '';
      ...
      console.log('Code length:', processedCode.length);
      ...
      const html = [
        ...
        'button { background: #3b82f6; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; margin: 0 4px; }',
        'ul { list-style: none; padding: 0; }',
        'li { background: white; margin: 8px 0; padding: 12px; border-radius: 8px; border: 1px solid #e5e7eb; }',
        ...
        '} catch (err) {',
        'document.getElementById("root").innerHTML = "<div style=\\"padding:20px;background:#fee;border:2px solid #f00;\\"><h2>Error</h2><pre>" + err.toString() + "</pre></div>";',
        '}',
        '<\/script>',
        '<script>window.onerror = (m) => console.error(m);<\/script>',
        '</body></html>'
      ].join('\n');
  ...
  return (
    <div className={'preview-sandbox ' + className} style={{ position: 'relative', width: '100%', height: '100%' }}>
      {isLoading && (
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'white', padding: 24, borderRadius: 12, boxShadow: '0 10px 25px rgba(0,0,0,0.1)', textAlign: 'center' }}>
          <div style={{ fontSize: 16, fontWeight: 600, color: '#374151', marginBottom: 8 }}>Loading Preview</div>
          <div style={{ fontSize: 13, color: '#9ca3af' }}>Compiling...</div>
        </div>
      )}
      {errorMsg && (
        <div style={{ padding: 20, background: '#fee', border: '2px solid #f00', borderRadius: 12, margin: 20 }}>
          <h3 style={{ color: '#f00', marginBottom: 12 }}>Setup Error</h3>
          <pre style={{ fontSize: 12, overflow: 'auto', background: 'white', padding: 16, borderRadius: 8 }}>{errorMsg}</pre>
        </div>
      )}
      <iframe ref={iframeRef} title="Preview" sandbox="allow-scripts allow-same-origin" style={{ width: '100%', height: '100%', border: 'none', display: errorMsg ? 'none' : 'block' }} />
    </div>
  );
};
