...
/**
 * Limpa código TypeScript para compatibilidade com Babel standalone
 * Remove ALL generics, imports externos, type annotations
 */
const cleanTypeScriptForBabel = (code: string, componentName: string): string => {
  let cleaned = code;
  
  console.log('🧹 Starting TypeScript cleanup for Babel...');
  console.log('Original code length:', code.length);

  // ✅ PASSO 1: REMOVER IMPORTS DE BIBLIOTECAS EXTERNAS (CRÍTICO!)
  const externalLibs = [
    'react-router-dom',
    'react-router', 
    'framer-motion',
  ...
  console.log('✅ Step 1: Removed external library imports');

  // ✅ PASSO 2: REMOVER IMPORTS LOCAIS (./components, ../utils, etc)
  const localImportRegex = /import\s+[^;]+from\s+['"]\.['"];?/g;
  cleaned = cleaned.replace(localImportRegex, '');
  
  console.log('✅ Step 2: Removed local file imports');

  // ✅ PASSO 3: REMOVER COMENTÁRIOS (JSDoc e inline)
  cleaned = cleaned
  ...
  // ✅ PASSO 4: SEPARAR APENAS O COMPONENTE PRINCIPAL
  const componentMatches = code.match(/(?:const|function)\s+([A-Z]\w+)/g);
  if (componentMatches && componentMatches.length > 1) {
    console.warn('⚠️ Multiple components detected. Extracting first component only.');
    
    const firstComponentRegex = new RegExp(
      `(const|function)\\s+${componentName}[\\s\\S]*?export\\s+default\\s+${componentName};?`,
      'm'
    );
    const match = code.match(firstComponentRegex);
    if (match) {
      cleaned = `import React, { useState, useEffect } from 'react';\n\n` + match;
    }
  }
  
  console.log('✅ Step 4: Extracted main component');

  // ✅ PASSO 5: REMOVER GENERICS (<Type>)
  cleaned = cleaned.replace(/<[^>]+>/g, '');
  
  console.log('✅ Step 5: Removed generics');

  // ✅ PASSO 6: REMOVER TYPE ANNOTATIONS
  cleaned = cleaned
    ...
    .replace(/:\s*void\b/g, '')
    .replace(/:\s*[A-Z]\w+(\[\])?\b/g, '')
    .replace(/:\s*[a-zA-Z]+\s*\|\s*null/g, '')
    .replace(/:\s*[a-zA-Z]+\s*\|\s*undefined/g, '')
    .replace(/:\s*string\[\]/g, '')
    .replace(/:\s*number\[\]/g, '')
    .replace(/:\s*boolean\[\]/g, '')
    .replace(/:\s*any\[\]/g, '');
  
  console.log('✅ Step 6: Removed type annotations');

  // ✅ PASSO 7: REMOVER PARAMETER TYPES
  cleaned = cleaned
    .replace(/\((\w+):\s*[A-Z]\w+(\[\])?\)/g, '($1)')
    .replace(/,\s*(\w+):\s*[A-Z]\w+(\[\])?/g, ', $1');
  
  console.log('✅ Step 7: Removed parameter types');

  // ✅ PASSO 8: REMOVER RETURN TYPES
  cleaned = cleaned.replace(/\):\s*[A-Z]\w+(\[\])?\s*=>/g, ') =>');
  console.log('✅ Step 8: Removed return types');

  // ✅ PASSO 9: REMOVER TYPE ASSERTIONS (as Type)
  cleaned = cleaned.replace(/\s+as\s+[A-Z]\w+/g, '');
  console.log('✅ Step 9: Removed type assertions');

  // ✅ PASSO 10: GARANTIR EXPORT DEFAULT
  if (!cleaned.includes('export default')) {
    console.log('⚠️ No export default found, adding:', componentName);
...
function cleanComponentCode(code: string) {
