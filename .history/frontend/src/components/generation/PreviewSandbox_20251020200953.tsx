...
function removeTypeScriptSyntax(code: string): string {
  ...
  // === TYPE ANNOTATIONS (CUIDADO COM OBJETOS!) ===
  // APENAS remover tipos em: function params, arrow function params, variable declarations
  // NÃO remover em object literals!
  
  // Remover tipo em parâmetros de função: (param: Type) → (param)
  // Usa lookahead negativo para NÃO remover dentro de { }
  cleaned = cleaned.replace(/\(([^)]+)\)/g, (match) => {
    // Se tem ":", processar
    if (!match.includes(':')) return match;
    
    // Remover type annotations MAS preservar { key: value }
    const processed = match.replace(/(\w+)\s*:\s*([^,)]+)/g, (m, name, typeOrValue) => {
      // Se o próximo caractere é { ou [ ou ' ou " ou número, é valor de objeto!
      if (/^[\s{[\d"'`]/.test(typeOrValue)) {
        return m; // Preservar object property
      }
      return name; // Remover type annotation
    });
    
    return processed;
  });
  
  // Remover return type: ): Type => → ) =>
  cleaned = cleaned.replace(/\)\s*:\s*[^{;=>\n]+(?=\s*=>)/g, ')');
  
  // Remover return type em functions: ): Type { → ) {
  cleaned = cleaned.replace(/\)\s*:\s*[^{;=\n]+(?=\s*\{)/g, ')');
  
  // === DECLARATIONS ===
  cleaned = cleaned.replace(/^(interface|type)\s+\w+[^{]*\{[\s\S]*?\}\s*$/gm, '');
