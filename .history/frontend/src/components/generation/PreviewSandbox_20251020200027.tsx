...
function removeTypeScriptSyntax(code: string): string {
  ...
  // === IMPORTS ===
  // Substituir import uuid por polyfill
  ...
  // Remover imports de libs externas (exceto react e imports relativos)
  cleaned = cleaned.replace(/import\s+.*\s+from\s+['"](?!react|\.)[^'"]+['"];?\s*/g, '');
  
  // === GENERICS ===
  // Remover generics de hooks
  ...
  // === UTILITY TYPES ===
  // Remover Omit<Type, Keys>
  cleaned = cleaned.replace(/Omit<[^>]+>/g, 'any');
  // Remover Pick<Type, Keys>
  cleaned = cleaned.replace(/Pick<[^>]+>/g, 'any');
  // Remover Partial<Type>
  cleaned = cleaned.replace(/Partial<[^>]+>/g, 'any');
  // Remover Record<Key, Value>
  cleaned = cleaned.replace(/Record<[^>]+>/g, 'any');
  
  // === TYPE ANNOTATIONS ===
  // Remover type annotations de parâmetros: (param: Type) → (param)
  cleaned = cleaned.replace(/(\w+)\s*:\s*[^,)=]+(?=[,)=])/g, '$1');
  
  // Remover return type annotations: ): Type => → ) =>
  ...
  // Remover type assertions: value as Type → value
