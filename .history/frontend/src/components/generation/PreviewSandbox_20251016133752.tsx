...
/**
 * PreviewSandbox V7.0 - SOLUÇÃO SIMPLES
 * 
 * ASSUME: Code already clean (no markdown blocks)
 * JUST: Remove imports, inject into Babel, render
 */
const PreviewSandbox: React.FC<PreviewSandboxProps> = ({ code, className = '' }) => {
  ...
  useEffect(() => {
    ...
    try {
      // ✅ SIMPLE: Just clean imports and inject
      let cleanCode = code
        // Remove imports (won't work in iframe)
        .replace(/import\s+.*?from\s+['"].*?['"];?\n?/g, '')
        // Remove type-only imports
        .replace(/import\s+type\s+.*?from\s+['"].*?['"];?\n?/g, '')
        .trim();

      // ✅ DETECT COMPONENT NAME (from export default)
      const exportMatch = cleanCode.match(/export\s+default\s+(\w+)/);
      const componentName = exportMatch ? exportMatch[1] : 'App';

      // ✅ REMOVE export default (but keep component)
...
<body>
    ...
    try {
      ...
      document.getElementById('root').innerHTML = \`
        <div style="padding: 20px; background: #fee; border: 2px solid #f00; border-radius: 8px;">
          <h2 style="color: #f00;">❌ Render Error</h2>
          <pre style="color: #900;">\${err.toString()}</pre>
        </div>
      \`;
    }
  </script>
  ...
  return (
    <div className={`preview-sandbox ${className}`} style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {error && (
        <div style={{ padding: 20, background: '#fee', border: '2px solid #f00', borderRadius: 8 }}>
          <h3>Setup Error</h3>
          <pre>{error}</pre>
        </div>
      )}
