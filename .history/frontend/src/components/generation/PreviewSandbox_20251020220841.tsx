...
/**
 * Remove TypeScript syntax de forma SIMPLES e SEGURA
 * Remove apenas o MÍNIMO necessário - sem regex complexos!
 */
function removeTypeScriptSyntax(code: string): string {
  let cleaned = code;
  
  console.log('🗑️ Removendo TypeScript essencial...');
  ...
  // === PASSO 3: REMOVER GENERICS DE HOOKS (MAIS COMUM) ===
  ...
  // === PASSO 4: REMOVER "React.FC" e type annotations simples ===
  cleaned = cleaned.replace(/:\s*React\.FC(<[^>]+>)?/g, '');
  cleaned = cleaned.replace(/:\s*React\.ReactNode/g, '');
  cleaned = cleaned.replace(/:\s*JSX\.Element/g, '');
  
  // === PASSO 5: REMOVER INTERFACES/TYPES (linha completa) ===
  // Remove interface/type declarations completas
  cleaned = cleaned.replace(/^(export\s+)?interface\s+\w+\s*\{[^}]*\}\s*;?\s*$/gm, '');
  cleaned = cleaned.replace(/^(export\s+)?type\s+\w+\s*=\s*[^;]+;\s*$/gm, '');
