...
function cleanMarkdownFences(code: string): string {
  ...
  // Remove fences com linguagem especificada (``````javascript, etc)
  cleaned = cleaned.replace(/```
  
  // Remove fences simples (```)
  cleaned = cleaned.replace(/```
  
  // Remove fences com \w+ (versões alternativas)
  cleaned = cleaned.replace(/```\w+/g, '');
  
  // ✅ CORRIGIDO: Escapa corretamente o \n literal
  cleaned = cleaned.replace(/\\n`````` (backslash-n + backticks)
  
  // ✅ CORRIGIDO: Remove $/gm (fim de regex que aparece no código)
  cleaned = cleaned.replace(/\$\/gm/g, '');  // Escapa apenas $ e /
  
  // Remove exports que podem quebrar Sandpack
  cleaned = cleaned.replace(/export default /g, '');
  cleaned = cleaned.replace(/export /g, '');
