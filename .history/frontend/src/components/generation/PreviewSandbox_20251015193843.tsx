...
function cleanComponentCode(code: string): string {
  ...
  // ✅ 2. Remover interfaces TypeScript (ANTES de adicionar React globals)
  cleaned = cleaned.replace(/interface\s+\w+\s*{[^}]*}/gs, ''); // g = global, s = multiline

  // ✅ 3. Adicionar React globals
  cleaned = `const { useState, useEffect, useCallback, useMemo, useRef } = React;\n\n${cleaned}`;

  // ✅ 4. REMOVER GENERICS TYPESCRIPT (CRÍTICO!)
  cleaned = cleaned
    ...
    .replace(/:\s*null\b/g, '')
    .replace(/:\s*void\b/g, '')
    // Remove generics complexos (Array<Type>, Promise<Type>, etc)
    .replace(/<[A-Z]\w+(\[\])?>/g, '')
    // Remove parameter types (product: Product) -> (product)
    .replace(/\((\w+):\s*\w+\)/g, '($1)')
    .replace(/,\s*(\w+):\s*\w+/g, ', $1')
  ...
  // ✅ 5. Garantir export default
  if (!cleaned.includes('export default')) {
    console.log('⚠️ No export default found, adding:', componentName);
    cleaned += `\n\nexport default ${componentName};`;
  }

  console.log('✅ Code cleaned, length:', cleaned.length);
  return cleaned;
}
