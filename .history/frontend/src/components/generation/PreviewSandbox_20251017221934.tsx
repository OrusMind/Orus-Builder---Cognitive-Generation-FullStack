...
function processMultiFileCode(code: string): { 
  ...
  // Debug
  console.log('🧹 Primeiros 200 chars:', cleanedCode.substring(0, 200));
  ...
  if (!isMultiFile) {
    ...
    console.log('✅ Single-file processado (exports removidos)');
  ...
  const processedFiles = files.map(file => {
    ...
    if (!content.includes('import React')) content = 'import React from "react";\n' + content;
  ...
  const componentName = files.find(f => f.path.includes('App.tsx')) ? 'App' : (detectComponentName(finalCode, '') || 'GeneratedComponent');
