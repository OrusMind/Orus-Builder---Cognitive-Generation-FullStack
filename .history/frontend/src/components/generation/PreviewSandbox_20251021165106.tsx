...
function cleanMarkdownFences(code: string): string {
  ...
  // ✅ SOLUÇÃO: Usar new RegExp() para evitar confusão de parser
  
  // Remove fences com linguagem especificada (``````jsx, etc)
  ...
  // Remove sequências literais \n```
  cleaned = cleaned.replace(new RegExp('\\\\n```', 'g'), '');
  
  // Remove variações com \w+
  cleaned = cleaned.replace(new RegExp('```
  
  // Remove sequências \n```
  cleaned = cleaned.replace(new RegExp('\\\\n```
  
  // Remove padrão $/gm que aparece em alguns códigos
  cleaned = cleaned.replace(new RegExp('\\$\\/gm', 'g'), '');
  
  // Remove exports
  cleaned = cleaned.replace(new RegExp('export\\s+default\\s+', 'g'), '');
  cleaned = cleaned.replace(new RegExp('export\\s+', 'g'), '');
