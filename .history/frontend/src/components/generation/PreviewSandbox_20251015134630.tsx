...
function cleanComponentCode(code: string): string {
  ...
  // ✅ STEP 1: Remover markdown code blocks (SEM REGEX COM /)
  ...
  // ✅ STEP 2: Remover imports (COM NEW REGEXP - SEGURO)
  ...
  // ✅ STEP 4: Remover comentários de path (COM NEW REGEXP)
  const pathCommentPattern = new RegExp('^//\\s*[\\w/.-]+\\.[tj]sx?\\s*$', 'gm');
  cleanedCode = cleanedCode.replace(pathCommentPattern, '');
  
  // ✅ STEP 5: Remover blocos de comentários /** ... */
  const blockCommentPattern = new RegExp('/\\*\\*[\\s\\S]*?\\*/', 'g');
  cleanedCode = cleanedCode.replace(blockCommentPattern, '');
  ...
  console.log('🧹 Preview:', cleanedCode.substring(0, 200));
