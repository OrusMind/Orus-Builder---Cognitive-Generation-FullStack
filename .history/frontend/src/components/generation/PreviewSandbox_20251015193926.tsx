...
function cleanComponentCode(code: string): string {
  ...
  console.log('🧹 Cleaning code, component name:', componentName);

  // ✅ PASSO 1: Remover imports
  cleaned = cleaned.replace(/import\s+.*?from\s+['"].*?['"];?\n?/g, '');
  console.log('✅ Step 1: Removed imports');

  // ✅ PASSO 2: Remover interfaces TypeScript (múltiplas linhas)
  cleaned = cleaned.replace(/interface\s+\w+\s*{[^}]*}/gs, '');
  console.log('✅ Step 2: Removed interfaces');

  // ✅ PASSO 3: Remover type aliases
  cleaned = cleaned.replace(/type\s+\w+\s*=\s*[^;]+;/g, '');
  console.log('✅ Step 3: Removed type aliases');

  // ✅ PASSO 4: Adicionar React globals
  cleaned = `const { useState, useEffect, useCallback, useMemo, useRef } = React;\n\n${cleaned}`;
  console.log('✅ Step 4: Added React globals');

  // ✅ PASSO 5: Remover generics TypeScript (CRÍTICO!)
  cleaned = cleaned
    // useState<Type> -> useState
    .replace(/useState<[^>]+>/g, 'useState')
    // useEffect, useMemo, useCallback generics
    .replace(/useEffect<[^>]+>/g, 'useEffect')
    .replace(/useMemo<[^>]+>/g, 'useMemo')
    .replace(/useCallback<[^>]+>/g, 'useCallback')
    .replace(/useRef<[^>]+>/g, 'useRef')
    // Outros generics genéricos Array<Type>, Promise<Type>
    .replace(/<[^>]+>/g, '');
  ...
  cleaned = cleaned
    ...
    .replace(/:\s*null\b/g, '')
    .replace(/:\s*void\b/g, '')
    // Tipos customizados (: Product, : Cart, etc)
    .replace(/:\s*[A-Z]\w+(\[\])?\b/g, '');
  
  console.log('✅ Step 6: Removed type annotations');

  // ✅ PASSO 7: Remover parameter types (product: Product) -> (product)
  cleaned = cleaned
  ...
  // ✅ PASSO 8: Remover return types ): Type =>
