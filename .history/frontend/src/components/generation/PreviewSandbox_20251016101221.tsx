 ...
 * Cleans TypeScript React code for browser execution
 */
const cleanComponentCode = (code: string): string => {
  console.log('🧹 Starting code cleaning...');
  console.log('Original code length:', code.length);
  ...
  if (cleaned.includes('// ') && (cleaned.match(/\/\/ \w+\.tsx/g) || []).length > 1) {
    console.log('📦 Multiple files detected, combining...');
    ...
    if (exportDefaults.length > 1) {
      exportDefaults.slice(0, -1).forEach(exp => {
        cleaned = cleaned.replace(exp, '');
      });
    }
    
    console.log('✅ Step 0.5: Combined multiple files');
  } else {
