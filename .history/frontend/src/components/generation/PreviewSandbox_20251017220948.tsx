 ...
 * Backend pode retornar código com ```tsx ... ```
 */
function cleanMarkdownFences(code: string): string {
  return code
    // Remover abertura de fence blocks (```tsx, ``````javascript, etc)
    .replace(/```
    // Remover fechamento de fence blocks
    .replace(/```\s*\n?/g, '')
...
function processMultiFileCode(code: string): { 
  ...
  // ✅ ADICIONAR AQUI - Limpar markdown ANTES de processar
  const cleanedCode = cleanMarkdownFences(code);
  
  // Detectar se é multi-file (contém separadores de arquivo)
  const fileBlocks = cleanedCode.split(/\/\/\s*src\/[\w\/\-\.]+/g).filter(b => b.trim());
  const isMultiFile = fileBlocks.length > 1 || cleanedCode.includes('// src/');

  if (!isMultiFile) {
    return { 
      isMultiFile: false, 
      processedCode: cleanedCode,  // ✅ USAR cleanedCode
      componentName: detectComponentName(cleanedCode, '') || 'App'  // ✅ USAR cleanedCode
    };
  }
  ...
  while ((match = filePattern.exec(cleanedCode)) !== null) {  // ✅ USAR cleanedCode
    files.push({
      path: match[1],
 ...
 files.sort((a, b) => {
  ...
  const processedFiles = files.map(file => {
    ...
    // Remover TODOS os imports (menos React)
