 ...
 * Deixa o trabalho pesado para o Babel!
 */
function removeTypeScriptSyntax(code: string): string {
  ...
  // === PASSO 1: REMOVER IMPORTS (Babel não precisa deles) ===
  cleaned = cleaned.replace(/import\s+[^;]+from\s+['"][^'"]+['"];?\s*/g, '');
  
  // === PASSO 2: REMOVER EXPORTS (Babel não precisa deles) ===
  cleaned = cleaned.replace(/^export\s+default\s+/gm, '');
  cleaned = cleaned.replace(/^export\s+/gm, '');
  cleaned = cleaned.replace(/export\s*\{[^}]+\}\s*;?/gm, '');
  
  // === PASSO 3: REMOVER "React.FC" type annotation APENAS ===
  // (Babel se confunde com isso específico)
  cleaned = cleaned.replace(/:\s*React\.FC(<[^>]+>)?/g, '');
  
  // === FIM! O RESTO É COM O BABEL! ===
  // Babel automaticamente remove:
  // - Interfaces
  // - Types
  // - Type annotations
  // - Generics
  // - E TUDO mais de TypeScript!
