...
/**
 * Processa múltiplos arquivos concatenando na ordem correta
 * e removendo imports/exports para funcionar no iframe.
 */
function processMultiFileCode(code: string): { 
  ...
  // Detectar se é multi-file (contém separadores de arquivo)
  const fileBlocks = code.split(/\/\/\s*src\/[\w\/\-\.]+/g).filter(b => b.trim());
  const isMultiFile = fileBlocks.length > 1 || code.includes('// src/');

  if (!isMultiFile) {
    return { 
      isMultiFile: false, 
      processedCode: code,
      componentName: detectComponentName(code, '') || 'App' // ✅ FIX 1
    };
  }

  console.log('🗂️ Multi-file detectado:', fileBlocks.length, 'arquivos');

  // Extrair blocos de código com nome do arquivo
  ...
  // Ordenar: types primeiro, App por último, resto no meio
  files.sort((a, b) => {
  ...
  const processedFiles = files.map(file => {
    ...
    // Remover TODOS os imports (menos React se existir)
    content = content.replace(/import\s+(?:type\s+)?\{[^}]+\}\s+from\s+['"][^'"]+['"];?\s*/g, '');
    content = content.replace(/import\s+[\w\s,{}]+\s+from\s+['"][^'"]+['"];?\s*/g, '');
    
    // Manter apenas import React
    if (!content.includes('import React')) {
      content = 'import React from "react";\n' + content;
    }
    
    // Remover exports (transformar em declarações normais)
    content = content.replace(/export\s+(interface|type|const|function|class|enum)\s+/g, '$1 ');
  ...
  // Concatenar tudo
  const finalCode = processedFiles.join('\n');
  
  // Detectar componente principal (App ou último arquivo .tsx)
  const appFile = files.find(f => f.path.includes('App.tsx'));
  const componentName = appFile ? 'App' : (detectComponentName(finalCode, '') || 'GeneratedComponent'); // ✅ FIX 2
...
function generatePreviewHTMLSafe(code: string, filename: string): string {
  ...
  const safeCode = processedCode.replace(/"/g, '&quot;');
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    #root { width: 100%; max-width: ${isMultiFile ? '100%' : '1200px'}; margin: 0 auto; }
  </style>
</head>
<body>
  <div id="root"></div>
  <script type="text/babel">
    ...
    try {
      ...
      document.getElementById('root').innerHTML = '<div style="color:red;padding:2rem;background:white;border-radius:8px;"><h2>❌ Erro ao Renderizar</h2><pre>' + error.message + '</pre></div>';
    }
  </script>
</body>
</html>`;
