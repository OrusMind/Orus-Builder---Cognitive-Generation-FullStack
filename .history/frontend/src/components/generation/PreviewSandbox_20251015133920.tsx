...
cleanedCode = cleanedCode.replace(/^\/\/\s*[\w/.-]+\.[tj]sx?\s*$/gm, '');
  cleanedCode = cleanedCode.replace(/```\n?/g, '');
  
  // ✅ Remover imports
  cleanedCode = cleanedCode.replace(/import\s+.*?\s+from\s+['"][^'"]+['"];?\n?/g, '');
  
  // ✅ Remover export default
  cleanedCode = cleanedCode.replace(/export\s+default\s+/g, '');
  
  // ✅ Remover comentários de path (// components/Login.tsx)
  cleanedCode = cleanedCode.replace(/^\/\/\s*[\w\/.-]+\.(tsx?|jsx?)\s*$/gm, '');
