...
function removeTypeScriptSyntax(code: string): string {
  ...
  cleaned = cleaned.replace(/import\s+[^;]+from\s+['"]([^'"]+)['"];?\s*/g, (match, lib) => {
    // MANTER: import React ou imports relativos (./  ../)
    if (lib === 'react' || lib.startsWith('.')) {
      return match;
    }
    // REMOVER: tudo o resto (react-router-dom, uuid, etc)
  ...
  // Remover imports residuais (exceto React)
  cleaned = cleaned.replace(/import\s+[^;]+;?\s*/g, (match) => {
  ...
  console.log('🔍 Contém "import" não-React?', /import\s+.*from\s+['"](?!react)/.test(cleaned));
...
function detectComponentName(code: string, filename: string): string | null {
  ...
  // 2. Procurar ALL componentes React (maiúscula + =)
  const allComponentMatches = code.match(/const\s+([A-Z]\w+)\s*=\s*\(/g);
  if (allComponentMatches && allComponentMatches.length > 0) {
    // Pegar o ÚLTIMO (geralmente é o component principal)
    const lastMatch = allComponentMatches[allComponentMatches.length - 1];
    const componentName = lastMatch.match(/const\s+([A-Z]\w+)/)?.[1];
    if (componentName) {
      console.log('✅ Detectado via const (último):', componentName);
  ...
  return 'App'; // ✅ FALLBACK - usar 'App' ao invés de null
}
