...
${cleaned}`;
  ...
  // ✅ PASSO 7: REMOVER TYPE ANNOTATIONS (VERSÃO CORRIGIDA)
cleaned = cleaned
  .replace(/:\s*React\.FC\s*/g, ' ') // ← Adiciona espaço
  ...
  .replace(/:\s*null\b/g, '')
  .replace(/:\s*void\b/g, '')
  .replace(/:\s*Error\s*\|\s*null/g, '') // ← Adiciona caso específico
  .replace(/:\s*[A-Z]\w+\s*\|\s*null/g, '') // ← Union types
  .replace(/:\s*[A-Z]\w+(\[\])?\b/g, '');

console.log('✅ Step 7: Removed type annotations');

// ✅ PASSO 7.5: LIMPAR ESPAÇOS DUPLOS (NOVO!)
cleaned = cleaned.replace(/\s{2,}/g, ' '); // Remove espaços múltiplos
console.log('✅ Step 7.5: Cleaned up whitespace');
  ...
  // Se não detectamos antes, tentar detectar agora
