...
function removeTypeScriptSyntax(code: string): string {
  ...
  // === TYPE ANNOTATIONS (SEM QUEBRAR OBJETOS!) ===
  // Remover APENAS type annotations óbvias, sem quebrar object literals
  ...
  // 3. Remover tipo em arrow function params SIMPLES (sem objetos)
  cleaned = cleaned.replace(/\(([^){}[\]]+)\)\s*=>/g, (match, params) => {
    // Se tem { ou [, PULAR (é object destructuring)
    if (params.includes('{') || params.includes('[')) {
      return match; // NÃO processar
    }
    
    // Remover types: param: Type, param2: Type
