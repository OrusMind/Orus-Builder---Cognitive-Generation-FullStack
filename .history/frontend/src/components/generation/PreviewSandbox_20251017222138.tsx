...
function processMultiFileCode(code: string): { 
  ...
  if (!isMultiFile) {
    ...
    return { isMultiFile: false, processedCode, componentName: detectComponentName(processedCode, '') || 'App' };
  }
  ...
  const processedFiles = files.map(file => {
    ...
    // Checar se tinha React ANTES de remover imports
    const hadReactImport = /import\s+React/.test(content);
    ...
    // Re-adicionar React UMA VEZ se tinha
    if (hadReactImport) {
  ...
  // ADICIONAR React global UMA VEZ no topo
