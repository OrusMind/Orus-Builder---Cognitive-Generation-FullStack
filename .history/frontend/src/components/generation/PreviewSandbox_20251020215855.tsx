...
function removeTypeScriptSyntax(code: string): string {
  ...
  // ✅ NOVO! Remover type unions MULTI-LINHA (ANTES de remover types simples)
  // Remove desde "type Nome =" até o ponto-e-vírgula final, incluindo quebras de linha
  cleaned = cleaned.replace(/^(export\s+)?type\s+\w+\s*=\s*[\s\S]*?;/gm, '');
  
  // === DECLARATIONS (CUIDADO - NÃO DELETAR COMPONENTES!) ===
  // Remover APENAS interface/type SIMPLES de 1 linha (não union types ou componentes)
  // NÃO remover se tem pipe | (union type complexo)
  cleaned = cleaned.replace(/^(export\s+)?(interface|type)\s+\w+(\s+extends\s+\w+)?\s*=\s*\{[^}|]{0,150}\}\s*;?\s*$/gm, '');
