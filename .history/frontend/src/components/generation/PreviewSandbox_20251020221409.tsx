...
/**
 * Remove TypeScript syntax MINIMALISTA
 * Remove APENAS o mínimo essencial - interfaces/types ficam (Babel ignora)!
 */
function removeTypeScriptSyntax(code: string): string {
  ...
  // === PASSO 5: REMOVER APENAS type annotations EM PARAMS DE FUNÇÃO ===
  // Remove ": Type" em parâmetros - MAS SÓ SE ESTÁ ENTRE ( e ) ou ( e ,
  cleaned = cleaned.replace(/(\w+)\s*:\s*[A-Z]\w+(\s*[,)])/g, '$1$2');
  
  // === PASSO 6: REMOVER return types ===
  cleaned = cleaned.replace(/\)\s*:\s*[A-Z][\w<>[\]|&\s]*\s*(?=\{|=>)/g, ') ');
  
  // === FIM! NÃO TENTAMOS REMOVER INTERFACES/TYPES ===
  // Babel/React ignoram elas - deixamos ficar!
...
function processMultiFileCode(code: string): { 
  ...
  if (!isMultiFile) {
    ...
    // ✅ Remover export default de FORMA INTELIGENTE
    // 1. Se for "export default ComponentName;" (linha sozinha) - remover linha toda
    processedCode = processedCode.replace(/^export\s+default\s+(\w+)\s*;?\s*$/gm, '');
    
    // 2. Se for "export default function/const" - remover só o "export default"
    processedCode = processedCode.replace(/^export\s+default\s+(?=function|const|class)/gm, '');
    
    // 3. Depois remover outros exports
  ...
  while ((match = filePattern.exec(cleanedCode)) !== null) {
    ...
    // ✅ FILTRAR: Apenas arquivos .ts, .tsx, .js, .jsx (IGNORAR .css, .json, etc)
  ...
  const processedFiles = files.map(file => {
    ...
    // Remover TODOS os imports (React será injetado globalmente)
    ...
    // ✅ NOVO! Remover export default de FORMA INTELIGENTE
    // 1. Se for "export default ComponentName;" (linha sozinha) - remover linha toda
    ...
    // 3. Depois remover outros exports normais
  ...
  // Detectar component name (App ou último componente)
...
function detectComponentName(code: string, filename: string): string | null {
  ...
  if (filename) {
    const fileMatch = filename.match(/([A-Z][a-zA-Z0-9]+)\.tsx?$/);
    if (fileMatch) {
      console.log('✅ Detectado via filename:', fileMatch[1]);
      return fileMatch[1];
    }
  }
  
  // === PRIORIDADE 2: "export default ComponentName" (MAIS CONFIÁVEL!) ===
  const exportDefaultMatch = code.match(/export\s+default\s+([A-Z][a-zA-Z0-9]+)/);
  if (exportDefaultMatch) {
    console.log('✅ Detectado via export default:', exportDefaultMatch[1]);
    return exportDefaultMatch[1];
  }
  
  // === PRIORIDADE 3: const/function declarations (PEGAR O PRIMEIRO, NÃO O ÚLTIMO!) ===
  const constMatches = code.match(/(?:const|function)\s+([A-Z][a-zA-Z0-9]+)\s*(?:=|:|\()/g);
  if (constMatches && constMatches.length > 0) {
    // ✅ MUDANÇA CRÍTICA: Pega o PRIMEIRO, não o último!
    const firstMatch = constMatches[0].match(/(?:const|function)\s+([A-Z][a-zA-Z0-9]+)/);
    if (firstMatch) {
      console.log(`✅ Detectado via const/function (primeiro): ${firstMatch[1]} - Total encontrados: ${constMatches.length}`);
      return firstMatch[1];
    }
  }
  
  console.log('❌ Nenhum component name detectado');
  return null;
}
