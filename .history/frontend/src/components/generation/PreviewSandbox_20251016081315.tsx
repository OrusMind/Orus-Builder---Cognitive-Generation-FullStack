...
function cleanComponentCode(code: string): string {
  ...
  // ✅ PASSO 1: REMOVER TODOS OS IMPORTS (externos E locais)
  cleaned = cleaned.replace(/import\s+.*?from\s+['"].*?['"];?\n?/g, '');
  console.log('✅ Step 1: Removed ALL imports');

  // ✅ PASSO 2: EXTRAIR APENAS O PRIMEIRO COMPONENTE
  // Se há múltiplos componentes (como no seu caso), pegar só o primeiro
  const componentRegex = new RegExp(
    `(const|function)\\s+${componentName}[\\s\\S]*?(?=\\n(?:const|function|export default|$))`,
    'm'
  );
  const match = code.match(componentRegex);
  
  if (match && code.includes('export default') && code.includes('./')) {
    console.warn('⚠️ Multiple files detected! Extracting first component only.');
    // Se tem imports locais, é múltiplos arquivos - usar só o primeiro componente
    cleaned = match[0];
  }
  
  console.log('✅ Step 2: Extracted main component');

  // ✅ PASSO 3: Remover interfaces TypeScript
  cleaned = cleaned.replace(/interface\s+\w+\s*{[^}]*}/gs, '');
  console.log('✅ Step 3: Removed interfaces');

  // ✅ PASSO 4: Remover type aliases
  cleaned = cleaned.replace(/type\s+\w+\s*=\s*[^;]+;/g, '');
  console.log('✅ Step 4: Removed type aliases');

  // ✅ PASSO 5: Adicionar React globals
  cleaned = `const { useState, useEffect, useCallback, useMemo, useRef } = React;\n\n${cleaned}`;
  console.log('✅ Step 5: Added React globals');

  // ✅ PASSO 6: Remover generics TypeScript (<Type>)
  cleaned = cleaned
  ...
  console.log('✅ Step 6: Removed generics');
  ...
  console.log('✅ Step 7: Removed type annotations');
  ...
  // ✅ PASSO 11: Garantir export default
