 ...
 * Limpa código TypeScript para compatibilidade com Babel
 * Remove imports, generics, type annotations, múltiplos componentes
 * HANDLE MULTIPLE FILES - Extrai apenas o primeiro componente
 */
function cleanComponentCode(code: string): string {
  ...
  if (hasMultipleFiles) {
    ...
    // Extrair apenas o primeiro arquivo/componente
  ...
  cleaned = cleaned
    .replace(/\/\/\s*\w+\.tsx/g, '') // Remove // LoginForm.tsx
  ...
  // ✅ PASSO 5: ADICIONAR REACT GLOBALS NO TOPO
...
${cleaned}`;
  ...
  // (vamos adicionar apenas UM no final)
  cleaned = cleaned.replace(/export\s+default\s+\w+;?/g, '');
  console.log('✅ Step 11: Removed all existing export default statements');

  // ✅ PASSO 12: DETECTAR NOME DO COMPONENTE E ADICIONAR EXPORT ÚNICO
  const componentName = detectComponentName(cleaned, 'Component');
  
  if (componentName) {
    // Garantir que só tem UM export default no FINAL
    cleaned = cleaned.trim() + `

export default ${componentName};`;
    console.log('✅ Step 12: Added single export default at END:', componentName);
  } else {
