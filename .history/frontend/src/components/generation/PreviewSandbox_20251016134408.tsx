  ...
  useEffect(() => {
    ...
    try {
      ...
      if (processedCode.includes('```
        const mdRegex = new RegExp('```(?:typescript|tsx|javascript|jsx)?\\n([\\s\\S]*?)```
        const matches = Array.from(processedCode.matchAll(mdRegex));
        if (matches.length > 0) {
          processedCode = matches.map(m => m).join('\n\n');[1]
        }
      }
      ...
      const importTypeRegex = new RegExp('import\\s+type\\s+.*?from\\s+[\'"].*?[\'"];?\\n?', 'g');
      processedCode = processedCode.replace(importTypeRegex, '').trim();

      // Find component name
      const exportRegex = new RegExp('export\\s+default\\s+(\\w+)', 'g');
      const allExports = Array.from(processedCode.matchAll(exportRegex)).map(m => m);[1]
      ...
      // Build HTML (without template literals in complex parts)
      const htmlParts = [
        ...
        `if (typeof Component !== "function") throw new Error("${componentName} is not a component");`,
        'root.render(<Component />);',
        `console.log("Rendered:", "${componentName}");`,
        '} catch (err) {',
        'console.error("Error:", err);',
        'rootElement.innerHTML = "<div style=\\"padding: 20px; background: #fee; border: 2px solid #f00; border-radius: 8px;\\"><h2 style=\\"color: #f00;\\">Preview Error</h2><pre style=\\"color: #900;\\">" + err.toString() + "</pre></div>";',
        '}',
        '</script>',
        '<script>window.onerror = (msg) => { console.error("Global error:", msg); return true; };</script>',
        '</body></html>'
      ];
