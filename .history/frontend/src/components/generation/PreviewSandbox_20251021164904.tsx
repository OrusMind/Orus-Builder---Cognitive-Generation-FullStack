...
function cleanMarkdownFences(code: string): string {
  ...
  // Remove fences com linguagem especificada (``````tsx, ```
  cleaned = cleaned.replace(/```[\w]*\n?/g, '');
  
  // Remove fences simples
  cleaned = cleaned.replace(/```
  
  // Remove variações de fences com \w+
  cleaned = cleaned.replace(/```\w+/g, '');
  
  // Remove sequências literais \n```
  cleaned = cleaned.replace(/\\n```/g, '');
  
  // Remove padrão $/gm que aparece em alguns códigos gerados
  cleaned = cleaned.replace(/\$\/gm/g, '');
