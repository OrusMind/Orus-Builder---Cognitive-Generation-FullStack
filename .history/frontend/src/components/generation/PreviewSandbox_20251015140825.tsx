...
function generatePreviewHTML(code: string, filename: string): string {
  ...
  const fixedCode = fixLayoutIssues(cleanCode); // ← ADICIONAR
  const codeEscaped = fixedCode; // ← USAR fixedCode
...
// ✅ REMOVER markdown blocks COM LOOP
const patterns = ['typescript', 'tsx', 'javascript', 'jsx', 'ts', 'js', ''];
const BACKTICKS = '```'; // Define fora do loop
patterns.forEach(lang => {
  const marker = lang ? BACKTICKS + lang : BACKTICKS;
  while (cleanedCode.indexOf(marker) !== -1) {
    cleanedCode = cleanedCode.replace(marker, '');
  }
});
...
function fixLayoutIssues(code: string): string {
  let fixed = code;
  
  console.log('🔧 Corrigindo problemas de layout...');
  
  // Fix 1: h-screen → min-h-screen no container principal
  fixed = fixed.replace(
    /className="flex h-screen bg-gray-/g,
    'className="flex min-h-screen w-full bg-gray-'
  );
  
  // Fix 2: Sidebar absolute → relative
  fixed = fixed.replace(
    /className={\`[^}]*\} md:block bg-white shadow-md[^`]*absolute top-\d+ left-0 h-screen[^`]*\`}/g,
    'className="hidden md:block w-64 bg-white shadow-md relative"'
  );
  
  // Fix 3: Main content com overflow
  fixed = fixed.replace(
    /<main className="flex-1 p-/g,
    '<main className="flex-1 overflow-auto p-'
  );
  
  // Fix 4: Header sticky
  fixed = fixed.replace(
    /<header className="bg-white shadow-md/g,
...
function escapeCodeForHTML(code: string): string {
  // ✅ NÃO ESCAPAR NADA! Babel entende JSX/TSX nativamente
  return code;
}
