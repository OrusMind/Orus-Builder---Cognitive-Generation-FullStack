...
// ⭐ PASSO 5.5: REMOVER GENERICS ISOLADOS (antes de JSX)
// Remove padrões como: const Product <Props> = 
...
// ✅ PASSO 7: REMOVER TYPE ANNOTATIONS (VERSÃO FINAL)
cleaned = cleaned
  ...
  // ⭐ CRÍTICO: Remove tipo COM array annotation (ANTES dos tipos simples!)
  .replace(/:\s*[A-Z]\w+\[\]/g, '')  // Remove : Type[] completo
  
  // Remove inline object types
  .replace(/:\s*\{[^}]+\}/g, '')
  
  // Remove tipos simples (DEPOIS dos arrays!)
  .replace(/:\s*[A-Z]\w+\b/g, '');

console.log('✅ Step 7: Removed type annotations (including arrays)');
