...
/**
 * 🔨 Remove markdown fences SEM regex - apenas split
 */
function cleanMarkdownFences(code: string): string {
  ...
  // Se não tem backticks, retorna direto
  if (!code.includes('`')) {
    return code;
  }
  ...
  // Se tem múltiplos blocos de code fences
  if (parts.length > 1) {
    // Pega partes em índice par (fora dos fences)
    const cleaned = parts
...
function processMultiFileCode(code: string, filename: string): Record<string, string> {
  console.log('🔍 CÓDIGO ORIGINAL (primeiros 500 chars):', code.substring(0, 500));
  
  const cleanedCode = cleanMarkdownFences(code);
  console.log('🧹 CÓDIGO LIMPO (primeiros 500 chars):', cleanedCode.substring(0, 500));
  console.log('📏 Tamanho código limpo:', cleanedCode.length, 'caracteres');
  
  const filePattern = /\/\/\s*(?:src\/)?([^\n]+\.tsx?)\n([^]*?)(?=\/\/\s*(?:src\/)?[^\n]+\.tsx?|$)/gi;
  const matches = [...cleanedCode.matchAll(filePattern)];
  
  if (matches.length > 1) {
    console.log(`🗂️ Multi-file: ${matches.length} arquivos`);
    ...
    matches.forEach(([_, path, content]) => {
      ...
      console.log(`✅ ${cleanPath}: ${cleanContent.length} chars`);
    });
  ...
  const finalCode = cleanMarkdownFences(cleanedCode);
  console.log('📄 App.tsx final:', finalCode.length, 'chars');
  console.log('📄 App.tsx conteúdo:', finalCode.substring(0, 200));
