...
import { ProjectCard } from '@components/project/ProjectCard'; // ✅ CORRETO
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    const realProjects = (projectsData.data || []).map((p: BackendProject) => ({
  id: p.id,
  name: p.name,
  ...
  files: p.files || [],
    ...
    setStats({
      ...
      totalGenerations: statsData.data?.totalFiles || 0,
      
      successRate: 95, // Mock por enquanto
    });

    // Check Trinity health (mantém seu código original)
    const health = await generationService.checkTrinityHealth();
    setTrinityHealth(health);

    // ✅ ACTIVITY REAL (baseado em projetos)
    setRecentActivity(
  realProjects.slice(0, 3).map((p: Project) => ({  // ✅ Usar tipo Project (já definido)
    id: p.id,
    type: 'generation',
