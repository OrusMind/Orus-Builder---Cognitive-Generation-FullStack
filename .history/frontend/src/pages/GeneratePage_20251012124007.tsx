...
export const GeneratePage: React.FC = () => {
  ...
  const handleGenerate = async (prompt: string, options: PromptOptions) => {
    ...
    try {
      // 🔥 CALL REAL BACKEND
      const response = await fetch('http://localhost:5000/api/v1/generation/prompt', {
        method: 'POST',
      ...
      // Get results
      const resultResponse = await fetch(`http://localhost:5000/api/v1/generation/${generatedJobId}`);
