...
// ===================================================================
// COMPONENTE LIVEPREVIEW - RENDERIZAÇÃO EM IFRAME
// ===================================================================
const LivePreview: React.FC<{ files: GeneratedFile[] }> = ({ files }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const componentName = exportMatch?.[1] || constMatch?.[1] || functionMatch?.[1] || 'Component';
          ...
          <script type="text/babel">
            ...
            try {
              ...
              document.getElementById('root').innerHTML = \`
                <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px;background:#fee;">
                  <div style="max-width:600px;padding:24px;background:white;border:2px solid #fcc;border-radius:12px;">
                    <h2 style="color:#c00;margin:0 0 12px 0;">❌ Erro no Preview</h2>
                    <pre style="margin:0;font-size:12px;overflow-x:auto;background:#fff8f8;padding:12px;border-radius:6px;border:1px solid #fcc;">\${error.message}</pre>
                  </div>
                </div>
              \`;
            }
      ...
      setError(err.message || 'Erro ao renderizar');
      setLoading(false);
    }
  }, [files]);
    ...
    return (
      <div className="flex items-center justify-center h-full bg-red-50 dark:bg-red-900/20 p-6">
        <div className="text-center max-w-md">
          <h3 className="text-xl font-bold text-red-600 mb-2">Erro no Preview</h3>
          <p className="text-sm text-red-500 font-mono bg-red-100 dark:bg-red-900/40 p-4 rounded-lg">
            {error}
          </p>
        </div>
      </div>
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-900/80 z-10">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Renderizando...</p>
          </div>
        </div>
      )}
      <iframe
