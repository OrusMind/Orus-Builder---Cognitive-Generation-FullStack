...
const LivePreview: React.FC<{ files: GeneratedFile[] }> = ({ files }) => {
 ...
 /**
 * Build preview HTML - VERSÃO CORRIGIDA PARA TYPESCRIPT
 */
const buildPreviewHTML = (): string => {
  try {
    ...
    if (!componentFile) {
      throw new Error('Nenhum arquivo de componente React (.tsx/.jsx) encontrado');
    }

    // ✅ Limpar código TypeScript para Babel no browser
    let cleanedCode = componentFile.content;
    
    // Remover import de React (já está no global)
    cleanedCode = cleanedCode.replace(/import\s+React[^;]+;?\s*/g, '');
    
    // Remover interfaces TypeScript (Babel browser não processa bem)
    cleanedCode = cleanedCode.replace(/interface\s+\w+\s*\{[^}]+\}/gs, '');
    
    // Remover tipos explícitos (React.FC<Props> → React.FC)
    cleanedCode = cleanedCode.replace(/React\.FC<\w+>/g, 'React.FC');
    
    // Remover type annotations de parâmetros
    cleanedCode = cleanedCode.replace(/:\s*React\.\w+/g, '');
    cleanedCode = cleanedCode.replace(/:\s*\w+\s*=/g, ' =');
    
    // Extrair nome do componente
    const componentMatch = cleanedCode.match(
      /(?:export\s+default\s+|const\s+|function\s+)(\w+)/
    );
    const componentName = componentMatch ? componentMatch[1] : 'Component';

    console.log('🔍 Arquivo:', componentFile.fileName);
...
<head>
  ...
  <style>
    ...
    .error-display h3 { margin: 0 0 15px 0; font-size: 20px; }
    .error-display pre {
      margin: 15px 0 0 0;
      padding: 15px;
      background: #fff;
...
<body>
  ...
  <script type="text/babel" data-type="module">
    try {
      // ✅ Código do componente (limpo de TypeScript)
      ${cleanedCode}
      ...
      if (!ComponentToRender) {
        ...
        if (names.length > 0) {
          ComponentToRender = window[names[0]];
          console.log('✅ Componente encontrado (window):', names[0]);
        }
      }
      
      if (!ComponentToRender) {
        throw new Error('Componente "${componentName}" não encontrado após transpilação');
      }
      
      // ✅ Renderizar com props default
      ...
      document.getElementById('root').innerHTML = 
        ...
        '<h3>❌ Erro ao Renderizar Preview</h3>' +
        '<p><strong>Erro:</strong> ' + error.message + '</p>' +
        '<pre>' + (error.stack || 'Sem stack trace') + '</pre>' +
        '<p style="margin-top: 20px; font-size: 11px;">💡 Dica: Abra o DevTools (F12) para mais detalhes</p>' +
        '</div>';
    }
  </script>
</body>
