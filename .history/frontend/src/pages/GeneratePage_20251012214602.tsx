...
interface ParsedComponent {
  ...
  type: 'dashboard' | 'form' | 'button' | 'generic';
...
/**
 * Parser inteligente que extrai dados do código gerado
 */
function parseComponentCode(code: string): ParsedComponent {
  ...
  if (lowerCode.includes('dashboard') || lowerCode.includes('metric')) {
    ...
    const metricsMatch = code.match(/const\s+(\w+)\s*:\s*\w+\[\]\s*=\s*\[([\s\S]*?)\];/);
    if (metricsMatch) {
      result.metrics = parseMetricsArray(metricsMatch[2]);
    }
  } else if (lowerCode.includes('form') || (lowerCode.includes('input') && lowerCode.includes('button'))) {
    result.type = 'form';
    result.hasForm = true;
    
    // Extrair campos do formulário
    const inputMatches = code.matchAll(/<input[^>]*type="(\w+)"[^>]*name="(\w+)"[^>]*>/g);
  ...
  } else if (lowerCode.includes('button')) {
    result.type = 'button';
  }
