...
const buildPreviewHTML = (): string => {
  try {
    // ✅ USAR O CÓDIGO REAL DO PRIMEIRO ARQUIVO
    const componentFile = files.find(f => 
      f.path.includes('App.tsx') || 
      f.path.includes('Component.tsx') ||
      f.path.endsWith('.tsx')
    ) || files[0];
    
    if (!componentFile) throw new Error('Nenhum arquivo encontrado');

    // ✅ RENDERIZAR O CÓDIGO REAL (não parsear!)
    const componentCode = componentFile.content;
    const componentName = componentFile.fileName.replace('.tsx', '').replace('.jsx', '');
    
    // ✅ GERAR HTML QUE EXECUTA O CÓDIGO REAL
    return `
      <!DOCTYPE html>
      <html lang="pt-BR">
      <head>
      ...
      <body>
        ...
        <script>
          try {
            // ✅ INJETAR O CÓDIGO REAL GERADO
            ${componentCode.replace(/export default /g, 'const Component = ')}
            
            // ✅ RENDERIZAR
            const root = ReactDOM.createRoot(document.getElementById('root'));
            root.render(React.createElement(Component || App));
            console.log('✅ Preview renderizado com código real!');
          } catch (error) {
            console.error('❌ Erro:', error);
            document.getElementById('root').innerHTML = \`
