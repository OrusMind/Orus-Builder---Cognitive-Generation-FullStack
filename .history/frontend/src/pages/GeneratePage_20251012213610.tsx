...
/**
 * Build preview HTML - VERSÃO REAL (Renderiza código gerado)
 */
const buildPreviewHTML = (): string => {
  try {
    ...
    // ✅ LIMPAR CÓDIGO PARA RODAR NO BROWSER
    let cleanedCode = componentFile.content;
    
    // Remover imports de CSS (não vai funcionar no browser)
    cleanedCode = cleanedCode.replace(/import\s+['"].*\.css['"];?\s*/g, '');
    
    // Remover outros imports (React já está no global)
    cleanedCode = cleanedCode.replace(/import\s+.*from\s+['"]react['"];?\s*/g, '');
    
    // Remover interfaces TypeScript (Babel browser processa mal)
    ...
    console.log('🔍 Código limpo:', cleanedCode.substring(0, 300));
    ...
    (function() {
      ...
      try {
        ...
        document.getElementById('root').innerHTML = 
          ...
          '<h2 style="color: #c00; margin-bottom: 16px;">❌ Erro ao Renderizar</h2>' +
          '<p style="color: #7f1d1d; font-size: 14px;">' + error.message + '</p>' +
          '<p style="margin-top: 16px; font-size: 12px; color: #6b7280;">Abra o DevTools (F12) para mais detalhes</p>' +
          '</div>';
      }
    })();
  </script>
