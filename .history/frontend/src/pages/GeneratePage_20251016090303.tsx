...
const handleSaveProject = async () => {
  ...
  try {
    ...
    console.log('📦 Enviando projeto:', {
      name: projectName,
description: prompt || chatMessages[0]?.content,  // ✅
    ...
    body: JSON.stringify({
  name: projectName,
  description: prompt || chatMessages[0]?.content || 'Projeto gerado pelo ORUS Builder',
  files: files,
  framework: 'react',
  prompt: prompt || chatMessages[0]?.content || '',
  status: 'active',
  createdAt: new Date().toISOString()
    ...
    if (response.ok && (data.success || data.project)) {
      ...
      alert(`❌ ${data.message || data.error || 'Erro ao salvar'}`);
    }
  } catch (error: any) {
   ...
   */
  /**
 * ✅ Add message helper (FIXED: unique IDs)
 * Garante IDs únicos usando contador incremental
 */
let messageIdCounter = 0; // ✅ ADICIONAR ESTA LINHA ANTES DA FUNÇÃO!

const addMessage = (role: 'user' | 'trinity', content: string) => {
  messageIdCounter++; // ✅ Incrementar contador
  
  setChatMessages(prev => [
    ...prev,
    {
      id: `${role}-${Date.now()}-${messageIdCounter}`,  // ✅ ADICIONAR CONTADOR!
      role,
      content,
      timestamp: new Date()
    }
...
useEffect(() => {
  // ✅ Previne dupla execução mesmo com StrictMode
  if (hasGeneratedRef.current) {
    console.log('🛑 Generation already triggered, skipping...');
    return;
  }

  if (hasInitialPrompt && state.prompt && !isGenerating && !isComplete && !jobId) {
    hasGeneratedRef.current = true; // ✅ Marca como executado
...
{!showCode && files && files.length > 0 && (
  ...
  >
    <PreviewSandbox 
      ...
      filename={files[0]?.path || files[0]?.fileName || 'Component.tsx'}
    />
  </motion.div>
)}
 : (
