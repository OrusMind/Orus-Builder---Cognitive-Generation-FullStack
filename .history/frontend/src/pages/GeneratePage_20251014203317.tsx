...
// ✅ FUNÇÃO PARA PREPARAR CÓDIGO PARA PREVIEW
// ✅ FUNÇÃO COMPLETA E FUNCIONAL
const prepareCodeForPreview = (file: GeneratedFile): string => {
  ...
  return `
    ...
    try {
      ...
      console.log('✅ Componente ${componentName} renderizado!');
    } catch (error) {
      console.error('❌ Erro ao renderizar:', error);
      document.getElementById('root').innerHTML = \`
        <div style="padding:40px;text-align:center;background:#fee;border-radius:12px;">
          <h2 style="color:#c00;">❌ Erro ao renderizar</h2>
          <pre style="background:#fff;padding:12px;border-radius:6px;text-align:left;">\${error.message}</pre>
        </div>
      \`;
    }
  `;
...
function generatePreviewHTML(parsed: ParsedComponent, componentName: string, type: string): string {
   ...
   * Update iframe
   */
// ✅ ONDE TEM O IFRAME, USAR ASSIM:
useEffect(() => {
  if (!files || files.length === 0) return;
  
  const mainFile = files[0];
  const preparedCode = prepareCodeForPreview(mainFile);
    ...
    <body>
      ...
      <script type="text/babel">
        try {
          ...
          document.getElementById('root').innerHTML = \`
            <div style="padding:40px;text-align:center;background:#fee;">
              <h2 style="color:#c00;">Erro no Preview</h2>
...
function prepareCodeForPreview(mainFile: GeneratedFile) {
