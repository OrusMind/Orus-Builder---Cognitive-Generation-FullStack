...
const buildPreviewHTML = (): string => {
  try {
    ...
    // ✅ PARSEAR CÓDIGO PARA EXTRAIR DADOS
    const parsedData = parseComponentCode(componentFile.content);
    
    const componentName = parsedData.componentName || 'Component';
    const componentType = parsedData.type; // 'dashboard' | 'form' | 'button'
    
    console.log('🔍 Preview Real - Tipo:', componentType);
    console.log('🔍 Preview Real - Dados:', parsedData);

    // ✅ GERAR HTML BASEADO NO CÓDIGO REAL
    return generatePreviewHTML(parsedData, componentName, componentType);
...
const [showPreview, setShowPreview] = useState(true); // ✅ PREVIEW PRIMEIRO
const [viewMode, setViewMode] = useState<'desktop' | 'mobile'>('desktop');
 ...
 const handleDownload = async () => {
  ...
  try {
    ...
    // Adicionar cada arquivo com sua estrutura de pasta
    result.files.forEach((file: any) => {
      const path = file.path || file.name;
      zip.file(path, file.content || file.code);
    });
    
    // Adicionar README.md
    const readmeContent = `# ${result.projectName || 'ORUS Generated Project'}

🚀 **Gerado por ORUS Builder** - Trinity AI
...
## 📊 Estatísticas
- **Arquivos gerados:** ${result.files.length}
- **Total de linhas:** ${result.totalLines || 'N/A'}
- **Linguagem:** ${result.files[0]?.language || 'TypeScript'}
...
---
**Powered by ORUS Builder** | [orus.dev](https://orus.dev)
`;
    ...
    const fileName = `${result.projectName || 'orus-project'}.zip`;
  ...
  {showPreview ? <FileCode className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
