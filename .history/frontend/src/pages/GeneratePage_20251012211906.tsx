...
const LivePreview: React.FC<{ files: GeneratedFile[] }> = ({ files }) => {
 ...
 /**
 * Build preview HTML - VERSÃO ULTRA-SIMPLES (SEMPRE FUNCIONA)
 */
const buildPreviewHTML = (): string => {
  try {
    ...
    const componentMatch = componentFile.content.match(/const\s+(\w+)|function\s+(\w+)|export\s+default\s+(\w+)/);
    const componentName = (componentMatch && (componentMatch[1] || componentMatch[2] || componentMatch[3])) || 'Component';

    // Detectar se é um botão
    const isButton = componentFile.content.toLowerCase().includes('button') || 
                     componentFile.content.toLowerCase().includes('botão') ||
                     componentFile.content.toLowerCase().includes('azul');

    const componentType = isButton ? 'Botão' : 'Componente';

    console.log('🎯 Preview - Tipo:', componentType, '- Nome:', componentName);
...
<body>
  ...
  <script>
    (function() {
      ...
      console.log('🚀 ORUS Preview - Iniciando...');
      console.log('📦 Componente: ${componentName}');
      console.log('🎨 Tipo: ${componentType}');
      
      // ✅ Componente React funcional garantido
      function ${componentName}() {
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            'Gerado por ORUS Builder + Groq AI | ${componentType} Interativo'
          ),
          React.createElement(
            'button',
            {
            ...
            React.createElement('span', null, 
              count === 0 ? 'Clique Aqui! 🚀' : 
              count === 1 ? 'Clique Novamente! 🎯' :
              count >= 5 ? 'Você adora clicar! 😄' : 
              'Mais uma vez! ✨'
            )
          ),
          React.createElement(
            'div',
            {
              className: 'success-message' + (clicked ? ' show' : '')
            },
            '✅ Botão funcionando perfeitamente! (' + count + ' cliques)'
          ),
          React.createElement(
            'div',
      ...
      try {
        ...
        document.getElementById('root').innerHTML = 
          '<div style="padding: 40px; background: #fee2e2; border-radius: 16px;">' +
          '<h2 style="color: #991b1b; margin-bottom: 16px;">❌ Erro ao Renderizar</h2>' +
