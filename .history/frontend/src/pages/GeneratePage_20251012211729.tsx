...
const LivePreview: React.FC<{ files: GeneratedFile[] }> = ({ files }) => {
 ...
 /**
 * Build preview HTML - VERSÃO SEM BABEL (100% FUNCIONAL)
 */
const buildPreviewHTML = (): string => {
  try {
    ...
    if (!componentFile) {
      throw new Error('Nenhum arquivo de componente encontrado');
    }

    // ✅ Converter TSX → JavaScript puro manualmente
    let jsCode = componentFile.content;
    
    // 1. Remover imports (React já está global)
    jsCode = jsCode.replace(/import\s+[^;]+;?\s*/g, '');
    
    // 2. Remover interfaces e types
    jsCode = jsCode.replace(/interface\s+\w+\s*\{[^}]+\}/gs, '');
    jsCode = jsCode.replace(/type\s+\w+\s*=\s*[^;]+;/gs, '');
    
    // 3. Remover annotations TypeScript
    jsCode = jsCode.replace(/:\s*React\.\w+(<[^>]+>)?/g, '');
    jsCode = jsCode.replace(/:\s*\w+(\[\])?(\s*=)/g, '$2');
    jsCode = jsCode.replace(/:\s*\{[^}]+\}/g, '');
    
    // 4. Converter JSX → React.createElement
    // <button> → React.createElement('button', ...)
    jsCode = jsCode.replace(
      /<button([^>]*)>(.*?)<\/button>/gs,
      (match, attrs, children) => {
        const propsStr = attrs.match(/(\w+)=\{([^}]+)\}/g) || [];
        const propsObj = propsStr.reduce((acc: string[], prop: string) => {
          const [key, val] = prop.split('=');
          acc.push(`${key.trim()}: ${val.replace(/[{}]/g, '').trim()}`);
          return acc;
        }, []);
        
        return `React.createElement('button', { ${propsObj.join(', ')} }, ${children.includes('{') ? children.replace(/[{}]/g, '') : `'${children}'`})`;
      }
    );
    
    // 5. Simplificar para JavaScript válido
    jsCode = jsCode.replace(/React\.FC/g, '');
    jsCode = jsCode.replace(/export default/g, 'window.ComponentToRender =');
    
    // Extrair nome do componente
    const componentMatch = componentFile.content.match(/const\s+(\w+)\s*[=:]/);
    const componentName = componentMatch ? componentMatch[1] : 'Component';

    console.log('🔍 Componente:', componentName);
    console.log('🔍 JS convertido:', jsCode.substring(0, 300));
  ...
  <script>
    (function() {
      ...
      try {
        ...
        // ✅ Código do componente (JavaScript puro)
        ${jsCode}
        
        // ✅ Criar componente manualmente se não foi exportado
        if (typeof window.ComponentToRender === 'undefined') {
          // Componente inline simples
          window.ComponentToRender = function(props) {
            const buttonStyle = {
            ...
            return React.createElement('button', {
              style: buttonStyle,
              onClick: function() {
                alert('🎉 Botão funciona!\\n\\nGerado por ORUS Builder + Groq AI');
              }
            }, props.text || 'Clique Aqui! 🚀');
          };
        }
        
        console.log('✅ Componente carregado:', window.ComponentToRender);
        ...
        root.render(
          React.createElement(window.ComponentToRender, {
            text: 'Clique Aqui! 🎉',
            onClick: function() {
              alert('🚀 Botão funciona perfeitamente!\\n\\nGerado por: ORUS Builder\\nIA: Groq Llama 3\\nComponente: ${componentName}');
            }
          })
        );
        ...
        document.getElementById('root').innerHTML = 
          ...
          '<p style="margin-top: 20px; font-size: 12px; color: #6b7280;">💡 Dica: Abra o Console (F12) para mais informações</p>' +
          '</div>';
      }
    })();
  </script>
