...
export const GeneratePage: React.FC = () => {
  ...
  /**
 * Auto-start generation - FIXED VERSION
 */
useEffect(() => {
  // Só executa uma vez, quando montar o componente
  if (hasInitialPrompt && state.prompt && !isGenerating && !isComplete) {
    const prompt = state.prompt;
    
    // Add messages
    addUserMessage(prompt);
    addTrinityMessage(
      '🚀 Conectando com Groq API para gerar código real...',
      'info',
      true
    );
  ...
  // ✅ Dependências corretas: só reconhece mudança de rota
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, [location.pathname]); // ← Só executa quando mudar de rota
