/**
 * ORUS BUILDER - GENERATE PAGE (COMPLETE VERSION v3.0)
 * ✅ Backend REAL com Groq
 * ✅ Download ZIP
 * ✅ Live Preview
 * ✅ Chat Trinity
 */
...
// No topo, após os imports, adicione:

/**
 * Simple Preview Component (temporário)
 */
const SimplePreview: React.FC<{ files: GeneratedFile[] }> = ({ files }) => {
  ...
  return (
    <div className="w-full h-full bg-white dark:bg-gray-900 rounded-lg overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b bg-gray-50 dark:bg-gray-800">
        ...
        <span className="ml-2 text-sm font-medium">Live Preview</span>
      </div>
      <iframe
        ref={iframeRef}
...
export const GeneratePage: React.FC = () => {
  ...
  const handleGenerate = async (prompt: string, options: PromptOptions) => {
    ...
    try {
      ...
      // Parse files from different possible response formats
      ...
      addMessage('trinity', `🎉 Código REAL gerado! ${formattedFiles.length} arquivos criados!`);
      toast.success(`✅ ${formattedFiles.length} arquivos gerados com Groq!`);
...
Generated by ORUS Builder with Trinity AI
