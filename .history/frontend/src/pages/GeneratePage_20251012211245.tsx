...
const LivePreview: React.FC<{ files: GeneratedFile[] }> = ({ files }) => {
 ...
 /**
 * Build preview HTML
 * ✅ CORREÇÃO: Aceita qualquer arquivo .tsx/.jsx
 */
const buildPreviewHTML = (): string => {
  try {
    // ✅ CORREÇÃO: Procurar QUALQUER arquivo TSX/JSX (não apenas App)
    ...
    console.log('🔍 Preview usando arquivo:', componentFile.fileName);
    console.log('🔍 Componente detectado:', componentName);
    console.log('🔍 Conteúdo do arquivo:', componentFile.content.substring(0, 200));

    // Build HTML com React e Babel
    return `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>
  ...
  <script type="text/babel">
    try {
      ...
      if (ComponentToRender) {
        // Verificar se componente tem props obrigatórias
        const componentString = ComponentToRender.toString();
        const hasTextProp = componentString.includes('text') || componentString.includes('props.text');
        const hasOnClickProp = componentString.includes('onClick') || componentString.includes('props.onClick');
        
        // Renderizar com props default se necessário
        if (hasTextProp || hasOnClickProp) {
          console.log('🎯 Renderizando com props default');
          root.render(
          ...
          console.log('🎯 Renderizando sem props');
          root.render(
            <div style={{ textAlign: 'center' }}>
              <ComponentToRender />
            </div>
        ...
        throw new Error('Componente "${componentName}" não encontrado. Verifique se está usando "export default".');
      }
    } catch (error) {
      ...
      document.getElementById('root').innerHTML = 
        ...
        '<h2>❌ Preview Error</h2>' +
        '<p>' + error.message + '</p>' +
        '<small>Abra o DevTools (F12) para mais detalhes</small>' +
        '</div>';
    }
  </script>
</body>
