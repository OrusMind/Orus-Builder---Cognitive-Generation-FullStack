...
/**
 * Build preview HTML - VERSÃO GENÉRICA (FUNCIONA COM QUALQUER COMPONENTE)
 */
const buildPreviewHTML = (): string => {
  try {
    // ✅ Pegar QUALQUER arquivo .tsx/.jsx (não apenas App.tsx)
    const componentFile = files.find(f => 
      f.path.endsWith('.tsx') || 
      f.path.endsWith('.jsx') ||
      f.fileName.endsWith('.tsx') ||
      f.fileName.endsWith('.jsx')
    ) || files[0]; // Fallback: primeiro arquivo
    ...
    console.log('🔍 Preview - Arquivo:', componentFile.fileName);
    console.log('🔍 Preview - Componente:', componentName);
    console.log('🔍 Preview - Tipo:', isButton ? 'Botão' : 'Componente');
    ...
    (function() {
      ...
      function ${componentName}() {
        ...
        return React.createElement(
          ...
          React.createElement(
            'button',
            { className: 'preview-button', onClick: handleClick },
            count === 0 ? 'Clique Aqui! 🚀' : 
            count >= 5 ? 'Você adora clicar! 😄' : 
            'Clique Novamente! (' + count + ')'
          ),
      ...
      try {
        ...
        document.getElementById('root').innerHTML = 
          '<div style="padding: 40px; background: #fee; border-radius: 16px;">' +
